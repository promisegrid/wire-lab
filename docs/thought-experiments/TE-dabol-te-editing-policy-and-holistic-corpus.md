...
- `TE-34` (integer alias)
- `TE-20260502-212810` (timestamp alias and pre-migration filename)
...
Classification heuristics (apply per match, parallel to the Cat-1a / Cat-1b heuristics in DI-020-20260502-232651):
...
**This is a Cat-3 navigational refinement, not a new DI.** The locked Cat-2 procedure of DI-020-20260502-213103 is unchanged in shape — vocabulary updates still happen in place with a top-of-file note pointing at the driving TE or TODO, now also enumerating unchanged DIs (per the prior Refinement). This Refinement only adds a mandatory pre-sweep grep step and a per-match classification rule. The Cat-3 vs. Cat-5–7 boundary is preserved.

Why not a superseding DI by symmetry with DI-020-20260502-232651 (the Cat-1a/Cat-1b split)? The Cat-1 case warranted DI-supersedence because it split one category into two with materially different rules (mechanical sweep vs. leave-alone). The Cat-2 case adds procedure (enumerate DIs; grep before sweep; classify per match) without changing what Cat-2 means or when it applies. Procedural tightening of an existing category's how-to is the Cat-3 case the locked policy was designed for. Treating DF-35.3 as Cat-3 keeps it consistent with DF-35.2 (also Cat-3); DI-supersedence is reserved for category splits and meaning changes.
