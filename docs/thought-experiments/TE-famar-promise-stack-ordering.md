...
- `TE-1` (integer alias)
- `TE-20260427-180000` (timestamp alias and pre-migration filename)
...
- The spec's `Promise` shape (§1.1 of `protocols/wire-lab.d/specs/harness-spec-draft.md`) is roughly right: each frame has a `promiser`, a `assertion`, optional `body`, optional `evidence`, optional `ttl`. We are not testing the shape; we are testing how a sequence of these is laid out and consumed.
