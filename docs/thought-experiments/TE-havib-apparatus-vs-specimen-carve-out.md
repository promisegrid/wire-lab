...
**Re-presentation under Alt-B disposition path (2026-05-05): the original alts above presupposed promise-stack survives as a separate envelope hypothesis.** That presupposition is wrong when checked against the prior corpus. TE-23 ([`TE-20260430-064307-congruence-convergence-duality-and-pcid-framing.md`](TE-20260430-064307-congruence-convergence-duality-and-pcid-framing.md)) and the framing essay [`docs/essays/congruence-convergence-and-the-grid.md`](../essays/congruence-convergence-and-the-grid.md) §3.1 already establish that payload-recursion is a property of the `<payload>` slot in `grid <pcid> <payload>`, governed at per-protocol discretion, not a separate envelope class. The 2026-05-04 PromiseGrid architecture research at [`docs/research/nested-vs-stacked-envelopes-20260504.md`](../research/nested-vs-stacked-envelopes-20260504.md) further critiques the promise-stack notion as conflating single-message envelopes with message sequence streams (commands/events) and locates the disagreement on the ISConf/cfengine declarative-vs-imperative axis. Read against that prior corpus, every concrete piece of promise-stack machinery (the `Promise` struct, `[]Promise` shape, CBOR array encoding, `promstack` Wrap/Peel/Project library) is invented machinery that re-implements payload-recursion at the wrong layer.

**Alt-2.A revised: Retire promise-stack as a separate hypothesis.** Under DF-36.1 Alt-1.A the §1.1 `Promise` struct, `[]Promise` shape, CBOR array encoding, and `promstack` library API leave harness-spec — but not into `protocols/promise-stack.d/`. They go to one of three places per their actual nature: (i) anything that's just payload-recursion in disguise gets dropped (the carve-out moves the conceptual work to per-protocol specs that need it); (ii) anything that's actually a generic library belongs as library code under `internal/` or equivalent, not a protocol directory at all; (iii) the conceptual material that motivated the original exercise (sequences-of-promises-with-inherited-authority, break-witness as a frame) gets recorded as part of TE-36 itself, with explicit pointers at TE-23 §3.1 and the framing essay §3.1 as the structural argument. TE-36's DF-36.2 lock IS the retirement decision. OQ-36.6 transitions from "deferred for separate investigation" to "resolved in the negative — see DF-36.2 lock and TE-23 §3.1 / framing essay §3.1."

**Lock (re-confirmed under Alt-B disposition path, 2026-05-05): Alt-2.A revised — retire promise-stack as a separate hypothesis.** Steve confirmed retirement after the bot reframed DF-36.2 in light of the prior corpus. Cascading consequences: (1) `protocols/promise-stack.d/` is NOT created; (2) DF-36.7's `promstack` row in the grid-poc directory table has no destination and gets retired in DF-36.7's lock; (3) TE-1 (promise-stack ordering), TE-12 (promise-stack as ZK envelope), TE-13 (time-traveling break-witness) all need follow-on Cat-3 refinements that recast their conclusions in payload-recursion vocabulary, with original narrative preserved as historical-quotation per TE-34 Cat-1b — this is tracked as a new open thread T-PROMSTACK-RETIRE-CASCADE on OPEN-THREADS.md, not blocking TE-36's merge.
...
| 36.2 | Alt-2.A revised (retire promise-stack as a separate hypothesis) | Per TE-23 §3.1 + framing essay §3.1, payload-recursion under per-protocol specs is the answer; promise-stack is invented machinery at the wrong layer. |
...
- **OQ-36.1 — Which existing TEs need refinement under promise-stack retirement?** REFRAMED on 2026-05-05. Original framing ("which TEs migrate WITH the promise-stack carve-out into `protocols/promise-stack.d/`") is obsolete since DF-36.2's lock retired promise-stack as a separate hypothesis. New framing: TE-1 (promise-stack ordering), TE-12 (promise-stack as ZK envelope), TE-13 (time-traveling break-witness) all need follow-on Cat-3 refinements that recast their conclusions in payload-recursion vocabulary, with original narrative preserved as historical-quotation per TE-34 Cat-1b. Tracked as new open thread T-PROMSTACK-RETIRE-CASCADE on OPEN-THREADS.md, not blocking TE-36's merge.
...
- **OQ-36.6 — Is promise-stack actually distinct from grid-pcid-payload, or a misreading of nested messages?** RESOLVED in the negative under Alt-B disposition path on 2026-05-05. Steve flagged this concern on 2026-05-03; on re-presentation of DF-36.2 the bot found the prior corpus already answers it. Per TE-23 §3.1 and `docs/essays/congruence-convergence-and-the-grid.md` §3.1, payload-recursion is a property of the `<payload>` slot in `grid <pcid> <payload>`, governed at per-protocol discretion. The `Promise` struct, `[]Promise` shape, CBOR array encoding, and `promstack` Wrap/Peel/Project library are invented machinery re-implementing payload-recursion at the wrong layer. DF-36.2 is therefore locked at Alt-2.A revised (retire promise-stack as a separate hypothesis); `protocols/promise-stack.d/` is not created. Cascading TE-1 / TE-12 / TE-13 refinements tracked under OPEN-THREADS.md thread T-PROMSTACK-RETIRE-CASCADE.
