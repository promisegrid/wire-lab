...
Where does the promise-stack family live? §1.1 prescribes the `Promise` struct, the `[]Promise` shape, the CBOR array encoding, and the `promstack` library (Wrap/Peel/Project). §1.2 argues for it. §1.3 lists tests over it. §2.2 break-witness is a promise-frame. §7.1 transport-promise is the outermost promise-frame. TE-famar, TE-muvuv, TE-robub reference it. None of the four protocol directories created by TODO 014 is the right home for it.
