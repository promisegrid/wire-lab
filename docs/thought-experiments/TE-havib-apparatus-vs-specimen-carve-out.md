...
- `TE-36` (integer alias)
- `TE-20260503-022446` (timestamp alias and pre-migration filename)

**Decision status:** decided (all 7 DFs locked; DF-36.1, .2, .3, .7 re-confirmed under Alt-B disposition path on 2026-05-05; DF-36.4, .5, .6 locked earlier on twig; OQ-36.6 resolved in the negative).
...
The earlier TE-vipir (protocols-as-simrepos) and TE-liviv (spec-vs-implementation split) already laid the directory shape: each protocol owns a `protocols/<slug>.d/` simrepo with its own specs/, docs/thought-experiments/, TODO/, DR/ inline, and CHANGELOG.md; implementations live under top-level `implementations/<impl-name>/`. TE-magup locked per-protocol TODO/ directories. TE-zukug inverted the doc/repo reference direction (spec-doc-CID upstream, CHANGELOG entries downstream). What TE-vipir-32 did NOT do is migrate the specimen-bearing material out of the harness-spec; the migration is deferred to whenever the wire-lab carves up its envelope hypotheses, which is now.

The just-finished TODO-vuhuj created `protocols/wire-lab.d/`, `protocols/group-session.d/`, `protocols/ppx-dr.d/`, and `protocols/udp-binding.d/`. None of them is the right home for the promise-stack envelope hypothesis: wire-lab is the apparatus; group-session is a transport (and currently uses a `grid <pcid>` carrier per TE-hogus); ppx-dr is the proposal protocol; udp-binding is an L4 binding. A new `protocols/promise-stack.d/` (or analogous) is required if the promise-stack hypothesis is to have an apparatus-clean home.
...
Bob (per §9 "Realism Suggestions") writes a second `promstack` implementation in a different language from a separate reading of the spec. He needs an authoritative spec-doc CID to hash and reference in his B-side `implementations/bob-promstack/CHANGELOG.md` per TE-liviv. Today the only candidate is `harness-spec-draft.md` itself, which forces him to claim conformance to the entire harness — including ToC scenarios, ingress models, agent profiles he does not implement.
...
Dave likes the promise-stack family but wants a variant: same wire shape, different acceptance semantics (e.g. mandatory innermost-signature instead of TE-famar's still-open ordering). Per C-4, this is a normal fork.
...
**Alt-1.C: Registry-of-knobs.** Harness-spec lists every studied dimension (envelope, ledger shape, discourse vocabulary, ordering rule, criticality model) as a knob with a placeholder; candidate values fill placeholders elsewhere. Closest to TE-vipir's directory shape applied to apparatus-vs-specimen: harness-spec is itself a registry, not a doc. The prose recasts every concrete shape as `<envelope-pcid>`, `<ledger-pcid>`, `<discourse-pcid>`. Reader of harness-spec leaves with a complete picture of the dimensions being studied but no concrete shape anywhere. S1, S3, S4, S6 pass cleanly; S2 passes; S5 cannot attack because the harness-spec has nothing to delete. Cost: substantial rewrite, much higher than Alt-1.A; risk that the registry abstraction is itself a guess about the future.

**Bot recommendation: Alt-1.A (Strict).** Alt-1.B silently privileges the promise-stack family and re-creates the problem TE-havib exists to solve, just with a banner that readers will skim past. Alt-1.C is structurally the cleanest but is a year of work and presumes knob shapes we have not yet seen used in anger. Alt-1.A is the minimum carve-out that satisfies all six tabletop scenarios and is the lightest sweep through the existing harness-spec text. The Cat-2 vocabulary updates per TE-dabol editing policy are well-understood for Alt-1.A: replace `Promise` / `[]Promise` / `promstack` references with envelope-agnostic apparatus phrasing, link to per-protocol directories for the candidates. Alt-1.A also pairs cleanly with the per-DF migration decisions below — each specimen-bearing item gets one concrete destination, no shared abstraction layer required.

**Lock (re-confirmed under Alt-B disposition path, 2026-05-05): Alt-1.A.** Steve confirmed strict carve-out after re-presentation per the standing one-DF-at-a-time rule. The S6/Ellen stale-canon failure mode under Alt-1.B and the OQ-36.6-robustness argument (harness-spec needs no edits if promise-stack later turns out to be a degenerate of grid-pcid-payload) were the deciding factors over the original bot recommendation rationale.

### DF-36.2 — Promise-stack hypothesis home

Where does the promise-stack family live? §1.1 prescribes the `Promise` struct, the `[]Promise` shape, the CBOR array encoding, and the `promstack` library (Wrap/Peel/Project). §1.2 argues for it. §1.3 lists tests over it. §2.2 break-witness is a promise-frame. §7.1 transport-promise is the outermost promise-frame. TE-famar, TE-muvuv, TE-robub reference it. None of the four protocol directories created by TODO-vuhuj is the right home for it.

**Alt-2.A: New `protocols/promise-stack.d/`.** Standard simrepo shape per TE-vipir. Spec doc at `protocols/promise-stack.d/specs/promise-stack-draft.md` carries the migrated §1.1/.2/.3 plus the Promise struct + promstack API. TODO/, docs/thought-experiments/ (TE-famar relocates), MANIFEST.md, CHANGELOG.md scaffolding installed. The slug is meaningful (matches the existing `promstack` library name) and forward-compatible with future variants (`promise-stack-v2.d/`, `promise-stack-zk.d/` for TE-muvuv's ZK variant).

**Alt-2.B: New `protocols/wire-envelope.d/` neutral umbrella.** A single directory holds all envelope hypotheses (promise-stack, grid, future Carols) as sub-specs. Promotes apparatus-vs-specimen at one level higher (one envelope-comparison protocol with sub-hypotheses). Cost: one more layer of indirection; conflicts with TE-vipir's "let each protocol name its own internals" because different envelopes will want different internal shapes; collapses Carol and Dave's forks into PRs against one shared directory rather than independent forks (violates C-4 in spirit).
...
**Bot recommendation: Alt-2.A.** The slug `promise-stack` is meaningful (already used by the library name `promstack`), forward-compatible with variants and forks, and matches TE-vipir's per-protocol-simrepo shape. Alt-2.B over-couples competing hypotheses and silently violates C-4. Alt-2.C punts the carve-out without solving anything. Creating `protocols/promise-stack.d/` is a single skeleton commit (mirrors what TODO-vuhuj did for the other four protocol directories) and unblocks the harness-spec sweep.

**Re-presentation under Alt-B disposition path (2026-05-05): the original alts above presupposed promise-stack survives as a separate envelope hypothesis.** That presupposition is wrong when checked against the prior corpus. TE-lozip ([`TE-lozip-congruence-convergence-duality-and-pcid-framing.md`](TE-lozip-congruence-convergence-duality-and-pcid-framing.md)) and the framing essay [`docs/essays/congruence-convergence-and-the-grid.md`](../essays/congruence-convergence-and-the-grid.md) §3.1 already establish that payload-recursion is a property of the `<payload>` slot in `grid <pcid> <payload>`, governed at per-protocol discretion, not a separate envelope class. The 2026-05-04 PromiseGrid architecture research at [`docs/research/nested-vs-stacked-envelopes-20260504.md`](../research/nested-vs-stacked-envelopes-20260504.md) further critiques the promise-stack notion as conflating single-message envelopes with message sequence streams (commands/events) and locates the disagreement on the ISConf/cfengine declarative-vs-imperative axis. Read against that prior corpus, every concrete piece of promise-stack machinery (the `Promise` struct, `[]Promise` shape, CBOR array encoding, `promstack` Wrap/Peel/Project library) is invented machinery that re-implements payload-recursion at the wrong layer.

**Alt-2.A revised: Retire promise-stack as a separate hypothesis.** Under DF-36.1 Alt-1.A the §1.1 `Promise` struct, `[]Promise` shape, CBOR array encoding, and `promstack` library API leave harness-spec — but not into `protocols/promise-stack.d/`. They go to one of three places per their actual nature: (i) anything that's just payload-recursion in disguise gets dropped (the carve-out moves the conceptual work to per-protocol specs that need it); (ii) anything that's actually a generic library belongs as library code under `internal/` or equivalent, not a protocol directory at all; (iii) the conceptual material that motivated the original exercise (sequences-of-promises-with-inherited-authority, break-witness as a frame) gets recorded as part of TE-havib itself, with explicit pointers at TE-lozip §3.1 and the framing essay §3.1 as the structural argument. TE-havib's DF-36.2 lock IS the retirement decision. OQ-36.6 transitions from "deferred for separate investigation" to "resolved in the negative — see DF-36.2 lock and TE-lozip §3.1 / framing essay §3.1."

**Lock (re-confirmed under Alt-B disposition path, 2026-05-05): Alt-2.A revised — retire promise-stack as a separate hypothesis.** Steve confirmed retirement after the bot reframed DF-36.2 in light of the prior corpus. Cascading consequences: (1) `protocols/promise-stack.d/` is NOT created; (2) DF-36.7's `promstack` row in the grid-poc directory table has no destination and gets retired in DF-36.7's lock; (3) TE-famar (promise-stack ordering), TE-muvuv (promise-stack as ZK envelope), TE-robub (time-traveling break-witness) all need follow-on Cat-3 refinements that recast their conclusions in payload-recursion vocabulary, with original narrative preserved as historical-quotation per TE-dabol Cat-1b — this is tracked as a new open thread T-PROMSTACK-RETIRE-CASCADE on OPEN-THREADS.md, not blocking TE-havib's merge.

### DF-36.3 — Grid-envelope hypothesis home

`grid([pcid, payload])` is Steve's "we *think* it's the right shape" working hypothesis. Currently `grid <pcid>` appears as a textual carrier in `protocols/group-session.d/specs/group-session-draft.md` per TE-hogus, but the envelope-shape claim is broader than the group-session transport: `grid([pcid, payload])` is also a candidate for non-group-session transports, including the eventual canonical PromiseGrid wire format.

**Alt-3.A: New `protocols/grid-envelope.d/`.** Parallel sibling to `protocols/promise-stack.d/`. The spec doc holds the `grid([pcid, payload])` envelope hypothesis as its own object, transport-agnostic. The group-session spec then references the grid-envelope spec as the envelope it carries (per TE-liviv inversion: implementation/use-side references upstream spec-CID).
...
**Alt-3.C: New `protocols/pcid-payload.d/` with a more neutral slug.** The slug `grid` is overloaded with PromiseGrid the project; `pcid-payload` is shape-descriptive. Cost: humans and existing TEs already use the word "grid" for this envelope (per TE-hogus/26/27 vocabulary); a slug rename would force a Cat-2 vocabulary sweep across the existing TE corpus.

**Alt-3.D: Defer.** Leave the envelope shape inside group-session until a second transport demands separation.

**Bot recommendation: Alt-3.A.** Honors the transport-≠-envelope distinction Steve made this session. The grid envelope is a hypothesis on its own merits and may be adopted by transports that aren't group-session (per Steve's "we need to be able to experiment with different message envelopes" — the experiment is over the envelope axis, not the transport axis). The slug `grid-envelope` is unambiguous within the wire-lab context (no clash with `grid-poc`, no clash with PromiseGrid). Alt-3.B silently forecloses the transport-agnostic experiments. Alt-3.C creates vocabulary churn for marginal gain. Alt-3.D punts. After Alt-3.A lands, group-session's spec doc relocates its envelope claims to "carries the envelope specified at `protocols/grid-envelope.d/specs/grid-envelope-draft.md`" via TE-zukug's spec-doc-CID reference convention.

**Lock (re-confirmed under Alt-B disposition path, 2026-05-05): Alt-3.A.** Steve confirmed `protocols/grid-envelope.d/` after re-presentation. DF-36.2's resolution sharpens the case: with promise-stack retired and payload-recursion under `grid <pcid> <payload>` nominated as the universal nesting mechanism, the grid envelope is no longer specific to any one transport and must live transport-agnostically. The framing essay §3.1 nesting discussion now has a concrete spec to point at once `protocols/grid-envelope.d/specs/grid-envelope-draft.md` is created. Migration cost is small: a single skeleton commit (mirrors TODO-vuhuj) and TE-hogus's existing `grid <pcid>` content moves from group-session.d's spec to grid-envelope.d's spec, with group-session.d/specs/group-transport-draft.md updated to reference grid-envelope.d/specs/grid-envelope-draft.md as the envelope it carries. The actual scaffolding work (skeleton creation, content migration, group-session.d reference update) is a follow-on TODO, not part of TE-havib's lock.
...
**Alt-4.A: Move PT-recast vocabulary to `protocols/ppx-dr.d/`.** ppx-dr is already created (TODO-vuhuj) for "proposals as messages on a transport." It is the natural home for the conditional-promise + assessment vocabulary. ppx-dr's spec doc absorbs both primitives as the discourse layer carried by the protocol.
...
**Alt-7.B: Delete the table entirely.** Per TE-liviv, each protocol's CHANGELOG names its implementations; the harness-spec doesn't need a directory table at all. Cost: loses a useful at-a-glance map of where things live; readers have to walk all CHANGELOG files to reconstruct it. Mitigation: harness-spec links to a programmatically-generated implementation-map under `tools/`.
...
**Bot recommendation: Alt-7.A.** Cleanest split that preserves the apparatus rows (which are useful as a fast orientation aid for anyone reading harness-spec) while moving specimen rows to where they belong per TE-liviv. The `promstack` row gains a destination once `protocols/promise-stack.d/` exists (DF-36.2 Alt-2.A); the `trust` row can be deferred under DF-36.6 Alt-6.D until needed. Alt-7.B is more architecturally pure but costs orientation aid for marginal gain. Alt-7.C silently outdates.

**Lock (re-confirmed under Alt-B disposition path, 2026-05-05): Alt-7.A, modified by DF-36.2's retirement.** Steve confirmed split disposition. Concrete row dispositions per the locks just made: (1) the `promstack` row is DROPPED entirely (DF-36.2 retired the hypothesis; no destination exists); a one-line annotation in harness-spec §10 points at the framing essay §3.1 + TE-lozip §3.1 for why payload-recursion is the answer. (2) The `grid-envelope` row appears, pointing at `protocols/grid-envelope.d/specs/grid-envelope-draft.md` (DF-36.3). (3) The `trust` row stays as a deferred-destination annotation (DF-36.6 Alt-6.D lazy carve-out). (4) Apparatus rows (transport directory layout, harness-spec location, audit memos, `docs/discussion/` location) stay in harness-spec §10 with a preamble pointing readers at per-protocol documents for specimen-side detail. (5) Specimen rows for the four protocols already created by TODO-vuhuj (`group-session.d`, `wire-lab.d`, `ppx-dr.d`, `ppx-bootstrap.d`) migrate to those directories' MANIFEST.md or README files. Actual sweep edit to harness-spec is part of TODO-vuhuj's leftover sweep, not part of TE-havib's lock.
...
| 36.2 | Alt-2.A revised (retire promise-stack as a separate hypothesis) | Per TE-lozip §3.1 + framing essay §3.1, payload-recursion under per-protocol specs is the answer; promise-stack is invented machinery at the wrong layer. |
...
| 36.7 | Alt-7.A modified (split + drop promstack row + add grid-envelope row + lazy-defer trust row) | Honors all four prior locks (DF-36.1 strict, DF-36.2 retirement, DF-36.3 new home, DF-36.6 lazy); preserves orientation aid. |

## Open questions surfaced (OQ-36.x)

- **OQ-36.1 — Which existing TEs need refinement under promise-stack retirement?** REFRAMED on 2026-05-05. Original framing ("which TEs migrate WITH the promise-stack carve-out into `protocols/promise-stack.d/`") is obsolete since DF-36.2's lock retired promise-stack as a separate hypothesis. New framing: TE-famar (promise-stack ordering), TE-muvuv (promise-stack as ZK envelope), TE-robub (time-traveling break-witness) all need follow-on Cat-3 refinements that recast their conclusions in payload-recursion vocabulary, with original narrative preserved as historical-quotation per TE-dabol Cat-1b. Tracked as new open thread T-PROMSTACK-RETIRE-CASCADE on OPEN-THREADS.md, not blocking TE-havib's merge.
...
- **OQ-36.4 — Forward-pointer hygiene during sweep.** Per TE-dabol, Cat-3 navigational additions (forward-pointers from old TEs to the new TE-havib / DI / target protocol) are append-only `## Refinements` entries. Sweep must ensure every TE that referenced specimen-level material in harness-spec gains a Refinement pointing at the new home. Cheap with grep; checklist needed.

- **OQ-36.6 — Is promise-stack actually distinct from grid-pcid-payload, or a misreading of nested messages?** RESOLVED in the negative under Alt-B disposition path on 2026-05-05. Steve flagged this concern on 2026-05-03; on re-presentation of DF-36.2 the bot found the prior corpus already answers it. Per TE-lozip §3.1 and `docs/essays/congruence-convergence-and-the-grid.md` §3.1, payload-recursion is a property of the `<payload>` slot in `grid <pcid> <payload>`, governed at per-protocol discretion. The `Promise` struct, `[]Promise` shape, CBOR array encoding, and `promstack` Wrap/Peel/Project library are invented machinery re-implementing payload-recursion at the wrong layer. DF-36.2 is therefore locked at Alt-2.A revised (retire promise-stack as a separate hypothesis); `protocols/promise-stack.d/` is not created. Cascading TE-famar / TE-muvuv / TE-robub refinements tracked under OPEN-THREADS.md thread T-PROMSTACK-RETIRE-CASCADE.

- **OQ-36.5 — `transport-spec-draft.md` companion audit.** Audit memo flagged `protocols/wire-lab.d/specs/transport-spec-draft.md` (≈9 KB) as needing the same apparatus-vs-specimen audit before step 4 sweep. Likely lighter than the harness-spec audit (transport-spec was carved at TE-zalut/27 to be apparatus-level for transport-protocol differentiation), but unverified. Either folds into TODO-vuhuj's leftover sweep or files a new follow-on TODO.
