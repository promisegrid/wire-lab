...
- §2.1 (congruence camp) draws on `doc/decomk-design.md` and `doc/isconf-design.md` from `github.com/stevegt/decomk` (read in full), and on Traugott & Brown, "Why Order Matters: Turing Equivalence in Automated Systems Administration," LISA '02 (read in full via [USENIX](https://www.usenix.org/legacy/events/lisa2002/tech/full_papers/traugott/traugott.pdf)). The Turing-equivalence quote and the cost-ordering inequality come directly from that paper. The canonical home for both "Bootstrapping an Infrastructure" and "Why Order Matters" is [infrastructures.org](http://www.infrastructures.org/) (see its [papers index](http://www.infrastructures.org/bootstrap/papers.shtml)), which is itself an expansion of the original "Bootstrapping an Infrastructure" paper and carries the most-recent text of both papers; the USENIX PDFs linked above are the point-in-time LISA proceedings versions and may differ from the current text.
