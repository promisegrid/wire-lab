...
- Source row/title: Seed application catalog entry `multicurrency-transactions`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against multicurrency-transactions application
pressure: Exchange-rate promises, settlement, local valuation, accounting, and currency-
risk evidence.
...
Alice depends on an outcome in the Multicurrency Transactions domain. Bob makes promises
about exchange-rate promises, settlement, local valuation, accounting, and currency-risk
evidence. Carol needs enough evidence to rely on Bob's promise without having complete
...
- 100-year durability: The Multicurrency Transactions pressure should still be understandable after tools,
...
`results/<sim-id>/multicurrency-transactions/<model-id>/<YYYYMMDD-HHMMSS>.md`
