...
- Source row/title: Seed application catalog entry `software-supply-chain-attestation`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against software-supply-chain-attestation
application pressure: Build provenance, dependency promises, signing, vulnerability
...
Alice depends on an outcome in the Software Supply Chain Attestation domain. Bob makes
promises about build provenance, dependency promises, signing, vulnerability response,
and reproducibility. Carol needs enough evidence to rely on Bob's promise without having
...
- 100-year durability: The Software Supply Chain Attestation pressure should still be understandable after
tools, organizations, people, keys, policies, and infrastructure change over a century.
...
`results/<sim-id>/software-supply-chain-attestation/<model-id>/<YYYYMMDD-HHMMSS>.md`
