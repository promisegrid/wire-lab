...
- Source row/title: Seed application catalog entry `devops`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against devops application pressure: Deploy,
rollback, monitoring, incident response, access control, and postmortem promises.
...
Alice depends on an outcome in the Devops domain. Bob makes promises about deploy,
rollback, monitoring, incident response, access control, and postmortem promises. Carol
...
- 100-year durability: The Devops pressure should still be understandable after tools, organizations, people,
...
`results/<sim-id>/devops/<model-id>/<YYYYMMDD-HHMMSS>.md`
