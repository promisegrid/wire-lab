# Maliciously similar name Matrix

## Authority Boundary

This matrix summarizes evidence. It does not declare a winning design by itself.
Source: `DI-faros`; `DI-vabor`; `DI-dimas`; `DI-nanih`.

| Simulation | Scenario | Latest result run | Status | Notes |
|---|---|---|---|---|
| `SIM-ligan-promisebase-reference-naming` | `promisebase-reference-naming-maliciously-similar-name` |  | not-run | Source simulation for the mined row. |
