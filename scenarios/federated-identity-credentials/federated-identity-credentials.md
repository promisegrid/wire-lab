...
- Source row/title: Seed application catalog entry `federated-identity-credentials`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against federated-identity-credentials
application pressure: Issuer trust, credential presentation, revocation, selective
...
Alice depends on an outcome in the Federated Identity Credentials domain. Bob makes
promises about issuer trust, credential presentation, revocation, selective disclosure,
and verifier policy. Carol needs enough evidence to rely on Bob's promise without having
...
- 100-year durability: The Federated Identity Credentials pressure should still be understandable after tools,
...
`results/<sim-id>/federated-identity-credentials/<model-id>/<YYYYMMDD-HHMMSS>.md`
