...
- Source row/title: Seed application catalog entry `makerspace-dues-access`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against makerspace-dues-access application
pressure: Dues payment, membership standing, door access, tool access, and
...
Alice depends on an outcome in the Makerspace Dues Access domain. Bob makes promises
about dues payment, membership standing, door access, tool access, and
suspension/reinstatement. Carol needs enough evidence to rely on Bob's promise without
...
- 100-year durability: The Makerspace Dues Access pressure should still be understandable after tools,
...
`results/<sim-id>/makerspace-dues-access/<model-id>/<YYYYMMDD-HHMMSS>.md`
