...
- Source row/title: Seed application catalog entry `air-traffic-control`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against air-traffic-control application pressure:
Separation, handoff, clearances, stale data, emergency priority, and real-time
coordination.
...
Alice depends on an outcome in the Air Traffic Control domain. Bob makes promises about
separation, handoff, clearances, stale data, emergency priority, and real-time
...
- 100-year durability: The Air Traffic Control pressure should still be understandable after tools,
...
`results/<sim-id>/air-traffic-control/<model-id>/<YYYYMMDD-HHMMSS>.md`
