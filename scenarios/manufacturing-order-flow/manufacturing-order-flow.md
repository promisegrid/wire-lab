...
- Source row/title: Seed application catalog entry `manufacturing-order-flow`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against manufacturing-order-flow application
pressure: Work orders, materials, machine availability, quality checks, and shipment
handoff.
...
Alice depends on an outcome in the Manufacturing Order Flow domain. Bob makes promises
about work orders, materials, machine availability, quality checks, and shipment
handoff. Carol needs enough evidence to rely on Bob's promise without having complete
...
- 100-year durability: The Manufacturing Order Flow pressure should still be understandable after tools,
...
`results/<sim-id>/manufacturing-order-flow/<model-id>/<YYYYMMDD-HHMMSS>.md`
