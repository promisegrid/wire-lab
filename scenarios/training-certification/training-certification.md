...
- Source row/title: Seed application catalog entry `training-certification`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against training-certification application
pressure: General credential issuance, renewal, revocation, prerequisites, and verifier
trust.
...
Alice depends on an outcome in the Training Certification domain. Bob makes promises
about general credential issuance, renewal, revocation, prerequisites, and verifier
trust. Carol needs enough evidence to rely on Bob's promise without having complete
...
- 100-year durability: The Training Certification pressure should still be understandable after tools,
...
`results/<sim-id>/training-certification/<model-id>/<YYYYMMDD-HHMMSS>.md`
