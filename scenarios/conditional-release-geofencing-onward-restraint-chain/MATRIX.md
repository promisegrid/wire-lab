# Onward-restraint chain Matrix

## Authority Boundary

This matrix summarizes evidence. It does not declare a winning design by itself.
Source: `DI-faros`; `DI-vabor`; `DI-dimas`; `DI-nanih`.

| Simulation | Scenario | Latest result run | Status | Notes |
|---|---|---|---|---|
| `SIM-zarud-conditional-release-geofencing` | `conditional-release-geofencing-onward-restraint-chain` |  | not-run | Source simulation for the mined row. |
