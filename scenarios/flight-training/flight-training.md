...
- Source row/title: Seed application catalog entry `flight-training`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against flight-training application pressure:
Student progress, instructor endorsements, aircraft checkout, weather minimums, and
...
Alice depends on an outcome in the Flight Training domain. Bob makes promises about
student progress, instructor endorsements, aircraft checkout, weather minimums, and
...
- 100-year durability: The Flight Training pressure should still be understandable after tools, organizations,
people, keys, policies, and infrastructure change over a century.
...
`results/<sim-id>/flight-training/<model-id>/<YYYYMMDD-HHMMSS>.md`
