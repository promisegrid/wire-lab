...
- Source row/title: Seed application catalog entry `erp`
- Source DI / TODO / TE: `DI-faros`; `DI-vabor`; `DI-botup`; `DI-midif`; `TODO-dadub`

## Purpose

Exercise PromiseGrid design candidates against erp application pressure: Multi-
department operational promises spanning orders, inventory, purchasing, accounting, and
fulfillment.
...
Alice depends on an outcome in the ERP domain. Bob makes promises about multi-department
operational promises spanning orders, inventory, purchasing, accounting, and
fulfillment. Carol needs enough evidence to rely on Bob's promise without having
...
- 100-year durability: The ERP pressure should still be understandable after tools, organizations, people,
keys, policies, and infrastructure change over a century.
...
`results/<sim-id>/erp/<model-id>/<YYYYMMDD-HHMMSS>.md`
