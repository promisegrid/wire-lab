# Token rotates among writers Matrix

## Authority Boundary

This matrix summarizes evidence. It does not declare a winning design by itself.
Source: `DI-faros`; `DI-vabor`; `DI-dimas`; `DI-nanih`.

| Simulation | Scenario | Latest result run | Status | Notes |
|---|---|---|---|---|
| `SIM-narok-transport-family-bakeoff` | `transport-family-bakeoff-token-rotates-among-writers` |  | not-run | Source simulation for the mined row. |
