# Duplicate advertisement Matrix

## Authority Boundary

This matrix summarizes evidence. It does not declare a winning design by itself.
Source: `DI-faros`; `DI-vabor`; `DI-dimas`; `DI-nanih`.

| Simulation | Scenario | Latest result run | Status | Notes |
|---|---|---|---|---|
| `SIM-zazit-chunk-feed-replication` | `chunk-feed-replication-duplicate-advertisement` |  | not-run | Source simulation for the mined row. |
