...
- **[TE-21: `grid <pcid>` as a repo-local channel-message carrier](docs/thought-experiments/TE-20260430-204108-grid-pcid-channel-carrier.md)** — Compares `grid <pcid>`, ordinary protocol headers, filename/path-selected protocols, and a pure structured object for the repo-local `channels/` experiment. Current finding: use `grid <pcid>` for repo-local channels so the repo can test pCID-selected dispatch, CID-linked receipts, and explicit promise bodies without prematurely freezing the canonical PromiseGrid wire format. Source: `DI-009-20260430-204108`; open graduation question: `DR-009-20260430-204108`.
