...
- Cell ID: `ga-canary-20260525-goban-dalor-zukis-v4-000015-SIM-vinag-child-tag42-port-observation-pairing--l6-cas-starting-profile-bakeoff-long-horizon-reprofile--openai-gpt-5.4-medium`
- Simulation ID: `SIM-vinag-child-tag42-port-observation-pairing`
- Scenario ID: `l6-cas-starting-profile-bakeoff-long-horizon-reprofile`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `proposals/ga-canary-20260525-goban-dalor-zukis-v4/results/SIM-vinag-child-tag42-port-observation-pairing/l6-cas-starting-profile-bakeoff-long-horizon-reprofile/openai-gpt-5.4-medium/20260525-152501.json`
...
### `proposals/ga-canary-20260525-goban-dalor-zukis-v4/simulations/SIM-vinag-child-tag42-port-observation-pairing/README.md`

```markdown
# SIM-vinag-child-tag42-port-observation-pairing

This child keeps the stronger outer-envelope discipline from `SIM-zukis-grid-envelope-tag42-protocol-owned-slot2-varsig` and the explicit sibling-proof audit emphasis from `SIM-dalor-grid-envelope-protocol-owned-signature-slot`, then repairs the main repeated weakness in both parents: neither one directly gives first-port teams or promise-accounting reviewers a standard payload record to audit.

## Design move

This simulation tests one bounded pairing:

1. keep the outer specimen `grid([42(pCID), payload, varsig])` with signable bytes over `[42(pCID), payload]`;
2. add a pCID-selected `port-implementation-record` payload protocol so a real port can name promisers, supported and unsupported pCIDs, host assumptions, exact-byte evidence policy, local API bindings, CID-rooted references, and checkpoint rules;
3. add a pCID-selected `service-observation-record` payload protocol so kept service, honest refusal, timeout/silence, corruption, unsupported-pCID, and host-unavailable outcomes are auditable without pushing those semantics into the base envelope.

## Why this should score better

Compared with both parents, this child should improve scenario fit on:

- `kernel-porting-boundary` by giving a concrete per-promiser implementation promise record;
- `promise-accounting-records-kept-storage-promise` by giving an observable kept outcome record and local future-policy fields;
- `promise-accounting-records-refused-service` by distinguishing signed refusal from timeout, corruption, and silence;
- `cas-object-model-dag-cbor-interop` by defining CID-bearing payload fields that are DAG-CBOR-compatible without depending on IPLD stacks;
- `l6-cas-starting-profile-bakeoff-long-horizon-reprofile` by allowing CID-rooted references to carry optional successor/freeze explanation fields while keeping old objects addressable.

## Boundaries

- The envelope still only carries current-sender evidence for the current sender's own promise.
- No promiser promises on behalf of another promiser.
- Split local services are expressed as multiple signed `port-implementation-record`s, one per promiser, with references between them.
- Refusal semantics, exact-byte dispute evidence, and successor explanations live in the pCID-selected payload protocols, not in extra outer selectors.
- Carriage is not semantic acceptance; every receiver keeps local trust judgments.
```

### `proposals/ga-canary-20260525-goban-dalor-zukis-v4/simulations/SIM-vinag-child-tag42-port-observation-pairing/QUESTION.md`

```markdown
# Question

Can PromiseGrid improve first-port auditability and promise-accounting clarity by keeping a tagged outer specimen `grid([42(pCID), payload, varsig])` while standardizing two pCID-selected payload protocols: one for per-promiser port implementation promises and one for observable service outcomes including signed refusal, timeout, corruption, CID-rooted references, and optional freeze-successor explanations?
```

### `proposals/ga-canary-20260525-goban-dalor-zukis-v4/simulations/SIM-vinag-child-tag42-port-observation-pairing/protocols/grid-envelope.d/specs/grid-envelope-draft.md`

```markdown
# Grid-envelope draft: tag-42 paired with auditable payload records

> **Status: DRAFT.** Not frozen. This is a specimen inside `SIM-vinag-child-tag42-port-observation-pairing`.

## Scope

This specimen keeps the locked family direction:

```text
grid([42(pCID), payload, ...])
```

and tests one direct member:

```text
grid([42(pCID), payload, varsig])
```

`pCID` is Protocol CID: the content identifier of the protocol specification document.

## Envelope shape

- slot `0`: `42(pCID)`
- slot `1`: opaque `payload` bytes until interpreted by the protocol named by `pCID`
- slot `2`: `varsig` proof bytes defined by that same `pCID`

This simulation pairs that envelope with two payload protocols, but the envelope does not itself define kernel promises, refusal classes, or CAS migration semantics.

## Signable bytes

The baseline signable view is the canonical bytes of:

```text
[42(pCID), payload]
```

The sender's outer proof is evidence only for the sender's own scoped promise that these payload bytes conform to the protocol named by `pCID`.

## Unknown pCID policy

If a receiver does not support `pCID`, it may preserve or relay the exact envelope bytes under local policy, but it MUST NOT claim to parse payload semantics or verify the proof.

## Exact-byte evidence

This simulation expects payload protocols to reference or preserve exact outer bytes when needed for:

- signed refusal evidence;
- unsupported-pCID carriage;
- replay or proof disputes;
- corruption or mismatch claims.

Those rules are defined by the payload protocol, not by extra outer selectors.

## Comparison pressure

This specimen preserves the stronger `SIM-zukis` alignment with `grid([42(pCID), payload, ...])` while making the missing higher-layer audit records concrete through paired payload pCIDs rather than through more envelope machinery.
```

### `proposals/ga-canary-20260525-goban-dalor-zukis-v4/simulations/SIM-vinag-child-tag42-port-observation-pairing/protocols/port-implementation-record.d/specs/port-implementation-record-draft.md`

```markdown
# Port-implementation-record draft

> **Status: DRAFT.** Payload protocol specimen for first-port auditability.

## Purpose

This payload protocol lets a promiser publish a concrete implementation promise for a PromiseGrid port without confusing harness apparatus with the porting target.

## Record rule

Each record is a promise by one promiser about that promiser's own behavior only.

For split local services, each service emits its own signed record. A coordinator may reference those records by CID, but MUST NOT promise their behavior on their behalf.

## Payload shape

Deterministic CBOR map with these fields:

- `kind`: `port_implementation_record_v1`
- `promiser`
- `port_label`
- `roles[]`: entries with:
  - `role_name` (`storage`, `dispatch`, `net_send`, `net_receive`, `keys`, `lifecycle`, `evidence`, `compute`, `namespace`, `checkpoint`, or another named role)
  - `supported_message_pCIDs[]`
  - `unsupported_message_pCIDs[]`
  - `unsupported_roles[]`
  - `service_conditions[]` for bounded availability promises
  - `refusal_mode` (`direct-refusal`, `non-promise`, or both)
- `host_assumptions[]`: dependencies not promised by this promiser
- `local_api_bindings[]`: local API name to underlying `grid([42(pCID), payload, ...])` message pCID mapping
- `exact_byte_evidence_policy`: what exact bytes are retained for proof, replay, unsupported-pCID carriage, and broken-promise evidence
- `component_refs[]`: CIDs of other promisers' own signed records
- `namespace_policy`: voluntary reciprocal promises only; no global namespace authority claim
- `cid_reference_policy`: receiver chooses whether and where to mount a CID-rooted reference into local view
- `checkpoint_policy`: current resource state is reconstructed from selected promise/event-log frontier
- `deferred_freeze_questions[]`: draft dependencies explicitly left open pending later freeze decisions

## Required semantics

A valid record for this simulation MUST make these things explicit:

- what the promiser implements now;
- which pCID-selected messages it supports now;
- which roles and pCIDs it refuses or does not promise;
- which host/runtime facts it depends on but does not promise;
- where exact-byte evidence is kept;
- whether local APIs are adapters over pCID-selected messages;
- what remains provisional until future guide or freeze decisions.

## Scenario notes

- **Browser/WASM:** browser storage, clocks, and lifecycle belong in `host_assumptions[]` unless separately promised by another promiser.
- **Mobile sandbox:** foreground/background limits belong in `service_conditions[]` or `host_assumptions[]`, not hidden success.
- **MCU/header-only:** a minimal credible record may list only one actuator pCID and one bounded evidence store.
- **Split local services:** use multiple records linked by `component_refs[]`.

## Non-goals

This protocol does not create global permission, universal kernel shape, or global namespace truth.
```

### `proposals/ga-canary-20260525-goban-dalor-zukis-v4/simulations/SIM-vinag-child-tag42-port-observation-pairing/protocols/service-observation-record.d/specs/service-observation-record-draft.md`

```markdown
# Service-observation-record draft

> **Status: DRAFT.** Payload protocol specimen for kept/refused/timeout/corrupt outcome accounting.

## Purpose

This protocol defines observable outcome vocabulary without moving promise-accounting semantics into the base envelope.

It supports two record kinds under one payload protocol:

- `service_refusal_v1`: a promiser's signed refusal record
- `service_observation_v1`: a local observer's outcome record

## Shared reference object

Records in this protocol may use an `object_ref` map with:

- `root_cid`
- `content_profile_pCID`
- `selector_or_path` (optional)
- `frontier_ref` (optional)
- `exact_bytes_ref` (optional)
- `successor_ref` (optional)

`successor_ref` is explanatory only. It may point to a later profile or freeze-successor record, but it does not rewrite the identity of the old object. Old raw chunks and old pointer objects remain addressable by their original CID.

CID-bearing fields SHOULD use DAG-CBOR-compatible link encoding when carried in CBOR, but the protocol does not require an IPLD runtime.

## `service_refusal_v1`

Deterministic CBOR map:

- `kind`: `service_refusal_v1`
- `promiser`
- `request_ref`
- `service_pCID`
- `reason_class` (`policy`, `cost`, `group-context`, `missing-authorization`, `unsupported-pcid`, `host-unavailable`, or another named class)
- `object_ref` (optional)
- `retry_hint` (optional)
- `notes` (optional)
...
- A refusal is an honest non-performance record, not corruption and not timeout.
- The refusal is signed by the current sender in the outer envelope like any other payload.

## `service_observation_v1`

Deterministic CBOR map:

- `kind`: `service_observation_v1`
- `observer`
- `subject_promiser`
- `request_ref`
- `service_pCID`
...
- `outcome`
- `evidence_refs[]`
- `local_policy_effects` (optional local-only fields such as future `pull`, `keep`, or `advertise` choices)
- `notes` (optional)

Allowed `outcome` values in this simulation:
...
- `kept`: the observer received bytes or behavior matching the promised service and stores enough evidence to relate the observation to the request.
- `refused`: exact refusal evidence is preserved, usually a referenced `service_refusal_v1` envelope or exact bytes thereof.
- `timeout`: the observer did not receive valid service or signed refusal before the local deadline.
- `corrupt`: bytes arrived but failed integrity, proof, or promised-byte matching checks.
- `unsupported-pcid`: the subject directly refused or clearly non-promised the unsupported protocol.
- `host-unavailable`: the subject had previously scoped service to host conditions and recorded unavailability honestly.

A receiver MUST NOT collapse `refused` into `timeout` or `corrupt` when signed refusal evidence is available.

## Exact-byte evidence

For `refused`, `corrupt`, and `unsupported-pcid`, the observer SHOULD preserve exact relevant envelope bytes directly or by CID reference so later auditors can distinguish what was actually observed from local interpretation.

## Local trust

`local_policy_effects` are local receiver choices only. This protocol does not create a global trust score or global reputation fact.
```
