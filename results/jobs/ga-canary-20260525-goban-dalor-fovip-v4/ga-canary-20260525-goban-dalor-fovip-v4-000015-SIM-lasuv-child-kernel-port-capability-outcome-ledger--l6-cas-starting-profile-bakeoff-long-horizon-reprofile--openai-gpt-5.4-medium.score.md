...
- Cell ID: `ga-canary-20260525-goban-dalor-fovip-v4-000015-SIM-lasuv-child-kernel-port-capability-outcome-ledger--l6-cas-starting-profile-bakeoff-long-horizon-reprofile--openai-gpt-5.4-medium`
- Simulation ID: `SIM-lasuv-child-kernel-port-capability-outcome-ledger`
- Scenario ID: `l6-cas-starting-profile-bakeoff-long-horizon-reprofile`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `proposals/ga-canary-20260525-goban-dalor-fovip-v4/results/SIM-lasuv-child-kernel-port-capability-outcome-ledger/l6-cas-starting-profile-bakeoff-long-horizon-reprofile/openai-gpt-5.4-medium/20260525-152502.json`
...
### `proposals/ga-canary-20260525-goban-dalor-fovip-v4/simulations/SIM-lasuv-child-kernel-port-capability-outcome-ledger/README.md`

```markdown
# SIM-lasuv-child-kernel-port-capability-outcome-ledger

This child keeps `SIM-fovip-kernel-promise-boundary-port-contract` as the center of gravity: the developer-facing porting boundary is still a kernel implementation promise record, not a global certificate, not a universal process shape, and not a hidden host API.

It repairs two weak spots from that parent by borrowing one bounded idea from `SIM-dalor-grid-envelope-protocol-owned-signature-slot`:

- exact-byte witness discipline becomes explicit and auditable;
- protocol-owned proof handling can be declared per supported `pCID`;
- but there is **no** universal mandatory outer proof slot.

## Candidate specimen

A port publishes two deterministic DAG-CBOR artifacts:

1. a `kernel_port_record` that names local promisers, per-`pCID` capabilities, host assumptions, unsupported behavior, adapters, namespaces, references, and outcome policy;
2. a `local_promise_outcome_record` that logs what the local observer concluded about a concrete attempt or observed message.

## Design deltas

### 1. Per-`pCID` capability table instead of a coarse supported list

Each supported or carried protocol gets its own entry naming:

- the local promiser;
- receive behavior;
- send behavior;
- proof handling behavior;
- exact-byte retention level;
- unsupported/refusal behavior.

This makes claims such as "blind-carry only", "validate and dispatch", or "verify protocol-owned proof" auditable instead of implied.

### 2. Concrete local outcome ledger

The parent named outcome categories but left the record shape too open. This child adds one local record shape that distinguishes:
...
Each record carries a witness as exact bytes or a CID reference to exact bytes, so refusal is not confused with silence and replay/audit evidence stays local and durable.

### 3. Protocol-owned proof declarations without envelope overreach

From `SIM-dalor`, this child keeps the useful audit idea that a port may promise to preserve or verify exact protocol-shaped bytes and, where a protocol defines it, verify proof over canonical `[pCID, payload]` bytes.

Unlike `SIM-dalor`, this child does **not** require a universal third slot or reopen the base envelope. Proof semantics remain owned by the payload protocol named by `pCID`.

## Why this should score higher

- Preserves `SIM-fovip`'s strong kernel/app/host boundary and local-trust framing.
- Answers the open question about a concrete refusal/kept evidence record shape.
- Improves kept-storage and refused-service scenario fit because Bob's local accounting artifact is now explicit.
- Improves DAG-CBOR and long-horizon reprofile auditability because the kernel record and outcome witnesses are CID-addressable DAG-CBOR artifacts.
- Avoids `SIM-dalor`'s main risk by not imposing a universal mandatory outer signature slot.

## Boundaries

- `pCID` always means Protocol CID.
- Higher-layer payload protocols still own application semantics, signed refusal bodies, transfer rules, promise-token behavior, and CAS object-model details.
- This simulation only asks what a port promises to preserve, verify, emit, refuse, or log locally.
- Each receiver keeps local trust judgments.
```

### `proposals/ga-canary-20260525-goban-dalor-fovip-v4/simulations/SIM-lasuv-child-kernel-port-capability-outcome-ledger/QUESTION.md`

```markdown
# Question

Can a first PromiseGrid port publish one deterministic kernel port record plus deterministic local outcome records such that:

- app/kernel crossings still map to `pCID`-selected messages;
- each storage, compute, network, key, device, lifecycle, dispatch, namespace, reference, and evidence promise names its local promiser;
- refusal is recorded distinctly from unavailability, corruption, broken promise, and timeout/silence;
- exact bytes or CID-rooted witnesses are retained where needed for proof, replay, unsupported-`pCID` carriage, and later audit;
- protocol-owned proof verification can be declared per supported `pCID` without imposing a universal outer signature slot; and
- the artifact remains credible across native, browser/WASM, mobile, MCU, and split local-service ports?
```

### `proposals/ga-canary-20260525-goban-dalor-fovip-v4/simulations/SIM-lasuv-child-kernel-port-capability-outcome-ledger/protocols/kernel-port-ledger.d/specs/kernel-port-ledger-draft.md`

```markdown
# Kernel port capability + outcome ledger draft

> Status: DRAFT. Simulation-local specimen only.

This draft extends the kernel implementation promise record approach with concrete per-`pCID` capability declarations and concrete local outcome records. It is meant to make first-port claims and audits sharper without freezing a universal envelope or universal application protocol.

## Scope

This draft defines:

- a deterministic `kernel_port_record`;
- a deterministic `local_promise_outcome_record`;
- a small vocabulary for per-`pCID` capability claims.

It does **not** define a final PromiseGrid kernel API, a universal object model, a universal refusal payload, or a universal outer signature slot.

## Encoding profile

The current draft encoding profile is deterministic DAG-CBOR positional arrays.

- CIDs and CID links are allowed.
- Exact byte witnesses may be carried inline as bytes or by CID reference to separately retained bytes.
- The use of DAG-CBOR here is for the audit artifact itself; it does not make DAG-CBOR the universal payload format for all higher-layer protocols.
...
`role_name` may name storage, compute, network-send, network-receive, key-use, device-access, lifecycle, dispatch, evidence, namespace, reference, checkpoint, or another explicit local role.

### `pcid_capability_entry`
...
outcome_policy = [
  encoding_profile,
  default_witness_mode,
  retention_minima,
  correlation_policy
]
```

### `local_promise_outcome_record`
...
This is intentionally small so split local-service ports can correlate dispatch, storage, network, and evidence records without pretending they are one promiser.

## Capability vocabulary

### `receive_mode`

- `none` — the promiser does not promise to receive or handle this `pCID`.
- `blind-carry-exact` — the promiser may preserve and relay exact bytes without claiming to parse them.
- `parse` — the promiser promises to parse enough to recognize local structure but not full semantic validation.
- `validate-dispatch` — the promiser promises protocol-aware validation and local dispatch.

### `send_mode`

- `none`
- `emit-pass-through` — can emit exact previously retained bytes.
- `emit-local` — can construct and emit fresh protocol-shaped bytes locally.

### `proof_mode`

- `protocol-owned-none`
- `protocol-owned-preserve-only`
- `protocol-owned-verify`
- `protocol-owned-emit-verify`

`proof_mode` never creates a universal outer proof rule. It only says what this port promises for protocols that define their own proof semantics.

If a protocol specimen uses exact canonical `[pCID, payload]` bytes as the signable prefix, this entry may say whether the port preserves or verifies that proof. If a protocol does not define such a proof, `proof_mode` must not imply one.
...
- `whole-message-bytes`
- `cid-of-whole-message`

This field makes exact-byte evidence auditable. A port cannot quietly advertise broad support while retaining too little evidence to prove what it actually observed.
...
Unsupported behavior must be named directly per `pCID` where relevant.

## Outcome classes

`local_promise_outcome_record.outcome_class` is one of:
...
These are local evidence classes, not universal application semantics.

- `refused` means the observer has evidence of an explicit refusal.
- `timeout-silence` means the observer lacks such refusal evidence.
- `unverifiable-proof` means bytes were observed but proof verification promised by the relevant `pcid_capability_entry` did not succeed.

If a higher-layer payload protocol defines a signed refusal body, that body remains owned by that protocol; the kernel outcome record only classifies the local observation and points at retained bytes or a CID witness.

## Semantics

### Local promiser discipline

A port promises only its own behavior. Host browsers, OS schedulers, runtimes, radios, clocks, or storage layers are listed in `host_assumptions` unless they are also explicit promisers with their own entries.

### Adapter discipline

If a local API wraps a PromiseGrid operation, `adapter_promises` must identify the corresponding `pCID`-selected message family or state that the operation is outside the PromiseGrid boundary.

### Split local-service discipline

In split deployments, `observer` and `promiser` may differ:

- a dispatch service may observe;
- a storage service may be the promiser;
- an evidence service may retain the witness bytes.

The `correlation` field ties those records together without collapsing them into one authority.

### DAG-CBOR and CAS interop discipline

This draft only promises DAG-CBOR compatibility for the audit artifact defined here. Claims about DAG-CBOR/IPLD-style payload objects must appear as explicit `pcid_capability_entry` claims for the relevant higher-layer protocol `pCID`.

### Long-horizon reprofile discipline

Because the audit artifacts themselves are deterministic and CID-addressable, old capability records, old witnesses, and old outcome records can remain explainable after later payload profiles land. This draft does not solve old object semantics by itself; it preserves the local evidence boundary so later agents can still inspect what was claimed and observed.

## Minimum credible small-port rule

A tiny MCU or header-only port may publish narrow records:

- empty `adapter_promises`, `namespace_promises`, or `reference_promises` lists are allowed;
- `pcid_capabilities` may contain only one supported `pCID` plus explicit non-promises for the rest;
- `bytes_retained` may be small, but it must be honest;
- outcome logging may be bounded, but the port must still distinguish `refused` from `timeout-silence` whenever it claims to observe both.

## Worked scenario mapping

### Kept storage promise

Bob's local node can record:

- `subject_pcid`: the storage protocol `pCID`;
- `outcome_class`: `kept`;
- `witness`: CID of exact response bytes or inline exact bytes;
- `correlation`: the local request attempt ID.

This gives Bob a durable local basis for later pull, keep, or advertise choices.

### Refused service

If Alice explicitly refuses, Bob records `refused`. If Alice never answers, Bob records `timeout-silence`. If bytes arrive but promised proof verification fails, Bob records `unverifiable-proof`. These are not merged.

### Kernel porting boundary

A native node, browser/WASM adapter, mobile sandbox, MCU, or split local-service graph can all use the same record shape while naming different promisers and different host assumptions.

## Non-goals

- no global certificate semantics;
- no universal namespace authority;
- no universal claim-card layer;
- no mandatory outer signature slot;
- no attempt to force higher-layer storage, refusal, transfer, or CAS object semantics into the kernel record.

## Authority boundary

This is evidence only. It sharpens what a port claims and what a local observer records. Payload protocols, future frozen `pCID`s, and local trust assessments still decide the rest.
```
