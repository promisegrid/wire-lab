...
- Cell ID: `ga-tugoz-20260523-073108-suzuf-000010-SIM-fitin-gordian-payload-wrapper--chunk-feed-replication-sparse-advertisement--openai-gpt-5.4-medium`
- Simulation ID: `SIM-fitin-gordian-payload-wrapper`
- Scenario ID: `chunk-feed-replication-sparse-advertisement`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `results/SIM-fitin-gordian-payload-wrapper/chunk-feed-replication-sparse-advertisement/openai-gpt-5.4-medium/20260523-073120.json`
...
### `simulations/SIM-fitin-gordian-payload-wrapper/README.md`

```markdown
# SIM-fitin-gordian-payload-wrapper: Gordian payload/wrapper probe

This simulation is a standalone grid-envelope specimen. It keeps the universal
outer grid minimal and moves Gordian-style subject/assertion/proof structure
into a pCID-owned payload/wrapper family instead of treating Gordian semantics
as universal envelope doctrine. Source: `DI-nohir`.

The local draft spec is
`protocols/grid-envelope.d/specs/grid-envelope-draft.md`.

Primary comparison targets: `SIM-maraz` and `SIM-natim`.
```

### `simulations/SIM-fitin-gordian-payload-wrapper/QUESTION.md`

```markdown
# Question

Can Gordian-style subject/assertion/proof structure improve PromiseGrid
auditability when it is confined to a pCID-owned payload/wrapper family, or
does it still add more wrapper complexity than the minimal grid needs? Source:
`DI-nohir`.
```
