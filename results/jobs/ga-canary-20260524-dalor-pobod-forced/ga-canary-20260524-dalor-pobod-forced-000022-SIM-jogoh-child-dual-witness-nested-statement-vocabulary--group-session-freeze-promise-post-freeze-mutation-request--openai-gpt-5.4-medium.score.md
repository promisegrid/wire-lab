...
- Cell ID: `ga-canary-20260524-dalor-pobod-forced-000022-SIM-jogoh-child-dual-witness-nested-statement-vocabulary--group-session-freeze-promise-post-freeze-mutation-request--openai-gpt-5.4-medium`
- Simulation ID: `SIM-jogoh-child-dual-witness-nested-statement-vocabulary`
- Scenario ID: `group-session-freeze-promise-post-freeze-mutation-request`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `proposals/ga-canary-20260524-dalor-pobod-forced/results/SIM-jogoh-child-dual-witness-nested-statement-vocabulary/group-session-freeze-promise-post-freeze-mutation-request/openai-gpt-5.4-medium/20260524-190507.json`
...
### `proposals/ga-canary-20260524-dalor-pobod-forced/simulations/SIM-jogoh-child-dual-witness-nested-statement-vocabulary/README.md`

```markdown
# SIM-jogoh-child-dual-witness-nested-statement-vocabulary

This child breeds two parent strengths into one bounded design:

- from `SIM-pobod-grid-envelope-outer-promise-nested-signed-payload`: explicit outer-layer promise and nested signed payload as evidence;
- from `SIM-dalor-grid-envelope-protocol-owned-signature-slot`: a mandatory outer proof slot over canonical envelope bytes.

## Design under test

The specimen is a dual-witness envelope:

```text
[env_pCID, statement_capsule, outer_signature]
```

- `outer_signature` is current-sender evidence over canonical `[env_pCID, statement_capsule]` bytes;
- `statement_capsule` is itself a signed nested statement:

```text
[statement_pCID, kind, subject_ref, body, inner_signature]
```

The outer proof answers: "this sender carried these exact statement bytes under this envelope variant."
The inner proof answers: "this signer made this specific scoped statement."

## Why this should outperform the parents

This child keeps both parents' best layer discipline, but repairs three recurring weaknesses seen in the scenario sample:

1. **Better generic auditability than dalor**: the outer proof family is fixed by the envelope variant, so generic tools can understand the outer signature slot without first resolving each payload protocol.
2. **Better current-sender accountability than pobod**: forwarding or relay now leaves explicit outer evidence instead of relying only on the nested signature.
3. **Better local promise-accounting fit than both parents**: the nested statement capsule standardizes a tiny outcome vocabulary so peers can locally record `kept`, `refused`, `failed`, `corrupt`, `timed_out`, relationship-scoped adoption, onward restraint, freeze succession, and capability grant events without adding market-token semantics to the base envelope.

## Expected scenario behavior

- **Peer-local assessment only**: no token field is required; local peers can act on locally observed signed statements.
- **Kept storage promise / refused service**: `kind` distinguishes honest refusal from failure classes.
- **Onward-restraint chain**: each hop can carry a new outer sender proof around a signed `onward_restraint` statement.
- **Relationship-scoped adoption**: `subject_ref` and body can bind adoption to a relationship context rather than only a peer identity.
- **Post-freeze mutation**: `freeze_successor` records a new signed successor relation instead of mutating frozen history.
- **Permissioned capability token**: `capability_grant` is supported as a non-transferable signed grant.
- **Transferable promise token**: still deferred; forwarding the same capsule does not transfer obligation or redemption rights by itself.

## Boundaries

This simulation is still envelope-first and Promise-Theory-clean:

- it does **not** create global reputation;
- it does **not** make outer signatures mean storage success, payment, or authority delegation;
- it does **not** define a bearer-token economy;
- it only standardizes enough inner statement shape to keep local observations auditable and comparable.
```

### `proposals/ga-canary-20260524-dalor-pobod-forced/simulations/SIM-jogoh-child-dual-witness-nested-statement-vocabulary/QUESTION.md`

```markdown
# Question

Can PromiseGrid get a better envelope-layer tradeoff by combining a mandatory envelope-owned outer sender signature with a nested signed statement capsule that exposes a tiny auditable promise-accounting vocabulary (`kind` + `subject_ref`), so peers gain stronger generic auditability and clearer local kept/refused/adoption/freeze/capability records without requiring transferable tokens or collapsing higher-layer trust into the envelope?
```

### `proposals/ga-canary-20260524-dalor-pobod-forced/simulations/SIM-jogoh-child-dual-witness-nested-statement-vocabulary/protocols/grid-envelope.d/specs/grid-envelope-draft.md`

```markdown
# Grid-envelope draft: dual-witness nested statement variant

> **Status:** DRAFT. Simulation specimen only.
> Variant: `dual-witness-nested-statement-vocabulary`.

## Scope

This spec defines one grid-envelope candidate for comparison. It is designed to keep the outer envelope small and auditable while carrying a nested signed statement capsule.
...
- `env_pCID`: identifies this envelope variant;
- `statement_capsule`: opaque bytes unless the nested statement-capsule profile is supported;
- `outer_signature`: mandatory current-sender proof bytes.

## Outer promise semantics

The current sender's scoped promise is:

- these exact `statement_capsule` bytes are being carried under the envelope variant named by `env_pCID`;
- the sender is not claiming that the outer signature alone settles payload truth, redemption, storage success, or onward authorization;
- any higher-layer meaning remains inside the nested capsule and each receiver's local trust policy.

## Signable bytes

The outer signature covers canonical bytes of:

```text
[env_pCID, statement_capsule]
```

## Outer proof-family policy

Unlike protocol-owned outer proof slots, this variant fixes outer proof semantics at the envelope layer. Generic peers that know this envelope variant can interpret the third slot consistently across all nested statement kinds.

The outer proof therefore means only:

- current-sender carriage evidence;
- exact-byte binding for the carried nested capsule;
- no implied acceptance of the inner claim by later relays.

## Receiver behavior

If a receiver supports this envelope variant, it should:

1. verify `outer_signature` over canonical `[env_pCID, statement_capsule]`;
2. preserve exact bytes even if the nested capsule is unsupported;
3. avoid inferring storage success, capability redemption, transfer rights, or group authority from the outer layer alone.

If the receiver does not support this envelope variant, it may preserve the exact bytes as uninterpreted evidence.

## Why this variant exists

This child combines:

- pobod's explicit outer promise about nested structure and evidence;
- dalor's compact mandatory outer proof slot;
- a fixed outer proof meaning so generic audit is less dependent on resolving every inner protocol first.

## Non-goals

This envelope does not itself define:

- global reputation;
- bearer-token transfer;
- double-spend prevention;
- storage accounting policy;
- freeze lineage policy beyond carrying signed successor statements.
```

### `proposals/ga-canary-20260524-dalor-pobod-forced/simulations/SIM-jogoh-child-dual-witness-nested-statement-vocabulary/protocols/promise-statement-capsule.d/specs/promise-statement-capsule-draft.md`

```markdown
# Promise statement capsule draft

> **Status:** DRAFT. Simulation specimen only.
> Variant: `signed-statement-capsule-v1`.

## Scope

This nested capsule gives a minimal auditable statement shape for local promise-accounting records. It is intentionally small: enough to distinguish common observation and policy outcomes without turning the envelope into a full application protocol.
...
- `statement_pCID`: identifies the statement profile and body rules;
- `kind`: compact statement kind from the required vocabulary below;
- `subject_ref`: reference to the promise, relationship, object, lineage node, or capability being discussed;
- `body`: kind-specific structured bytes;
- `inner_signature`: signer evidence over canonical `[statement_pCID, kind, subject_ref, body]`.

## Required kind vocabulary

This variant requires support for these top-level `kind` values:
...
All statements are signed claims or observations, not universal truth. Receivers decide locally whether to trust the signer, store the record, act on it, or ignore it.

### Observation kinds

`kept`, `refused`, `failed`, `corrupt`, and `timed_out` are intentionally distinct. A peer can therefore record honest refusal differently from transport failure or corrupted delivery.

### Relationship-scoped adoption

`relationship_adoption` binds an adoption claim to a specific relationship context named by `subject_ref` or the signed body. Different peer relationships may legitimately carry different adoption statements without looking like equivocation by default.

### Onward restraint

`onward_restraint` records a signed conditional-forwarding promise. The body may name a parent statement or prior restraint reference so receivers can audit a hop-by-hop restraint chain locally.

### Freeze successor

`freeze_successor` records that a post-freeze change is a new signed successor relation. This variant does not allow mutation of frozen historical bytes; it carries a new statement that points back to the older frozen subject.

### Capability grant

`capability_grant` is non-transferable by default. Forwarding the same capsule does not transfer redemption rights or obligations. A new audience requires a new signed grant statement.

## Generic audit behavior

A peer that understands this capsule profile can store at least:

- exact capsule bytes;
- inner signer identity or verification result;
- `kind`;
- `subject_ref`.

That minimal record is enough to improve cross-scenario local audit without forcing all peers to understand every kind-specific `body` in full detail.

## Unknown-profile policy

If the capsule shape is recognized but `statement_pCID` is unsupported, a receiver may still preserve the exact bytes. It must not claim to understand the body semantics unless it supports that profile.

## Boundaries

This capsule does not define:

- exchange rates or marketplace semantics;
- bearer-token transfer;
- globally shared trust scores;
- mandatory centralized revocation services.

It exists only to make local signed observations and scoped promises more comparable across peers and scenarios.
```
