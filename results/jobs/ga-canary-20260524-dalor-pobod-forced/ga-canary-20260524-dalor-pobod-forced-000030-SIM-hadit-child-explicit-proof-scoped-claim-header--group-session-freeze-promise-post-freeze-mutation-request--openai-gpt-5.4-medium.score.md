...
- Cell ID: `ga-canary-20260524-dalor-pobod-forced-000030-SIM-hadit-child-explicit-proof-scoped-claim-header--group-session-freeze-promise-post-freeze-mutation-request--openai-gpt-5.4-medium`
- Simulation ID: `SIM-hadit-child-explicit-proof-scoped-claim-header`
- Scenario ID: `group-session-freeze-promise-post-freeze-mutation-request`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `proposals/ga-canary-20260524-dalor-pobod-forced/results/SIM-hadit-child-explicit-proof-scoped-claim-header/group-session-freeze-promise-post-freeze-mutation-request/openai-gpt-5.4-medium/20260524-190507.json`
...
### `proposals/ga-canary-20260524-dalor-pobod-forced/simulations/SIM-hadit-child-explicit-proof-scoped-claim-header/README.md`

```markdown
# SIM-hadit-child-explicit-proof-scoped-claim-header

This child combines two parent strengths:

- from `SIM-dalor-grid-envelope-protocol-owned-signature-slot`: a mandatory outer proof slot with deterministic signed bytes;
- from `SIM-pobod-grid-envelope-outer-promise-nested-signed-payload`: an explicit outer-layer Promise-Theory statement and a nested signed payload that keeps higher-layer meaning local.

## Design under test

The child makes two bounded changes intended to improve sampled-scenario fitness without collapsing layer boundaries:

1. **Explicit outer proof selector.** The outer envelope names a separate `sig_pCID`, so outer proof-family evolution no longer forces payload-protocol churn and generic auditors can know how to verify the outer signature without first understanding the payload protocol.
2. **Tiny generic claim header.** The outer signed bytes include a minimal `claim_header` with `kind`, `scope_ref`, and `supersedes`. This gives generic peers a small, auditable vocabulary for honest refusal, local observation, relationship scope, onward-restraint context, capability scope, and post-freeze lineage hints even when payload semantics remain protocol-local.

## Why this should score better

This should improve on both parents in the current scenario set by:

- raising **generic auditability** versus `dalor` through explicit outer proof naming;
- raising **scenario fit** versus both parents by giving peers a standard way to record refusal distinctly from silence or corruption, and to carry relationship/session/lineage context without inventing tokens;
- preserving strong **layer-boundary clarity** because the header is only an audit hint and does not replace payload-protocol semantics.

## Boundaries

This simulation still does **not** make the envelope into a full promise-accounting, capability, transfer, or geofencing protocol. The envelope only standardizes:

- outer proof verification discovery;
- a small signed header for cross-scenario audit hints;
- nested signed-payload carriage.

Payload protocols still own detailed storage, adoption, restraint, redemption, and transfer semantics.

## Files

Primary draft spec: `protocols/grid-envelope.d/specs/grid-envelope-draft.md`.
```

### `proposals/ga-canary-20260524-dalor-pobod-forced/simulations/SIM-hadit-child-explicit-proof-scoped-claim-header/QUESTION.md`

```markdown
# Question

Can PromiseGrid improve over both a protocol-owned outer signature slot and a plain nested signed-payload wrapper by using:

- an explicit outer `sig_pCID` for generic proof auditability, and
- a tiny signed `claim_header` for refusal, scope, and lineage hints,

while still keeping trust local and higher-layer promise semantics inside the payload protocol?
```

### `proposals/ga-canary-20260524-dalor-pobod-forced/simulations/SIM-hadit-child-explicit-proof-scoped-claim-header/protocols/grid-envelope.d/specs/grid-envelope-draft.md`

```markdown
# Grid-envelope draft: explicit outer proof with scoped claim header

> **Status: DRAFT.** Not frozen. This is a comparison specimen inside `SIM-hadit-child-explicit-proof-scoped-claim-header`.
> Variant: `explicit-proof-scoped-claim-header`.

## Scope

This draft defines a grid-envelope candidate that keeps the Promise-Theory-clean outer promise from the nested-signed-payload line while repairing two weaknesses seen in the parent evidence:

- generic outer auditability was weaker when proof rules were owned only by the payload `pCID`;
- several sampled scenarios wanted a small cross-protocol way to distinguish refusal, observation, scope, and lineage without forcing full application semantics into the envelope.

The design stays envelope-layer. It does not define storage policy, transfer ledgers, global reputation, or centralized authorization.

## Envelope shape

The outer envelope is a deterministic CBOR positional array:

```text
[env_pCID, sig_pCID, claim_header, nested_signed_payload, outer_signature]
```

Where:

- `env_pCID` names this outer envelope specification;
- `sig_pCID` names the proof profile for `outer_signature`;
- `claim_header` is a tiny signed audit-hint structure defined below;
- `nested_signed_payload` is a deterministic inner array;
- `outer_signature` is mandatory proof bytes over the canonical unsigned outer prefix.

The nested signed payload is:

```text
[payload_pCID, payload, inner_signature]
```

Where:

- `payload_pCID` names the payload protocol specification;
- `payload` is opaque bytes until interpreted by that protocol;
- `inner_signature` is mandatory evidence for the nested payload claim.

## Promise-Theory interpretation

The sender's outer signature is evidence only for the sender's own scoped promise:

- that the bytes are shaped as this envelope spec requires;
- that the included `claim_header` is the sender's stated classification and scope hint for the carried claim;
- that the nested bytes are the evidence object the sender chose to carry.

The outer signature is **not** universal truth, transfer authority, settlement finality, or proof that the payload claim succeeded in the world.

The inner signature is evidence for the nested payload claim under the payload protocol. Receivers still decide locally whether they trust the sender, the inner signer, the payload protocol, the claim kind, the scope reference, and any application consequence.

## Signable bytes

The outer signature signs the canonical bytes of:

```text
[env_pCID, sig_pCID, claim_header, nested_signed_payload]
```

The inner signature signs the canonical bytes of:

```text
[payload_pCID, payload]
```

This gives generic tools a fixed structural audit baseline even when payload meaning remains protocol-local.

## Outer proof policy

Unlike the `dalor` parent, outer proof-family selection is not hidden behind the payload `pCID`.

`sig_pCID` defines:

- proof family and verification algorithm for `outer_signature`;
- signer binding rules for the outer sender;
- threshold, multisig, delegation, freshness, and revocation semantics for the outer proof, if any.

This decouples outer-proof evolution from payload-protocol evolution.

## Claim header

`claim_header` is a deterministic CBOR array:

```text
[kind, scope_ref, supersedes]
```

Fields:

- `kind`: required small string or enum code.
- `scope_ref`: optional opaque reference or `null`.
- `supersedes`: optional opaque reference or `null`.

### Header semantics

`kind` is a small generic audit hint. Initial values for this specimen are:
...
The header is intentionally small. It does not replace payload semantics.

`scope_ref` is an opaque locally auditable pointer to the relationship, session, audience, chain, chunk-family, or other context that scopes the claim. The envelope does not require a single global format for that reference.

`supersedes` is an opaque reference to an earlier specimen, claim, or frozen lineage item when the sender wants to state amendment or supersession intent.

### Header validity guidance

- `amendment` SHOULD carry a non-null `supersedes`.
- `adoption`, `capability`, and `restraint` SHOULD carry a non-null `scope_ref`.
- `refusal` MAY carry `scope_ref` when refusal is relationship-, policy-, or session-scoped.
- `observation` MAY carry `scope_ref` when the observation is tied to a particular peer relationship, chunk family, or group context.

These are auditability rules, not universal truth guarantees.

## Why the header exists

The sampled scenario pressure repeatedly asked for small, local, auditable distinctions that neither parent standardized:

- honest refusal versus timeout or corruption;
- relationship-scoped adoption versus peer-global identity claims;
- onward-restraint context without forcing the whole recursive graph into the envelope;
- post-freeze lineage hints without mutating history;
- capability scoping without implying exchange rates or transferability.

This header gives generic peers a minimal shared vocabulary for those distinctions while leaving actual semantics to the payload protocol.

## Failure handling

The envelope distinguishes several cases that were blurred in the parent evidence.

### 1. No message received

No envelope means no envelope-level evidence. Absence is not a signed refusal.

### 2. Outer proof invalid

If `outer_signature` fails under `sig_pCID`, the envelope is structurally present but outer evidence failed. A receiver MUST NOT reinterpret that as a valid refusal, observation, or amendment.

### 3. Outer proof valid, inner payload unknown

If the receiver knows `env_pCID` and `sig_pCID` but not `payload_pCID`, it MAY still:

- verify the outer signature;
- record `kind`, `scope_ref`, and `supersedes` as sender-stated audit hints;
- preserve the exact bytes for later analysis.

It MUST NOT claim to understand the detailed payload semantics.

### 4. Honest refusal evidence

A valid outer envelope with `kind = refusal` is auditable evidence that the sender issued a refusal-shaped claim. It is still local evidence, not automatic proof that the refusal was justified.

This directly supports local accounting that treats refusal differently from timeout, corruption, or missing response.

### 5. Inner verification failure

If the outer envelope verifies but the inner signature fails or the payload protocol rejects the nested claim, receivers can record that as payload-level failure rather than envelope absence. The envelope does not force that to be read as misbehavior or as a refusal.

## Unknown-ID policy

- Unknown `env_pCID`: preserve bytes if desired, but do not claim envelope parsing.
- Known `env_pCID`, unknown `sig_pCID`: preserve bytes, but do not claim outer verification.
- Known outer layer, unknown `payload_pCID`: outer verification and header recording remain possible; payload meaning does not.

This is meant to improve audit utility for generic peers without pretending they know more than they do.

## Scenario notes

### Peer-local assessment only

The base envelope still requires no token field. A peer can use local observations and locally retained signed envelopes. `kind = observation` is enough to carry peer-local evidence without introducing a transferable unit.

### Kept storage promise

A payload protocol can carry storage evidence while the header marks the claim as `observation` and scopes it to a chunk or relationship. Bob can record the signed observation locally even if different storage protocols exist underneath.

### Refused service

`kind = refusal` gives a generic envelope-level distinction between an explicit signed refusal and mere non-response. Detailed reasons remain in the payload.

### Onward-restraint chain

`kind = restraint` plus `scope_ref` can point to the relevant release chain or session context. The full recursive graph still belongs to the payload protocol.

### Relationship-scoped adoption

`kind = adoption` plus `scope_ref` lets a peer signal that the claim is relationship-scoped rather than peer-global. This reduces accidental equivocation readings.

### Post-freeze mutation request

`kind = amendment` plus `supersedes` gives a minimal lineage hint. The envelope still preserves immutable old bytes and does not mutate frozen history.

### Permissioned capability token

`kind = capability` plus `scope_ref` can identify the intended audience or family. The envelope does not imply transferability.

### Transferable promise token

This specimen deliberately does **not** make transferability native. Any transfer or delegation semantics must be expressed by a payload protocol and locally accepted by the receiver. This keeps the base envelope from overcommitting to marketplace behavior.

## Comparison against the parents

### Better than `SIM-dalor-grid-envelope-protocol-owned-signature-slot`

- outer proof rules are explicitly named by `sig_pCID` rather than hidden behind `payload_pCID`;
- generic tools can verify outer proofs without first understanding payload semantics;
- a small signed header improves cross-protocol audit legibility.

### Better than `SIM-pobod-grid-envelope-outer-promise-nested-signed-payload`

- the outer proof profile is explicit rather than implicit in the outer protocol story;
- the envelope now carries a minimal generic claim vocabulary for refusal, scope, and lineage;
- sampled promise-accounting scenarios get a direct, auditable hook without abandoning nested signed payloads.

## Costs and risks

This specimen is slightly larger and less minimal than either parent. Risks include:

- implementers may overread `kind` as if it settled full application semantics;
- too many `kind` values would bloat the envelope, so the vocabulary should stay small;
- some ecosystems may still want richer payload-level accounting records than this header can provide.

## Non-goals

This draft does not standardize:

- storage receipts in full detail;
- token exchange rates or market semantics;
- global anti-double-spend state;
- universal identity or authority systems;
- a complete onward-restraint graph language.

## Open questions

- Is the three-field `claim_header` enough to improve generic auditability without becoming a second payload protocol?
- Should `scope_ref` stay fully opaque, or should later work define one or two common reference shapes for relationship and session scope?
- Is the added outer `sig_pCID` worth the extra byte cost for constrained deployments that rarely change proof families?

## Non-canonical status

This is a comparison specimen only. It is intended to test whether explicit outer proof selection plus a tiny signed claim header outperforms both parent approaches on auditability and scenario fit while preserving PromiseGrid's local-trust discipline.
```
