...
- Cell ID: `ga-canary-20260523-extracted-34x7-000174-SIM-zakil-spec-sections-guide-only--group-session-freeze-promise-post-freeze-mutation-request--openai-gpt-5.4-medium`
- Simulation ID: `SIM-zakil-spec-sections-guide-only`
- Scenario ID: `group-session-freeze-promise-post-freeze-mutation-request`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `results/SIM-zakil-spec-sections-guide-only/group-session-freeze-promise-post-freeze-mutation-request/openai-gpt-5.4-medium/20260523-230210.json`
...
### `simulations/SIM-zakil-spec-sections-guide-only/README.md`

```markdown
# SIM-zakil-spec-sections-guide-only

This simulation turns the guide-only prose alternative from
`SIM-ranib-spec-requirement-sections` into a concrete candidate specimen. It
tests whether the development guide should own explanatory prose while protocol
specs stay minimal and cite the guide. Source: `DI-fibuv`.

## Design Under Test

Protocol specs remain small normative documents. Promise vocabulary, 100-year
pressure, and layperson mental models live in guide prose, with specs carrying
only citations or short hooks.

## Boundaries

This simulation does not rewrite the development guide. It tests whether moving
explanation out of specs improves durability or separates implementers from the
exact promises they need to follow.
```

### `simulations/SIM-zakil-spec-sections-guide-only/QUESTION.md`

```markdown
# Question

Can PromiseGrid keep protocol specs minimal by putting explanatory sections in
the development guide, or does that make each pCID's promised meaning too
dependent on external prose?
