...
- Cell ID: `ga-canary-20260523-extracted-34x7-000251-SIM-dafod-child-dual-surface-contract-witness-ledger--group-session-freeze-promise-post-freeze-mutation-request--openai-gpt-5.4-medium`
- Simulation ID: `SIM-dafod-child-dual-surface-contract-witness-ledger`
- Scenario ID: `group-session-freeze-promise-post-freeze-mutation-request`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `proposals/ga-canary-20260523-extracted-34x7/results/SIM-dafod-child-dual-surface-contract-witness-ledger/group-session-freeze-promise-post-freeze-mutation-request/openai-gpt-5.4-medium/20260523-230210.json`
...
### `proposals/ga-canary-20260523-extracted-34x7/simulations/SIM-dafod-child-dual-surface-contract-witness-ledger/README.md`

```markdown
# SIM-dafod-child-dual-surface-contract-witness-ledger

This simulation breeds together the main strength of `SIM-tuhas-group-session-two-surface-freeze-gate` (clear separation between outer/feed and session-semantic freeze surfaces) and the main strength of `SIM-jaboj-udp-feed-reference-first-conformance` (concrete, replayable implementation evidence), while repairing their main weaknesses.

## Design Under Test

The specimen defines four artifacts:

1. **Frozen Outer/Feed Contract (OFC)**
   - Normative, human-readable.
   - Promises what the transport/feed layer must do without session leakage.
   - Includes explicit outcomes for:
     - unknown typed objects: store/advertise/forward opaquely or refuse parsing safely
     - replay/condition failure: reject with recorded condition mismatch evidence
     - per-hop authorization failure: deny forward and record refusal evidence
   - Exposes a minimal next-layer contract for session composition.

2. **Frozen Session-Semantic Contract (SSC)**
   - Normative, human-readable.
   - Covers group/session semantics, freeze scope, and post-freeze mutation lineage.
   - Keeps answer keys and semantic vocabulary spec-local rather than globally implied.

3. **Executable Witness Suite**
   - A minimal implementation and traces may exist, but they are explicitly **non-normative**.
   - Their role is to demonstrate the contracts, not define them.
   - Conformance checks compare witness behavior to OFC/SSC promises.

4. **Supersession and Mapping Manifest**
   - Any post-freeze change mints a new OFC CID, SSC CID, or both.
   - The manifest states which surface changed, which stayed frozen, and any vocabulary mapping for renamed or removed spec-local fields.
   - No mutation rewrites prior freeze evidence.

## Why this should score higher

This design keeps the parent boundary clarity, but adds direct failure handling and durable audit artifacts where both parents were weak.

- Better than `SIM-tuhas...` on concrete behavior because unknown-type, replay, and authorization-deny outcomes are now explicit.
- Better than `SIM-jaboj...` on evolution safety because code is downgraded from authority to witness.
- Better on 100-year review because the durable promise lives in frozen text and manifests, not only in implementation behavior.
- Better on session-layer composition because the outer/feed contract now states the minimal next-layer interface directly.

## Expected Scenario Behavior

- **Unknown typed object**: peer may preserve and forward opaquely under OFC without claiming parse understanding; refusal to parse is logged.
- **Replay outside conditions**: OFC binds rejection evidence to the checked condition set and epoch.
- **Per-hop authorization failure**: OFC records deny-forward locally; ring recovery policy may remain separate, but failure is no longer silent.
- **Vocabulary drift**: SSC uses spec-local names; mapping manifests carry renames/removals across supersession.
- **100-year review**: future readers inspect OFC, SSC, and manifests without needing Go tooling as authority.
- **Post-freeze mutation**: change creates a new surface artifact and explicit lineage link, never in-place mutation.
- **Session-layer composition**: session code depends on OFC contract fields, not witness quirks.

## Boundaries

This simulation does not choose final UDP-feed v0 wire details, final group-session semantics, or full ring reconfiguration policy after authorization failure. It only tests whether contract-first dual-surface freezes, plus non-normative witness evidence and standardized refusal/mapping records, produce a more auditable and evolution-safe specimen.
```

### `proposals/ga-canary-20260523-extracted-34x7/simulations/SIM-dafod-child-dual-surface-contract-witness-ledger/QUESTION.md`

```markdown
# Question

Should PromiseGrid freeze a normative outer/feed contract and a normative session-semantic contract separately, while treating executable implementations as non-normative witnesses and requiring peer-local refusal plus supersession-mapping records?

Source blend: `SIM-tuhas-group-session-two-surface-freeze-gate` and `SIM-jaboj-udp-feed-reference-first-conformance`.
```
