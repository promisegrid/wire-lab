...
- Cell ID: `ga-canary-20260523-extracted-34x7-000244-SIM-mipag-child-split-release-capsule-template--group-session-freeze-promise-post-freeze-mutation-request--openai-gpt-5.4-medium`
- Simulation ID: `SIM-mipag-child-split-release-capsule-template`
- Scenario ID: `group-session-freeze-promise-post-freeze-mutation-request`
- Model ID: `openai-gpt-5.4-medium`
- Result path: `proposals/ga-canary-20260523-extracted-34x7/results/SIM-mipag-child-split-release-capsule-template/group-session-freeze-promise-post-freeze-mutation-request/openai-gpt-5.4-medium/20260523-230210.json`
...
### `proposals/ga-canary-20260523-extracted-34x7/simulations/SIM-mipag-child-split-release-capsule-template/README.md`

```markdown
# SIM-mipag-child-split-release-capsule-template

This simulation breeds `SIM-sutap-spec-sections-split-template` with `SIM-gibut-conditional-release-group-session-local`.

It tests a concrete hybrid: conditional release is no longer owned mainly by mutable group/session semantics. Instead, the session carries or references a frozen **Release Capsule** bound to the content CID. The capsule's spec uses a split template: a compact pCID-owned normative surface plus separable companion prose.

## Design under test

- Conditional-release promises live in a frozen Release Capsule artifact, not only in session-local state.
- Group/session behavior is narrowed to carrying references, checking locally known conditions, and recording outcomes.
- The pCID-owned spec must include compact required sections for:
  - promise surface
...
This keeps the split-template parent's auditability and 100-year readability, while repairing the session-local parent's biggest weakness: policy, replay, and audit meaning were too entangled with mutable session context.

It also gives a direct answer to unknown-type pressure: peers may store, advertise, and forward payload bytes opaquely without unsafe parsing, because release evaluation targets the capsule and local context, not the payload codec itself.

## Boundaries

This simulation does not solve trusted geolocation proof, global identity, or a universal policy language. It only tests the minimum artifact and section structure needed to keep conditional release durable, auditable, and layer-bounded.
```

### `proposals/ga-canary-20260523-extracted-34x7/simulations/SIM-mipag-child-split-release-capsule-template/QUESTION.md`

```markdown
# Question

Can PromiseGrid improve conditional release by moving it from mostly session-local semantics into a frozen, payload-bound Release Capsule with a split-template spec that normatively defines unknown handling, replay/refusal evidence, lineage, and composition boundaries?
```

### `proposals/ga-canary-20260523-extracted-34x7/simulations/SIM-mipag-child-split-release-capsule-template/COMPANION.md`

```markdown
# Companion mental model

The session no longer owns the full meaning of conditional release. It only carries a frozen reference and makes local decisions.

## Example flow

1. Alice publishes `payload_cid` with `release_capsule_cid`.
2. Bob receives the tuple over any transport.
3. If Bob knows the capsule profile, he evaluates local conditions.
4. If Bob does not know the payload codec, he may still store or forward opaquely.
5. If Bob does not know the capsule codec, he may preserve the tuple but cannot newly authorize release.
6. If Bob refuses onward delivery to Carol, he records a refusal record rather than silently collapsing policy into transport loss.

## Parent strengths preserved

- From the split-template parent: compact normative hooks and evolvable companion prose.
- From the session-local parent: explicit participant promises and local onward-restraint decisions.

## Parent weaknesses repaired

- Replay and authorization are no longer trapped inside mutable session memory.
- Unknown typed payloads no longer pressure peers toward unsafe parsing.
- Post-freeze changes become new frozen profiles instead of ambiguous amendments.
- Auditors get a stable artifact boundary: payload, capsule, and local refusal/decision records.
```

### `proposals/ga-canary-20260523-extracted-34x7/simulations/SIM-mipag-child-split-release-capsule-template/SPEC.md`

```markdown
# Normative hooks for the Release Capsule design

## 1. Promise surface

A releasable item is the tuple:

- `payload_cid`
- `release_capsule_cid`
- optional `session_binding`

Any authorization or restraint claim refers to that exact tuple, not to mutable prose or unstated session memory.

## 2. Unknown and unsupported handling

### Unknown payload codec

If a peer does not implement the payload codec, it:

- may store the payload opaquely
- may advertise the payload CID opaquely
- may forward the payload opaquely when a valid release decision already exists
- must not parse the payload semantically
- must preserve the referenced `release_capsule_cid`

### Unknown release-capsule codec

If a peer cannot interpret the capsule codec, it:

- may store and advertise the tuple opaquely
- must not make a new authorization decision from that capsule
- must record `condition-unknown` if release evaluation was needed but impossible

This separates safe forwarding from unsafe policy invention.

## 3. Replay and refusal evidence

A local release decision is keyed by:

- `payload_cid`
- `release_capsule_cid`
- intended recipient or audience handle
- epoch or freshness handle
- optional geography-evidence class
- optional `session_binding`

If a peer denies or refuses onward delivery, it records a local refusal record with one reason code:
...
`transport-failure` is distinct from policy denial.

## 4. Vocabulary and lineage

All capsule fields and refusal reason codes are spec-local to the capsule profile pCID.

If a frozen profile renames or removes a field, the change requires a new pCID plus either:

- an explicit alias/migration map, or
- an explicit non-equivalence statement

Breaking post-freeze mutation never rewrites the old frozen profile.

## 5. Composition boundary

Transport and session layers are not required to inspect payload semantics.

A minimal next-layer API is:

- bytes or CID for payload
- `release_capsule_cid`
- optional `session_binding`
- local decision outcome or refusal record

This keeps UDP-feed-like transport usable without leaking policy internals into the transport contract.

## 6. Long-horizon assumptions

The normative surface must name, in compact form:

- what counts as audience identity
- what counts as epoch/freshness
- what geography evidence class is assumed, if any
- what happens when those assumptions are unavailable

Companion prose may explain these assumptions, but the pCID-owned hooks must still let a future reviewer reconstruct the decision surface.
```
