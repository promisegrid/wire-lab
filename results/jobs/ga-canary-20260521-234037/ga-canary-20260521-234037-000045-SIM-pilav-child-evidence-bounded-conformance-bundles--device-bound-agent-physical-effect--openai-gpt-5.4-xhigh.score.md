...
- Cell ID: `ga-canary-20260521-234037-000045-SIM-pilav-child-evidence-bounded-conformance-bundles--device-bound-agent-physical-effect--openai-gpt-5.4-xhigh`
- Simulation ID: `SIM-pilav-child-evidence-bounded-conformance-bundles`
- Scenario ID: `device-bound-agent-physical-effect`
- Model ID: `openai-gpt-5.4-xhigh`
- Result path: `proposals/ga-canary-20260521-234037/results/SIM-pilav-child-evidence-bounded-conformance-bundles/device-bound-agent-physical-effect/openai-gpt-5.4-xhigh/20260521-234037.json`
...
### `proposals/ga-canary-20260521-234037/simulations/SIM-pilav-child-evidence-bounded-conformance-bundles/README.md`

```markdown
# SIM-pilav-child: Evidence-bounded conformance bundles

This simulation combines the parent strengths on honest provisional semantics and porting-boundary hygiene, then adds a concrete auditable claim pattern. The design hypothesis is that PromiseGrid should teach early apps and early ports to publish an evidence-bounded conformance bundle instead of loose prose or a harness-shaped target.

## Question

How should a provisional PromiseGrid app or first port make an honest, locally auditable claim that separates protocol-boundary behavior from host/runtime details while preserving migration room before `DR-tuhaz` and `DR-davod` close? Source: `DI-ragaz`.

## Design deltas

1. **Two-surface claim bundle**
   - **Boundary contract claim:** names the protocol-facing identity, cited draft or frozen artifacts, claimed features, and excluded features.
   - **Embodiment claim:** names host/runtime limits, local carriage choices, storage or key assumptions, device or driver dependencies, and other non-protocol constraints.
2. **Minimum evidence packet**
   - Each claim carries a small feature matrix, a dependency ledger, and a local observation or failure record set that Bob and Carol can inspect without hidden authority.
3. **Negative-promise accounting**
   - Unclaimed properties must be stated plainly: a hash is an address unless a separate read or retention promise says otherwise; a live channel is outside durable audit conformance unless separately claimed; a device effect or vendor SDK dependency is an embodiment fact, not a default kernel obligation.

## Suggested bundle contents

- Claim subject and embodiment identifier
- Protocol-boundary identity used for interoperability
- Draft and frozen artifacts cited by the claim
- Feature matrix with statuses such as `implemented`, `provisional`, `blocked`, `external-dependency`, or `out-of-scope`
- Dependency ledger for runtime, storage, crypto, transport, driver, or SDK assumptions
- Promise ledger naming promiser, scope, and non-promises
- Local observation and failure records that survive denial, replay, restart, stale availability, or host replacement

## Decision axes

- Authoritative protocol-boundary identity versus local handles
- Implemented, provisional, blocked, external, and out-of-scope status wording
- Minimum locally auditable artifacts for partial-conformance and first-port claims
- Separation of app contract, runtime or port surface, and embodiment-specific dependencies
- How failures, denials, stale availability, replay, and host replacement are recorded without implying a final witness format

## Why this may score better

- Gives the app-semantics scenario a concrete honest-claim template.
- Gives the kernel-porting scenario a first-port target that is not the wire-lab harness.
- Improves blob, live, device, multi-embodiment, and signing-key scenarios by forcing explicit promiser, scope, exclusions, dependencies, and failure records.
...
This simulation does not settle the final app contract, the final runtime term of art, a capability-token standard, a witness format, or a live transport binding. It only proposes a safer provisional claim discipline that is standalone, locally auditable, and migration-friendly. Source: `DI-ragaz`.
```

### `proposals/ga-canary-20260521-234037/simulations/SIM-pilav-child-evidence-bounded-conformance-bundles/QUESTION.md`

```markdown
# Question

What is the smallest evidence-bounded conformance bundle that lets Alice publish an honest partial-conformance or first-port claim, lets Bob verify it from protocol-visible artifacts plus local observations, and lets Carol audit it later after host, driver, or storage replacement? Source: `DI-ragaz`.

Open decision points:

- Which fields are mandatory in the bundle header, feature matrix, dependency ledger, and failure log?
- Which statuses are safe now: `implemented`, `provisional`, `blocked`, `external-dependency`, `out-of-scope`?
- How should the bundle express non-promises such as no durability claim, no read-capability claim, no at-most-once claim, or no frozen signature-carriage claim?
- What shared protocol-boundary identity ties multiple embodiments to one app while keeping each embodiment's limits explicit?
- What minimum frozen artifacts define a first port, and what draft references may still be cited as orientation rather than obligation?
- What local records should exist when a blob is unavailable, a live channel partitions, a device replay is denied, or a key rotates across embodiments?
```
