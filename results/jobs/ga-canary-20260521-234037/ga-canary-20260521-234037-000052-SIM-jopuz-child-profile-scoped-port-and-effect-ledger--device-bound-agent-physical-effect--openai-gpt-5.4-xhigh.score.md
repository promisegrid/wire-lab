...
- Cell ID: `ga-canary-20260521-234037-000052-SIM-jopuz-child-profile-scoped-port-and-effect-ledger--device-bound-agent-physical-effect--openai-gpt-5.4-xhigh`
- Simulation ID: `SIM-jopuz-child-profile-scoped-port-and-effect-ledger`
- Scenario ID: `device-bound-agent-physical-effect`
- Model ID: `openai-gpt-5.4-xhigh`
- Result path: `proposals/ga-canary-20260521-234037/results/SIM-jopuz-child-profile-scoped-port-and-effect-ledger/device-bound-agent-physical-effect/openai-gpt-5.4-xhigh/20260521-234037.json`
...
### `proposals/ga-canary-20260521-234037/simulations/SIM-jopuz-child-profile-scoped-port-and-effect-ledger/README.md`

```markdown
# SIM-jopuz: Profile-scoped port and effect ledger

This simulation breeds `SIM-zisan-device-bound-agent` and `SIM-funas-kernel-porting-boundary` into a more concrete teaching pattern: a first real PromiseGrid port claims only a small core runtime surface, while app-specific behaviors such as device control live in explicit capability profiles with their own evidence rules.

## Question

Can the guide safely teach a first port as **core runtime + profile-scoped conformance**, with a device-bound agent profile that carries its own delegation, deduplication, receipt, and break-witness evidence?

## Design Move

1. **Core runtime surface, not harness clone:** a port claims only the frozen binding/session/message/CAS behavior it actually implements.
2. **Profile claim card:** each runtime, embodiment, or app profile publishes a compact auditable claim card naming boundary identity, implemented obligations, blocked items, host dependencies, and evidence artifacts.
3. **Device effect ledger:** irreversible physical effects use a crash-safe local ledger at the profile layer: delegation -> intent -> outcome receipt or break-witness.

## Why this should score higher

- Preserves both parents' boundary discipline and provisional safety.
- Repairs both parents' lack of a concrete auditable claim template.
- Repairs the device parent's lack of a crash-safe record flow for replay-after-restart.
- Repairs the porting parent's lack of a sharper "what is the first port target" answer.
- Helps adjacent scenarios by separating content identity, availability, authorization, and dependency claims instead of letting them blur together.

## Decision Axes

- **Core vs profile obligations:** what every port must do versus what only a device profile may claim.
- **Authoritative boundary identity:** which signer is speaking for the runtime, agent, owner delegation, or embodiment.
- **Local durable evidence:** which records must survive restart, driver loss, and later audit.
- **Dependency disclosure:** how CUPS, libusb, IIO, browser storage, helpers, or vendor SDKs are named without being mistaken for PromiseGrid core conformance.
...
- `docs/core-runtime-surface.md`
- `docs/profile-claim-card.md`
- `docs/device-effect-ledger.md`

## Boundaries

This simulation does not freeze the final runtime term, final device protocol, or final witness schema. It proposes a guide-safe structure for provisional teaching and audit:

- the core port target is smaller than a full app;
- device replay safety is profile evidence, not kernel magic;
- possession of a hash is only an address unless a separate claim card says more.
```

### `proposals/ga-canary-20260521-234037/simulations/SIM-jopuz-child-profile-scoped-port-and-effect-ledger/QUESTION.md`

```markdown
# Question

Can PromiseGrid guidance improve safety and auditability by teaching three layers explicitly:

1. a minimal first-port core runtime surface;
2. a reusable profile claim card for honest partial conformance;
3. a device-effect ledger for replay-sensitive physical actions?

Open decision points:

- What minimum frozen binding/session/message/CAS bundle is enough for a first real port claim?
- Which claim-card fields are mandatory for Bob and Carol to verify partial conformance from local evidence alone?
- What exact intent, outcome, and break-witness records are sufficient to prevent unsafe replay after restart?
- Which signer is authoritative at each boundary: runtime porter, agent, owner, or delegated operator?
```

### `proposals/ga-canary-20260521-234037/simulations/SIM-jopuz-child-profile-scoped-port-and-effect-ledger/docs/core-runtime-surface.md`
...
A first real port is **not** a copy of the wire-lab harness and **not** a promise to implement every app profile.

## In scope for a core port claim

- Accept and emit the frozen protocol object types the port explicitly names.
- Sign and verify those objects for the boundary identities it explicitly names.
- Store and retrieve the content-addressed objects it explicitly promises locally.
- Publish a versioned conformance claim card and implementation changelog.
- Preserve enough local records for later audit of what was claimed and what was omitted.

## Out of scope unless claimed by a separate profile

- Device drivers and physical-effect semantics.
- Live CRDT transport guarantees.
- Adapter-local UX or local-only identifiers.
- Unfrozen K1-K5 details beyond provisional orientation.
- App-specific capability, witness, or deduplication semantics.

## Audit rule

Every core port claim should cite:

- exact frozen references it relies on;
- omitted layers and blocked behaviors;
- local storage and key-handling posture;
- dependency disclosures that are not part of core PromiseGrid conformance.
```

### `proposals/ga-canary-20260521-234037/simulations/SIM-jopuz-child-profile-scoped-port-and-effect-ledger/docs/device-effect-ledger.md`

```markdown
# Device effect ledger

This ledger is a profile-level evidence kit for non-idempotent physical actions.

## Required record chain

1. **Delegation record**
   - owner identity
   - agent/operator identity
   - device identity
   - allowed operations
...
2. **Effect intent**
   - stable `op_id`
   - request digest
   - target device/effect
   - deduplication scope
   - recorded before actuation

3. **Effect outcome**
   - `done`, `replayed`, `refused`, or `unknown`
   - local evidence refs
   - external device receipt if any

4. **Break-witness**
   - emitted when restart or inspection cannot prove whether the effect happened

## Restart rule

If a non-idempotent effect has an unresolved `op_id`, restart must not automatically re-actuate it. The agent must first inspect local evidence and then emit either an outcome record or a break-witness.

## Conformance note

The effect ledger belongs to the device profile, not to the core runtime surface. A porter may implement the core port without claiming any physical-effect semantics.
```

### `proposals/ga-canary-20260521-234037/simulations/SIM-jopuz-child-profile-scoped-port-and-effect-ledger/docs/profile-claim-card.md`

```markdown
# Profile claim card

A claim card is the minimum auditable record for honest partial conformance.

## Required fields

- **Claim subject:** runtime, embodiment, agent, or app profile identifier.
- **Boundary signer:** which key is authoritative for this claim.
- **Contract/profile reference:** the pCID, draft path, or named provisional profile being claimed.
- **Implemented obligations:** what is actually supported now.
- **Blocked or deferred obligations:** what is intentionally not claimed.
- **Dependency disclosures:** browser APIs, helpers, drivers, SDKs, OS services.
- **Evidence artifacts:** local logs, receipts, manifests, delegation records, changelog references.
- **Promise splits:** content identity, availability, authorization, discovery, replication, and retention must be stated separately when relevant.
- **Carriage status:** which wire/signature choices are frozen, provisional, or purely local.

## Honest defaults

- A hash is an address unless a separate claim states an availability or read-capability promise.
- A host dependency is disclosed, not hidden inside protocol conformance.
- Omitted merge, capability-token, or break-witness behavior must be named as omitted.
- UX sameness does not by itself prove shared app identity.
```
