...
Snapshot generated by Codex on 2026-05-25 08:26 local time. This is a
...
`DI-pagin`; `DI-kafiz`; `DI-rusup`; `DI-funaf`.
...
| Kernel profile | The kernel remains provisional; `TE-mazop` and `SIM-fovip` now test app-facing promises, host assumptions, unsupported features, evidence records, and minimum credible port contracts before `DR-davod` closes. |

Source: `DI-sisak`; `DI-rusup`; `DI-funaf`.
...
| `FB-vitih`, `FB-mulum`, `FB-potin` | `simulations/SIM-funas-kernel-porting-boundary/`; `simulations/SIM-fovip-kernel-promise-boundary-port-contract/`; `docs/thought-experiments/TE-jimar-kernel-runtime-portability-boundary.md`; `docs/thought-experiments/TE-mazop-kernel-promise-boundary-and-minimum-port-contract.md` | `scenarios/kernel-porting-boundary/` | Provisional kernel/runtime/dispatcher and minimum-port-contract evidence; settled porting target remains blocked by `DR-davod`. |
...
| Kernel Devs | Provisional / blocked for final porting instructions | The porting target is not wire-lab. A porter should expect explicit app-facing promises, host/runtime assumptions, unsupported features, pCID coverage, evidence records, and profile declarations around the substrate/feed/CAS/session/message layers the porter promises to implement. Daemon, microkernel, host-runtime, browser/WASM, MCU/header-only, and split-object shapes remain profiles, not one universal process shape. | The minimum credible port contract and first required frozen spec set are not locked yet. `TE-mazop` and `SIM-fovip` are the current evidence path; `DR-davod` remains open. | Future frozen binding/session/message specs by pCID, implementation promise records, `SIM-fovip` evidence, and guide prose once stabilized. |
...
- Settled porting guidance is blocked by `DR-davod` until the minimum credible
  port contract, runtime expectations, evidence obligations, and required pCID
