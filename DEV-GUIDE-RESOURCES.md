...
Snapshot generated by Codex on 2026-05-25 15:20 local time. This is a
...
promises**, not a universal daemon, microkernel, RPC authority, or conformance
certificate. A PromiseGrid implementation promises only its own behavior to
local apps/operators: supported pCIDs, unsupported behavior, app-facing storage /
compute / send / receive / key / lifecycle / dispatch / evidence promises, host
assumptions, adapter mappings, voluntary namespace/reference behavior, and
make/break evidence. Local APIs may be ergonomic adapters, but the PromiseGrid
boundary remains pCID-selected `grid([42(pCID), payload, ...])` messages when an
operation crosses that boundary. Source: `DI-fidot`; `DI-ripuz`.
