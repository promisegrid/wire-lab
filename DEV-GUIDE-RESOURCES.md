...
Snapshot generated by Codex on 2026-06-08 06:53 PDT. This is a
...
`DI-fumol`; `DI-hohuf`; `DI-mosil`; `DI-lupag`; `DI-nisaz`.
...
bytes through an issuer-local capability token, Alice's immediate Bob retrieval
fails at the TCP dial layer, Frank issues his own replica token and later serves
the bytes as Alice's trusted-enough fallback, Carol executes payload-provided
Fibonacci code with explicit context from Ellen while also sending one bad
result that Alice/Dave/Grace reject by recomputation, Dave checkpoints the exact
compute tuple, Grace verifies compute and records Mallory's corrupt-byte,
unknown-pCID, and unsupported-variant evidence, and local trust/economics/repair
records remain local promise evidence rather than authority. POC13's analyzer
now reports a bounded score report for transport, storage, compute, economics,
trust, verification, and replica recovery in addition to raw event counts, and
it gates on capacity refusal, TCP send failure, bad-result rejection, evidence
pCID routing, and local non-commitment cases. POC13's live-provider evidence
must contain actual role-aligned local promise judgment text; placeholder
provider reachability records are analyzer failures, not autonomous promise
evidence. Source:
`DI-gopag`; `DI-bibom`; `DI-notig`; `DI-lasuh`; `DI-fumol`; `DI-hohuf`;
`DI-mosil`; `DI-lupag`; `DI-nisaz`.
...
- `implementations/poc13-cas-compute-functions/` is executable provisional
  ...
  on fixed startup/settle sleeps, stores and retrieves real CAS bytes by CID,
  replicates bytes to Frank, recovers from a TCP-level Bob retrieval failure by
  redeeming Frank's own replica token, executes payload-provided Fibonacci
  compute with explicit context, rejects one bad Carol result by Alice-local and
  Dave/Grace recomputation, routes verifier observations through
  `evidence_report_v1`, checkpoints the exact compute tuple, rejects corrupt CAS
  evidence, records unknown-pCID and unsupported-variant non-commitment, records
  local trust-driven peer choice, accepts/refuses bounded credit offers, records
  credits spent/earned and payment withholding, reserves/refuses bounded
