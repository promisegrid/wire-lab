...
Snapshot generated by Codex on 2026-06-08 16:47 PDT. This is a
...
- `implementations/poc13-cas-compute-functions/` is executable provisional
  ...
  compute promises, with `cas_storage_v1`, `cid_compute_v1`, and
  `evidence_report_v1` as protocol pCID names and content/function/context/result
  CIDs as payload-level values. It sends signed grid envelopes over
  length-framed TCP, records runtime readiness/done evidence instead of relying
  on fixed startup/settle sleeps, stores and retrieves multiple real CAS byte
  objects by CID, replicates bytes to Frank, recovers from named TCP-level Bob
  outage variants by redeeming Frank's own replica tokens, exercises token TTL,
  expiry, revocation, and renewal, executes payload-provided Fibonacci and sum
  compute with explicit context, rejects bad Carol results by Alice-local and
  Dave/Grace recomputation, routes verifier observations through
  `evidence_report_v1`, resolves verifier disagreement locally, checkpoints
  ...
  capacity, and issues/redeems issuer-local capability tokens. Its
  ...
  `DI-notig`; `DI-lasuh`; `DI-fumol`; `DI-hohuf`; `DI-mosil`; `DI-lupag`;
  `DI-nisaz`; `DI-kikoj`; `TODO-godad`.
