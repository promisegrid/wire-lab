...
Snapshot generated by Codex on 2026-06-09 06:58 PDT. This is a
...
`DI-sinur`; `DI-punib`; `DI-sunuf`; `DI-vahan`.
...
authority. POC13's analyzer now reports a bounded score report for transport,
storage, compute, economics, trust, verification, replica recovery, durability,
retention, pressure, and replay in addition to raw event counts, and it gates on
capacity refusal, TCP send failure, bad-result rejection, evidence pCID routing,
local non-commitment, token lifecycle, cache reuse, alternate compute, verifier
disagreement, persisted trust, bad-proof/key-rotation, run-scoped store,
retention/GC, rate-limit/backpressure, and replay cases.
...
- `implementations/poc13-cas-compute-functions/` is executable provisional
  ...
  `DI-punib`; `DI-sunuf`; `TODO-godad`; `TODO-pazif`; `TODO-juput`.
