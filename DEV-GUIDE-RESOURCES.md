...
Snapshot generated by Codex on 2026-06-19 20:38 PDT. This is a
...
`DI-nogij`; `DI-rigup`; `DI-vulit`; `DI-gazin`; `DI-bitug`.
...
| Protocol specs | Draft specs may live at `docs/protocols/<slug>.md`; POC16 executable specs now also have `docs/protocols/cidv1-raw-sha2-256:<hex>.md` symlinks to the slug files | Makes pCID provenance reviewable while preserving pCID-as-content identity and avoiding a central registry. |
...
- `implementations/poc15-multihop-multiarity-dag/` is executable provisional
  ...
  transport, or app API.
	...
	- `implementations/poc16-secure-tokens-maps-encrypted-payloads/` is executable
	  ...
	  encrypted-payload, pCID-owned map, embedded-spec, and parser/builder role
	  pressure. It preserves all POC15 named agents and analyzer categories, hashes
	  embedded markdown protocol specs into pCIDs, publishes matching
	  `docs/protocols/cidv1-raw-sha2-256:<hex>.md` symlinks, injects spec excerpts
	  into LLM prompt context, emits raw CBOR artifacts for map, secure capability,
	  encrypted payload, and parser/builder profile pCIDs, and gates CWT-style
	  COSE-signed bearer and holder-bound tokens, expiry/audience/replay/transfer
	  failures, AEAD wrong-recipient/tamper failures, visible and hidden parent
	  references, malformed parser rejection, pCID-only dispatch, and pCID-as-address
	  drift checks. It should be cited as POC evidence only, not as a final token,
	  encryption, key agreement, parser-module, or payload-map standard.
	  Source: `DI-vulit`; `TODO-zugok`; `TE-ritig`.
