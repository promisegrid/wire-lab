...
Snapshot generated by Codex on 2026-06-06 01:12 PDT. This is a
...
simulation evidence surface. POC13 is now planned as the next storage/compute
pressure point: decentralized sparse CAS storage promises under `cas_storage_v1`
and CID-named function-call compute promises under `cid_compute_v1`. A CID names
...
Pure compute results can be cached by exact code/input/context identity; impure
work must externalize timestamp, randomness, sensor reads, or other ambient
inputs as context objects so the run is replayable and pure-after-the-fact.
Source: `DI-gopag`; `DI-bibom`.
...
- `docs/research/DN-nuras-poc13-cas-compute-functions.md` defines planned POC13
...
- `protocols/wire-lab.d/TODO/TODO-godad-poc13-cas-compute-functions.md`
  tracks the planned POC13 storage/compute definition and later executable POC.
...
- `implementations/poc12-production-progress/` is executable provisional
  ...
  `DI-vutok`; `DI-zapab`.
- `implementations/poc13-cas-compute-functions/` is the planned next executable
  POC path for decentralized CAS storage promises and CID-named function-call
  compute promises. It should not be cited as executable evidence until it
  exists. Source: `DI-bibom`; `TODO-godad`.
