...
Snapshot generated by Codex on 2026-06-07 07:27 PDT. This is a
...
`DI-fumol`; `DI-hohuf`; `DI-mosil`; `DI-lupag`.
...
evidence: decentralized sparse CAS storage promises under `cas_storage_v1` and
CID-named function-call compute promises under `cid_compute_v1`. A CID names
content or function code inside protocol-owned payloads; it does not by itself
promise availability, retention, access, serving, or trustworthy computation.
...
bytes through an issuer-local capability token but does not currently promise
immediate serving, Frank replicates and later serves the bytes as Alice's
trusted-enough fallback, Carol executes payload-provided Fibonacci code with
explicit context from Ellen, Dave checkpoints and verifies the exact compute
tuple, Grace verifies compute and records Mallory's corrupt-byte evidence, and
local trust/economics/repair records remain local promise evidence rather than
authority. POC13's analyzer now reports a bounded score report for transport,
storage, compute, economics, trust, verification, and replica recovery in
addition to raw event counts. POC13's live-provider evidence must contain actual
...
- `implementations/poc13-cas-compute-functions/` is executable provisional
  ...
  It sends signed grid envelopes over length-framed TCP, records runtime
  readiness/done evidence instead of relying on fixed startup/settle sleeps,
  stores and retrieves real CAS bytes by CID, replicates bytes to Frank,
  recovers from Bob's current serving non-commitment by retrieving from Frank,
  executes payload-provided Fibonacci compute with explicit context, performs
  Alice-local and Dave/Grace peer compute verification, checkpoints the exact
  compute tuple, rejects corrupt CAS evidence, records local trust-driven peer
  choice, accepts/refuses bounded credit offers, records credits spent/earned,
  reserves bounded capacity, and issues/redeems issuer-local capability tokens.
  Its
