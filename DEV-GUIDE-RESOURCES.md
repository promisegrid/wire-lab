...
Snapshot refreshed by Codex on 2026-07-29 11:46 PDT. This is a
...
are also present. POC18 inter-agent flows now run in Docker containers over
length-framed TCP instead of simulated in-process CAS transfer: Bob retrieves
from Alice, Carol retrieves from Bob, Dave retrieves from Bob and records a
merge snapshot, Ellen retrieves from Dave and records a review statement, and
Frank retrieves from Alice and records paid retention. Each cross-agent object
...
signed transferable CWT/COSE storage/forwarding payment token in an
`object_availability` promise to Frank, Frank redeems that bearer token locally
...
accepted TE survivor is durable local CAS event streams with local projections,
indexes, caches, JSON files, SQLite tables, or in-memory summaries treated only
...
payloads. The implementation-local POC20 design entrypoint is
...
identity remain open under `DR-junaz` and `DR-lupiz`. `DN-gagog` records the
...
- `docs/research/DN-gagog-grid-language-profiles-and-runtime-descriptors.md`
  records the POC21 Grid language-family direction: one common data layer, a
  finite Gridfile journal profile, a Turing-complete `*.grid` program profile,
  and runtime descriptors that keep source meaning separate from a selected
  executable artifact. The canonical syntax and type/effect system remain open
  under `DR-junaz`. Source: `DI-rigob`; `DI-bigap`.
- `docs/thought-experiments/TE-fakof-grid-source-shebang-identity.md` compares
...
- `docs/research/DN-gagog-grid-language-profiles-and-runtime-descriptors.md`
  explains why stage0 should parse only an effect-free config subset while a
  fetched stage1 engine evaluates the full language, and why finite machine
  journals and Turing-complete programs remain distinct profiles of one family.
  Source: `DI-rigob`; `DI-bigap`.
- `docs/thought-experiments/TE-fakof-grid-source-shebang-identity.md` is the
...
- `implementations/poc18-cas-git-replacement/` now contains the first executable
  ...
  Docker agent containers that exchange promise-shaped `grid()` messages over
...
- `DR-lupiz` — decides whether `*.grid` source names a language-spec CID or
  pCID and how exact runtime selection is recorded separately or in-band.
- `DR-junaz` — decides the canonical Grid syntax, typing, effect model,
  evaluation rules, and content-addressed definition semantics.
