...
Snapshot generated by Codex on 2026-05-26 22:52 local time. This is a
...
`DI-punam`; `DI-sizoh`; `DI-sagos`; `DI-tugih`; `DI-fibok`.

### Current Wire Direction

| Item | Current rule | Why |
|---|---|---|
| Outer envelope | CBOR array `grid([42(pCID), ...protocol-defined-slots])`; recommended example profile `grid([42(pCID), payload, ...])` | Keeps the universal parse surface small while letting the pCID-named spec define the whole following slot vector. |
...
CBOR `grid([42(pCID), payload, proof])` app messages over length-framed TCP,
...
Mallory-to-Dave stale-token circulation, and local trust updates after a revoked
promise breaks. The visible message kinds are promise-first rather than
...
API, storage API, compute API, auditor authority, or permission system. Source:
`DI-tugih`; `DI-fibok`; `DI-tanat`; `DI-pabot`.
