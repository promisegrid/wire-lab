...
Snapshot generated by Codex on 2026-05-31 18:46 local time. This is a
...
`DI-pabot`; `DI-rodog`; `DI-hanih`; `DI-sirus`; `DI-vorus`.
...
POC9 is now planned as the next evidence step rather than settled design. Its
pressure is discovery and durable local strategy: agents should learn about
peers, offers, routes, and trials from existing relationships, introductions,
referrals, signed promise evidence, and low-risk probes. Promise relationships
should influence whether an agent opens, keeps, retries, or uses TCP to a peer;
TCP delivery, malformed bytes, disconnects, timeouts, retries, and latency then
become local transport-promise observations. An open TCP connection is not trust,
and a referral is evidence rather than transitive authority. Source: `DI-vorus`.
...
| Kernel profile | The kernel remains provisional; `DN-lujad`, `TODO-binag`, `TODO-hozaz`, `TODO-tapov`, `TODO-hipak`, the expanded `SIM-fovip`, executable `poc3`, executable `poc4`, executable `poc5`, executable `poc7`, and executable `poc8` evidence now synthesize `TE-mazop`, `TE-pudiv`, `TE-dunas`, and `TE-gakoh` into role/profile guidance for app-facing promises, same-grid app/kernel messaging, app-level relay promises, local trust decrease after broken promises, selective sending, app-level capability-token promises, signed CBOR-grid app messages over length-framed TCP evidence, promise-shaped reciprocal exchange vocabulary, autonomous need/offer/counter/acceptance payload variants, Mallory-to-Dave stale-token circulation, deterministic local economic decisions before app action, issuer-local revocation/redemption evidence, voluntary group namespaces, CID-rooted promise-bound references, file-like resources over promise/event logs, lower-patent-risk prior-art influence guardrails, host assumptions, unsupported features, kernel-local evidence records, app-local promise judgments, and minimum credible port-promise records before `DR-davod` closes. |
| Promise-economy autonomy | POC8 now demonstrates one-pCID payload variants for autonomous need advertisements, offer/counter/acceptance promises, token issue/redemption promises, outcome observations, collateral/stake promises, bearer-for-non-transferable exchange, peer-local exchange-rate quotes, and stale-token trust decay. POC9 is planned to test richer discovery, non-deterministic peer strategy, scoped trust thresholds, low-risk probe promises, TCP evidence feedback, dynamic route promises, durable long-horizon relationship learning, and larger-scale economics beyond a bounded POC. |
...
discovery, scoped trust, low-risk probes, dynamic route promises, TCP evidence,
and durable per-agent strategy after POC8. Source: `DI-vabij`; `DI-pidag`;
`DI-vorus`.
