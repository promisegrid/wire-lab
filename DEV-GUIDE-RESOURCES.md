...
- `implementations/poc11-adaptive-trust-tcp/` is executable provisional
  ...
  relationship decay/repair, and trust-correlated direct TCP link promises. It
  produced a successful first live run with strong autonomy, local trust, and
  imposition-avoidance evidence, but only medium protocol validity because live
  outputs still generated malformed targets, one invalid JSON response,
  repeated promises, late event ordering, and noisy shutdown evidence. It is not
  a stable LLM-agent, monitor, discovery, topology, adaptive transport, trust,
  provider, or economics API. Source: `DI-hotos`; `DI-horuh`; `DI-mosoj`.
