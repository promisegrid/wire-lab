...
Snapshot generated by Codex on 2026-05-21 18:56 local time. This is a
developer-facing protocol-design snapshot, not a frozen PromiseGrid API. It is
derived from all simulation trees, promoted GA specimens, and canonical root
`results/` JSON evidence; proposal children remain review evidence until
promoted. Source: `DI-volos`.

### Evidence Signal

| Area | Current signal | Design implication |
|---|---:|---|
| Guide-facing claim/evidence sims | `SIM-zifoj` 89.9, `SIM-tizad` 88.2, `SIM-virim` 84.0, `SIM-savak` 83.0 | Strong convergence on explicit claim bundles, durable citations, and peer-local records. |
| Promoted grid-envelope sims | `SIM-jufag` 83.7, `SIM-natim` 83.7, `SIM-bimos` 83.0, `SIM-maraz` 78.3 | Envelope convergence is real but less settled than claim/evidence convergence. |
| Baseline envelope variants | Most fixed matrix variants score about 42-64 | Plain hard-reject, blind opaque, unsigned, and unconstrained variable-arity choices are weaker. |
| Weak rubric axes | Failure handling, scenario fit, residual risk | Specs need explicit failure states, quarantine rules, and concrete scenario profiles. |

### Dominant Candidate: Fixed Outer Envelope + Nested Typed Payload
...
grid-envelope = [
  grid_pcid: cid,      ; pCID for this envelope profile and fixed arity
  payload_pcid: cid,   ; pCID for the nested payload schema
  payload: bstr,       ; exact canonical payload bytes
  proof_pcid: cid,     ; pCID for signature / attestation semantics
  proof: bstr          ; proof over grid_pcid, payload_pcid, payload, context
]
```
...
|---|---|
| `grid_pcid` unsupported | Preserve exact bytes as uninterpreted evidence if framing permits; do not parse payload. |
| `payload_pcid` unsupported | Quarantine semantic acceptance; retain and optionally relay exact bytes with local outcome note. |
| `proof_pcid` unsupported | Quarantine or reject semantic acceptance; retain bytes if policy permits. |
| Proof invalid | Reject acceptance; optionally keep exact bytes plus verifier/version/failure record. |
| All supported and valid | Accept payload under the payload pCID; app semantics still come from payload profile. |

Strengths:

- Gives the kernel a small positional parse surface.
- Keeps payload evolution inside pCID-defined nested schemas.
- Lets generic peers preserve evidence without pretending to understand it.
- Makes signature semantics explicit instead of hidden in transport context.

Weaknesses:

- Exact CID target is still unsettled: full envelope, nested payload, proof
  transcript, or multiple CIDs.
- Quarantine retention, relay, privacy, and DoS bounds are not frozen.
- Encoding is not final; DAG-CBOR scores well for exact-byte evidence, while
  CBOR remains attractive for simple early tooling.

### Candidate Payload Profiles for Guide Authors

#### App / Port Conformance Bundle

Derived mainly from `SIM-zifoj`, `SIM-tizad`, and `SIM-savak`.

```json
{
  "kind": "claim-bundle-v0",
  "app_manifest_cid": "cid:...",
  "cards": {
    "port_boundary": {
      "implemented_surface": ["cas.read", "cas.write", "session.receive"],
      "blocked_surface": ["live-transport-conformance"],
      "status": "provisional"
    },
    "app_semantics": {
      "contract_family_pcid": "cid:...",
      "claimed_operations": ["put_blob", "get_blob"],
      "non_promises": ["retention", "authorization", "global_availability"]
    },
    "host_embodiment_dependency": {
      "runtime": "browser | helper | daemon",
      "local_dependencies": ["driver", "filesystem", "webcrypto"]
    }
  },
  "evidence_refs": ["cid:..."],
  "promise_accounting_record_refs": ["cid:..."]
}
```

Strengths:

- Gives app and kernel guide writers concrete partial-conformance wording.
- Separates protocol claims from host, runtime, storage, and device facts.
- Prevents content identity from being mistaken for availability or authority.

Weaknesses:

- Mandatory card fields are not final.
- Needs machine-checkable status vocabulary and witness requirements.

#### Savepoint / Embodiment Audit Payload

Derived mainly from `SIM-virim`.

```json
{
  "kind": "savepoint-audit-v0",
  "app_manifest_cid": "cid:...",
  "embodiment_claim_refs": ["cid:..."],
  "identity_continuity_receipt_refs": ["cid:..."],
  "cited_object": {
    "kind": "snapshot | save_blob | effect_receipt | break_witness",
    "cid": "cid:..."
  },
  "live_transport_claim": "off-grid-not-promised",
  "promise_body_cid": "cid:..."
}
```

Strengths:

- Makes "one app across embodiments" a manifest/receipt fact, not a UX label.
- Keeps live collaboration separate from durable PromiseGrid publication.
- Gives Carol a concrete object to audit after save, crash, or key rotation.

Weaknesses:

- Needs a portable signing-key continuity object and browser/helper threat
  model.
- Needs one normalized savepoint object shape or a clearly profiled family.

#### Outer Attestation Proof Payload

Derived mainly from `SIM-natim`, with pressure from routing, chunk-feed, and
conditional-release scenarios.

```json
{
  "kind": "outer-attestation-v0",
  "role": "origin | relay | holder | forwarder",
  "signer": "did-or-key-ref:...",
  "context_refs": ["cid:policy", "cid:freshness", "cid:restraint"],
  "signed_transcript": {
    "grid_pcid": "cid:...",
    "payload_pcid": "cid:...",
    "payload_digest": "cid-or-hash:...",
    "context_digest": "cid-or-hash:..."
  },
  "proof": "multisig-or-signature-bytes"
}
```

Strengths:

- Distinguishes payload authorship from current relay/holder/origin promises.
- Carries routing, sparse-replication, and onward-restraint context without a
  central trust registry.

Weaknesses:

- More complex than a simple signature tuple.
- Role vocabulary, freshness semantics, and revocation are still profile-level
  work.
...
|---|---|---|---|
| Quarantine + explicit `sig_pcid` tuple | `SIM-jufag`, `SIM-bimos` | Strongest envelope scores; clean no-speculative-parse rule; exact-byte evidence. | Needs retention, relay, and privacy policy; less expressive about current actor role. |
| Outer attestation / Multisig | `SIM-natim` | Best at origin/relay/holder/forwarder distinction. | Higher implementation and key-management cost. |
| Signed summary header + nested schema | `SIM-maraz` | Lets generic peers audit summary refs while payload schema evolves. | Risk of summary/payload mismatch or generic over-trust of summaries. |
| pCID-defined variable arity | `SIM-sajar` | Maximum extension flexibility. | Low current score; generic peers cannot bound or inspect enough without profile knowledge. |
| Hard reject unknowns | `SIM-gazan`, `SIM-dorut`, `SIM-nipoh`, `SIM-tohol` | Simple and safe against accidental acceptance. | Discards or suppresses useful future evidence; weak mixed-version recovery. |
| Opaque accept/store-forward | `SIM-gasus`, `SIM-hagom`, `SIM-ruzil` | Preserves bytes and supports delayed interpretation. | Too easy to confuse carriage with semantic acceptance unless quarantine is explicit. |
| Unsigned v0 | `SIM-gazan`, `SIM-fonig`, `SIM-gasus`, `SIM-hagom` | Simple bootstrapping. | Weak provenance; poor fit for promises, routing, and effect/audit scenarios. |

### Open Protocol Decisions

| Decision | Why it matters |
|---|---|
| Full-envelope CID vs payload CID vs both | Determines what pointers, signatures, and audit records actually name. |
| Mandatory proof shape | Chooses between simple signature tuple, outer attestation, Multisig, or profile-specific proof. |
| Quarantine policy | Needs retention, relay, quota, privacy, and failure-record rules. |
| Encoding profile | CBOR is simpler; DAG-CBOR and strict canonical bytes look stronger for CAS/audit. |
| pCID / sig-pCID recovery | 100-year audit needs decentralized retrieval or archival of protocol specs. |
| First kernel port surface | Guide still needs the minimum frozen runtime/session/CAS obligations. |
| Claim-bundle mandatory fields | App guide needs comparable conformance claims across implementations. |
| Identity continuity object | Multi-embodiment apps need key rotation and browser/helper continuity without central identity. |
