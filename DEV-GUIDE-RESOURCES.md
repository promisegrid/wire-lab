...
Snapshot generated by Codex on 2026-05-24 14:04 local time. This is a
...
`DI-baral`; `DI-lunuv`; `DI-pagin`; `DI-rusup`; `DI-kafiz`.
...
| Peer-local promise accounting | `SIM-tupog`, `SIM-vuliv`, `SIM-konit`, `SIM-zifoj`, `SIM-tizad`, `SIM-savak`, `SIM-haros` | Promise observations are kept by peers for their own relationships; signed refusal, silence/timeout, and exact-byte observations belong in higher-layer pCID-owned records, not generic base-envelope claim headers. |
...
| Capability tokens as promises remain provisional | `SIM-lurip`, historical harness/TE evidence, and adjacent pressure in `SIM-haros`, `SIM-tizad`, `SIM-zifoj`, `SIM-savak` | A token is a promiser's promise, not a global permission object. Treat dedicated simulation coverage as active evidence, not settled current consensus. |
| Kernel as portable role/profile | `TE-jimar`; `SIM-funas`; `DR-davod`; `TE-sihih`; `TE-vipir`; `TE-jikaf` | Treat daemon, microkernel, host-runtime, browser/WASM, MCU/header-only, and split-object shapes as profiles around the same local role set, not as one universal process shape. |
...
- Allows pCID-defined payloads to evolve without changing the universal grid parse
  surface.
- Avoids selector-shopping stacks such as `env_pCID`/`sig_pCID`/`payload_pCID`
  at the base-envelope layer when higher-layer payload protocols can own their
  record shapes.
...
- `SIM-lurip` is now the focused simulation for this pressure, but the shape
  remains provisional until scored evidence and a later DR/DI settle it.

#### Promise-Accounting Observation or Refusal

Candidate shape for a pCID that means "promise-accounting evidence":
...
- Signed refusal is evidence of the refusing agent's own promise/intent.
- Silence or timeout is the observer's local record, not proof of intent.
- Exact bytes let later peers reassess evidence without trusting a summary.
- `SIM-tupog` is the focused current simulation; it is not a final API.

#### Freeze Successor Record

Candidate shape for a pCID that means "freeze or successor promise":
...
- A freeze record is one agent's promise over exact bytes, not an external
  authority freezing every peer's behavior.
- A successor record links evidence and intent without mutating the predecessor.
- `SIM-ravad` is the focused current simulation; it is not a final API.
...
| `SIM-dalor-grid-envelope-protocol-owned-signature-slot` | Direct test of a three-slot outer envelope where the proof stays in the outer sibling slot but the protocol named by `pcid` defines varsig/multisig-style proof rules. |
...
| Capability-token-as-promise simulation | `SIM-lurip` gives the pressure a focused home, but scoring and later DR/DI work are still needed before guide prose treats it as settled. |
...
| Kernel/runtime portability profile | `DR-davod` still needs DF after `TE-jimar`: decide whether guide prose defines kernel as a role/profile, a concrete runtime process, or wire-only interoperability. |
...
| `FB-vitih`, `FB-mulum`, `FB-potin` | `simulations/SIM-funas-kernel-porting-boundary/`; `docs/thought-experiments/TE-jimar-kernel-runtime-portability-boundary.md` | `scenarios/kernel-porting-boundary/` | Provisional kernel/runtime/dispatcher boundary evidence; settled porting target remains blocked by `DR-davod`. |
...
| Kernel Devs | Provisional / blocked for final porting instructions | The porting target is not wire-lab. A porter should expect a portable local role/profile around pCID-selected messages, exact bytes, local promises, peer-local trust assessment, evidence records, and the substrate/feed/CAS/session/message layers the porter promises to implement. Daemon, microkernel, host-runtime, browser/WASM, MCU/header-only, and split-object shapes are profiles, not one universal process shape. | The first required frozen spec set, runtime expectations, and implementation obligations are not locked yet. `DR-davod` remains open. | Future frozen binding/session/message specs by pCID, implementation `CHANGELOG.md` promise entries, and guide prose once stabilized. |
