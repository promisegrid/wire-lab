...
Snapshot generated by Codex on 2026-05-21 19:11 local time. This is a
...
| Small positional outer grid | `SIM-jufag`, `SIM-bimos`, `SIM-natim`, `SIM-maraz`, many fixed matrix variants | Keep the universal outer parse surface tiny and deterministic. Put evolution inside the pCID-selected payload profile. |
...
| Signature/proof remains profile-level | `SIM-jufag`, `SIM-bimos`, `SIM-natim`, `SIM-maraz` | A proof may be required by a pCID profile, but the current consensus does not require a large universal proof/claim card. |
...
- Allows payload profiles to evolve without changing the universal grid parse
  surface.

Weaknesses:

- The signature/proof location is not settled. It may belong inside the
  pCID-defined payload, in a profile-specific outer wrapper, or in both.
...
### Candidate Payload Profiles

Payloads are not universal claim cards. Each payload shape is owned by its pCID.
The following examples show the current level of complexity the guide should
prefer unless future simulations justify more.
...
capability-token-promise = [
  ...
  promisee: agent-ref / nil,    ; Bob, or nil for a bearer-style promise profile
  promise_body: bstr,           ; compact pCID-defined promise body
  constraint: bstr / nil,       ; optional expiry, scope, or redemption condition
  proof: bstr / nil             ; profile-defined signature or witness bytes
]
```
...
- Current simulation coverage is adjacent rather than dedicated, so this should
  remain a candidate profile until `rozas.12` or a protocol-specific TODO adds a
  focused simulation.

#### Conformance Promise Record

Candidate payload for app/kernel guide evidence without large claim cards:

```cddl
; Candidate payload only. Field meaning and arity are fixed by this payload pCID.
conformance-promise = [
  promiser: agent-ref,
  subject: cid,           ; implementation, manifest, spec, object, or receipt
  promise_body: bstr,     ; compact statement fixed by the pCID
  evidence: [* cid],      ; bounded supporting references
  proof: bstr / nil
]
```
...
- Use a pCID-specific promise body instead of a generic map of capability cards.
- Keep evidence bounded and content-addressed.
- Let Alice, Bob, and Carol decide locally whether the promise was kept in their
  relationship and workload.

#### Nested Payload Profile

Candidate shape when a profile needs recursion:
...
- Variable arity belongs to a pCID-defined profile, not to the universal grid
  envelope.
...
|---|---|---|---|
| Minimal `[pcid, payload]` | Turn-157/177 notes, `SIM-kurim`, fixed-envelope family | Smallest durable parse surface; pCID determinism does the work. | Needs profile decisions for proof, unknown retention, and canonical encoding. |
| Explicit `sig_pcid` tuple | `SIM-jufag`, `SIM-bimos`, mandatory signature-payload variants | Good when generic peers must see that a proof profile exists. | Adds universal fields that may duplicate payload-pCID semantics. |
| Outer attestation / Multisig | `SIM-natim` | Useful for origin/relay/holder/forwarder distinctions and high-value audit paths. | Heavier than the small-device default; should remain profile-specific unless consensus broadens. |
| Signed summary header + nested schema | `SIM-maraz` | Gives sparse peers bounded audit hooks. | Risks recreating generic claim cards if summary fields grow. |
| pCID-defined variable arity | `SIM-sajar` | Good inside a pCID profile. | Poor as a universal envelope rule because generic peers cannot bound parse work without profile knowledge. |
...
|---|---|
| Proof placement | Decide whether signatures/proofs are always payload-profile fields, sometimes outer profile fields, or both. |
| Unknown-pCID policy | Define store-forward, quarantine, privacy, quota, and relay behavior without implying acceptance. |
| Encoding profile | Choose CBOR, DAG-CBOR, or a phased rule while preserving exact canonical bytes. |
...
| Conformance vocabulary | Replace generic claim-card language with pCID-specific promise bodies and peer-local accounting records. |
