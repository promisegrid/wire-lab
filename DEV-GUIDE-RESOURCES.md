...
Snapshot generated by Codex on 2026-06-08 18:32 PDT. This is a
...
one-use replica tokens and later expires/revokes/renews them, Carol executes
payload-provided Fibonacci and sum functions with explicit context from Ellen
while also sending bad results that Alice/Dave/Grace reject by recomputation,
Dave checkpoints exact compute tuples and returns cache miss/hit/reuse evidence,
...
bad-proof, and key-rotation evidence. Local trust/economics/repair records remain
local promise evidence rather than authority. POC13's analyzer now reports a
bounded score report for transport, storage, compute, economics, trust,
verification, and replica recovery in addition to raw event counts, and it gates
on capacity refusal, TCP send failure, bad-result rejection, evidence pCID
routing, local non-commitment, token lifecycle, cache reuse, alternate compute,
verifier disagreement, persisted trust, and bad-proof/key-rotation cases.
...
`DI-mosil`; `DI-lupag`; `DI-nisaz`; `DI-kikoj`.
...
- `implementations/poc13-cas-compute-functions/` is executable provisional
  ...
  capacity, and issues/redeems issuer-local capability tokens. Live-agent
  ...
  protocol exchanges, and negative ACK verdicts do not inflate local trust. Its analyzer now
  gates inherited POC11/POC12 evidence and POC13 storage/compute evidence so
  future POCs cannot silently regress unless a scoped DI declares an explicit
  non-superset exception. It should be cited as POC evidence only, not as a
  final storage, compute, cache, provider, token, economics, trust, transport,
  or kernel API. Source: `DI-bibom`; `DI-notig`; `DI-lasuh`; `DI-fumol`;
  `DI-hohuf`; `DI-mosil`; `DI-lupag`; `DI-nisaz`; `DI-kikoj`; `DI-sinur`;
  `DI-punib`; `TODO-godad`; `TODO-pazif`.
