...
Snapshot generated by Codex on 2026-05-25 20:01 local time. This is a
...
that boundary. `poc3` is current executable evidence for this shape: two
containers each run one kernel plus hello, echo, and signed apps; apps make
receive promises and local keep/break judgments, while the kernel routes/refuses
bytes and records kernel evidence only. Source: `DI-fidot`; `DI-ripuz`;
`DI-horak`.
