...
Snapshot generated by Codex on 2026-05-25 23:35 local time. This is a
...
`DI-kuvum`; `DI-ripuz`; `DI-horak`; `DI-ponor`; `DI-fofik`.
...
pCID-selected `grid([42(pCID), payload, ...])` messages when an operation crosses
that boundary. `poc3` is executable evidence for the single-node local boundary:
...
`DI-kuvum`; `DI-fidot`; `DI-horak`; `DI-ponor`; `DI-fofik`.
