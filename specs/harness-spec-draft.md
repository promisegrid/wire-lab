...
25. **OQ-29.1 — Freeze ceremony (from TE-29).** Two-step commit (Alt-B), Merkle bundle (Alt-C), release record file (Alt-D), git tag with no in-spec pCID (Alt-E), or hybrid spec-doc-pCID-plus-tree-hash (Alt-F)? Decision deferred until Steve answers the two clarifying questions: (a) how important is "the spec doc, taken alone, is sufficient to verify everything important"; (b) is the simulation modelling a doc-centric or repo-centric world. Each freeze-ceremony alternative implies a different answer to those two questions.
...
33. **OQ-29.9 — External network-simulator integration, ns-3 or similar (from TE-29).** Once at least two binding specs and one session protocol have v0 implementations in `tools/`, can the wire-lab harness drive ns-3 or a similar packet-level simulator to produce realistic loss/latency adversarial scenarios? Lean: yes — treat ns-3 as an external test fixture invoked from `tools/`, producing transport-level traces replayed through the binding layer's recv path. Warrants a dedicated TE once we have something to test.
