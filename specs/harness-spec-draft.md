...
25. **OQ-29.1 — Freeze ceremony (from TE-29). CLOSED by TE-31 with Alt-G.** The spec is frozen when its doc-CID is published and at least one simrepo `CHANGELOG.md` entry references it. No tree hash, no bundle. Supersedes Alt-A through Alt-F. The simrepo's CHANGELOG points at the spec-doc CID, never the other way around (RFC-shape inversion). See TE-31 for the CHANGELOG format sketch, harness conformance-check loop, and self-consistency with PromiseGrid's promise/acceptance primitive.
