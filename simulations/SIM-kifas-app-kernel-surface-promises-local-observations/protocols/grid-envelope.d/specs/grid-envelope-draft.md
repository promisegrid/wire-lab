# Grid-envelope draft for surface promise observations

> Status: DRAFT. Simulation-local specimen only. Source: `DI-pavub`.
...
The same envelope shape is used between apps and kernels in this simulation. A local API may wrap message construction or delivery, but the PromiseGrid boundary remains the pCID-selected message. The wrapper is an adapter promise by the local implementation, not a separate authority.

Kifas keeps app/kernel surface promises and local observations under one stable protocol pCID. A payload `kind` field distinguishes surface promise records from observation records inside that protocol instead of requiring separate pCIDs for tightly coupled message variants.
