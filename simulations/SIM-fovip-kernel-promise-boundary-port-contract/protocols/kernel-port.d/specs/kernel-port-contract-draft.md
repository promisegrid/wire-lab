...
A kernel port contract is a local implementation promise record. It says what a
PromiseGrid port promises to local apps and operators, what it depends on from
the host runtime, and what it explicitly does not promise.

The record is not a global conformance certificate. Each receiving agent still
assesses the record and the port's make/break history locally.

## Record shape

```text
kernel_port_contract = [
  contract_pcid,
  port_identity,
  profile,
  supported_pcids,
...
- `contract_pcid` is the Protocol CID for this contract record shape.
...
- `app_facing_promises` states what the port promises for storage, compute,
  network, key use, device access, lifecycle, pCID dispatch, and evidence
  recording.
