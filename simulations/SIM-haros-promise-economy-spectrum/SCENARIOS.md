...
| RFC-1005 test-driven fabric prior art | Alice asks Bob to run an executable tree against a test tree with arguments; Bob caches the result only when the test passes. | Whether test tree CID, executable tree CID, and cache-on-pass terms are promise-economy primitives, prior-art vocabulary, or a separate execution layer. | PromiseGrid should learn from RFC-1005 without accidentally making test-driven execution the universal economics model. |
