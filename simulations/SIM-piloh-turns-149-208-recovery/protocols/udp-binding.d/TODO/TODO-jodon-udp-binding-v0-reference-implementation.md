...
Simulation note: this TODO moved with the `udp-binding` protocol specimen into
`simulations/SIM-piloh-turns-149-208-recovery/protocols/udp-binding.d/` per
`DI-fakin`. Historical path references below remain evidence of the pre-Mupoz
layout unless a line explicitly names the simulation-local path.
