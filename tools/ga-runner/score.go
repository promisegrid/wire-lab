...
func buildScorePrompt(repo Repo, state GAState, cell GACell, scenario Scenario) (string, error) {
	...
	out.WriteString("- `app_protocol_promise_semantics`: reward higher-layer/app protocols that model storage, computation, send/receive, reciprocal promises, selective sending, promise-as-capability-token behavior, make/break evidence, and local trust updates without command, request/response service, permission, policy-enforcement, or conformance-authority framing. Related promise and observation messages that evolve together should normally be payload `kind` variants under one stable protocol pCID; separate pCIDs are strong only when message families are independently deployable, independently understandable, or separated by a real layer boundary.\n")
