...
func buildScorePrompt(repo Repo, state GAState, cell GACell, scenario Scenario) (string, error) {
	...
	//
	// Intent: Treat pCID as a stable protocol-spec identifier rather than a
	// per-message type. Tightly coupled promise/observation records should score
	// well when represented as payload kinds under one protocol pCID, while
	// needless per-record pCID fragmentation should score as overbuilt. Source:
	// DI-gakij
	//
	// Intent: Penalize spurious action-kind proliferation so future GA work does
	// not turn repair, economics, routing, storage, compute, token exchange, or
	// TCP-link choices into RPC-like verbs. Those meanings belong in pCID-owned
	// payloads and local trust/evidence interpretation unless a TE/DI proves a
	// distinct wire-level role. Source: DI-mosoj
	var out strings.Builder
	...
	out.WriteString("- `envelope_discipline`: reward alignment with `DN-jotob` and the current envelope direction: CBOR `grid([42(pCID), payload, ...])`, `pCID` as Protocol CID, protocol-owned slot roles after slot 0, local unknown-pCID behavior, and no universal proof-slot overreach. Treat pCID as a stable protocol-spec identifier, not as a per-message type, payload CID, request ID, operation code, or nonce.\n")
	...
	out.WriteString("- `app_protocol_promise_semantics`: reward higher-layer/app protocols that model storage, computation, send/receive, reciprocal promises, selective sending, promise-as-capability-token behavior, make/break evidence, and local trust updates as voluntary cooperation among agents with promise-based local trust, decentralized accountability, and resilient community-scale operation rather than command/control, request/response service, permission, or policy-enforcement framing. The default future-facing top-level semantic action is `promise`; observation should be a promise about local observation, and refusal should be promise absence or a promise about local non-action/non-commitment. Repair, offer/counter/acceptance, route, introduction, redemption, transfer, storage, compute, and TCP-link changes should appear as pCID-defined payload meaning or local interpretation unless the candidate explicitly justifies a distinct wire-level role. Related promise payloads that evolve together should normally remain under one stable protocol pCID; separate pCIDs are strong only when message families are independently deployable, independently understandable, or separated by a real layer boundary.\n")
	...
	out.WriteString("- `pt_reframe_needed`: technically interesting but drifts into non-PT framing, including RPC-like service dispatch, service-registry vocabulary, capability-table vocabulary, spurious action-kind proliferation, or kernel-known-support claims that could be repaired as local promises and observations; it may survive only as a question-home or rework candidate.\n")
