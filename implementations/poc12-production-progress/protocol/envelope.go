...
// Intent: Keep POC12 aligned with grid([42(pCID), ...protocol-defined-slots])
...
func ParseEnvelope(envelopeBytes []byte) (Envelope, error) {
	...
	if arrayLength != 3 {
		return Envelope{}, fmt.Errorf("poc12 envelope must have three slots, got %d", arrayLength)
	}
...
func DeterministicPrivateKey(seedText string) ed25519.PrivateKey {
	seed := sha256.Sum256([]byte("poc12 protocol signer: " + seedText))
