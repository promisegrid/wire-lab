...
func (node *Node) sendUnknownProtocolPromise(target string) error {
	...
	unknownCID := protocol.NewProtocolCID([]byte("poc14 unknown mallory probe protocol"))
...
// Intent: POC14 needs exact-byte promise accounting for real sends while still
...
// loadRunScopedState restores app-local state that belongs to this one POC14
...
func (node *Node) recordRunScopedRetentionAndGC() {
	state := node.exportRunScopedState()
	node.record("retention_promise_recorded", "kept", "", "run-scoped CAS, compute, token, replay, and journal state is retained only for this POC14 run")
...
func (node *Node) recordReplayWindowPromise(protocolName string) {
	...
	if node.rememberCheckpoint(checkpointRecord{
		...
		Detail:       "exact-envelope replay window scoped to one POC14 run",
	}) {
		return
	}
...
// Intent: Keep relationship learning across POC14 runs without introducing a
...
// Intent: Let multi-run POC14 experiments test relationship decay and repair
