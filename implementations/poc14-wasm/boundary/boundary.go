...
func RunWASMModule(ctx context.Context, moduleBytes []byte, inputValue uint64) (result WASMRunResult, err error) {
	...
	module, err := wasmRuntime.InstantiateModule(ctx, compiledModule, wazero.NewModuleConfig().WithName("poc14-peggy"))
	if err != nil {
...
// envelope through the normal POC14 app/kernel path.
