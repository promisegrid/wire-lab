...
func NewRegistry() Registry {
	...
	for _, entry := range []struct {
		...
		{RelationshipV1, "poc14 relationship trust discovery observation protocol v1"},
		{PostalScaleV1, "poc14 postal scale package weighing protocol v1"},
		{UPSLabelV1, "poc14 ups label printing cost tracking protocol v1"},
		{AccountingV1, "poc14 accounting address lookup shipment update protocol v1"},
		{PrinterPortV1, "poc14 printer port capability token issue redemption protocol v1"},
...
func (registry Registry) MustCID(name string) protocol.ProtocolCID {
	...
	if !ok {
		panic(fmt.Sprintf("unknown POC14 pCID name %q", name))
	}
	return protocolCID
}
...
// Known reports whether a protocol name is in the POC14 registry.
