...
- `poc13-cas-compute-functions/` — proof-of-concept 13: self-contained executable evidence for decentralized CAS storage promises and CID-named function-call compute promises. The provisional protocols are `cas_storage_v1` and `cid_compute_v1`; content/function/context/result identity lives in pCID-owned payloads, while pCID remains protocol identity. It uses signed `grid([42(pCID), payload, proof])` envelopes over length-framed TCP, local evidence logs, analyzer gates, and LLM-capable agents with scripted fallback when no provider key is available. It now proves real CAS store/serve/retrieve/replicate checks, payload-provided Fibonacci compute, local trust updates, voluntary repair promises, credit-style economics, and issuer-local capability-token issue/redeem evidence. Live-provider runs extract nested provider text, record role-aligned local promise judgments, and fail placeholder decision evidence. It is executable evidence, not a final storage, compute, cache, provider, token, economics, trust, transport, or kernel API. Source: `DI-bibom`; `DI-notig`; `DI-lasuh`; `DI-fumol`; `DI-hohuf`; `TODO-godad`.
