...
- `poc7-capability-token-exchange/` — proof-of-concept 7: five-container bounded simulation for signed CBOR-grid bearer and non-transferable promise tokens, issuer-local redemption/revocation, real storage/compute/data redemption payloads, peer-local exchange offers, and local trust updates. It is executable evidence for `scenarios/promise-economy-capability-token-exchange/`, not a final token, storage, compute, or economics API. Source: `DI-tugih`; `DI-fibok`.
