...
- `poc7-capability-token-exchange/` — proof-of-concept 7: five-container bounded simulation for bearer and non-transferable promise tokens, issuer-local redemption/revocation, peer-local exchange offers, and local trust updates. It is executable evidence for `scenarios/promise-economy-capability-token-exchange/`, not a final token or economics API. Source: `DI-tugih`.
