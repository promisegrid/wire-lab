...
- `poc13-cas-compute-functions/` — proof-of-concept 13: self-contained executable evidence for decentralized CAS storage promises and CID-named function-call compute promises. The provisional protocols are `cas_storage_v1`, `cid_compute_v1`, and `evidence_report_v1`; content/function/context/result identity lives in pCID-owned payloads, while pCID remains protocol identity. It uses signed `grid([42(pCID), payload, proof])` envelopes over length-framed TCP, local evidence logs, analyzer gates, and LLM-capable agents with scripted fallback when no provider key is available. It now proves real CAS store/serve/retrieve/replicate checks, TCP-level Bob retrieval failure, Frank-issued replica-token fallback, payload-provided Fibonacci compute, bad-result rejection by Alice/Dave/Grace recomputation, evidence-report pCID routing, unknown-pCID and unsupported-variant non-commitment, local trust-driven peer choice, voluntary repair promises, capacity/opportunity-cost/credit economics, issuer-local capability-token issue/redeem evidence, and analyzer score/report fields. Live-provider runs extract nested provider text, record role-aligned local promise judgments, and fail placeholder decision evidence. It is executable evidence, not a final storage, compute, cache, provider, token, economics, trust, transport, or kernel API. Source: `DI-bibom`; `DI-notig`; `DI-lasuh`; `DI-fumol`; `DI-hohuf`; `DI-mosil`; `DI-lupag`; `DI-nisaz`; `TODO-godad`.
