...
- `poc13-cas-compute-functions/` — proof-of-concept 13: self-contained executable evidence for decentralized CAS storage promises and CID-named function-call compute promises. The provisional protocols are `cas_storage_v1`, `cid_compute_v1`, and `evidence_report_v1`; content/function/context/result identity lives in pCID-owned payloads, while pCID remains protocol identity. It uses signed `grid([42(pCID), payload, proof])` envelopes over length-framed TCP, local evidence logs, analyzer gates, and LLM-capable agents with scripted fallback when no provider key is available. It now proves real CAS store/serve/retrieve/replicate checks, named TCP outage variants, Frank-issued replica-token fallback with TTL/expiry/revocation/renewal, multi-object CAS pressure, payload-provided Fibonacci and sum compute, bad-result rejection by Alice/Dave/Grace recomputation, Dave cache miss/hit/reuse, verifier disagreement with Alice-local resolution, evidence-report pCID routing, unknown-pCID and unsupported-variant non-commitment, persisted-trust peer choice, key-rotation promises, bad-proof rejection, voluntary repair promises, capacity/opportunity-cost/credit economics, issuer-local capability-token issue/redeem evidence, and analyzer score/report fields. Live-provider runs extract nested provider text, record role-aligned local promise judgments, and fail placeholder decision evidence. It is executable evidence, not a final storage, compute, cache, provider, token, economics, trust, transport, or kernel API. Source: `DI-bibom`; `DI-notig`; `DI-lasuh`; `DI-fumol`; `DI-hohuf`; `DI-mosil`; `DI-lupag`; `DI-nisaz`; `DI-kikoj`; `TODO-godad`.
- `poc14-wasm/` — proof-of-concept 14: POC13 superset scaffold that adds heterogeneous runtime boundary evidence. Peggy runs as a WASM-boundary app process, Victor uses a stdio-only worker process behind a local adapter, and both still exchange ordinary signed `grid([42(pCID), payload, proof])` PromiseGrid envelopes through local kernels. Its analyzer adds boundary, decentralized-monitoring, mixed-version pCID migration, and run-internal restart/recovery gates without treating analyzer output as production authority. It is executable evidence, not a final WASM host, stdio adapter, monitor, pCID migration, crash-recovery, kernel, or app API. Source: `DI-sihuz`; `DI-sifot`; `DI-lulof`; `DI-linof`; `TODO-gogug`.
- `poc15-multihop-multiarity-dag/` — proof-of-concept 15: multihop, multiarity, and parent-linked DAG pressure after POC14. It keeps exact `grid()` CBOR messages, supports pCID-owned slot vectors, records raw message artifacts, exercises promise-shaped forwarding, and keeps observer/analyzer behavior outside the production trust path. It is executable evidence, not a final routing, forwarding, DAG, monitor, or proof API. Source: `TODO-gogug`.
- `poc16-secure-tokens-maps-encrypted-payloads/` — proof-of-concept 16: POC15 successor focused on pCID-selected parser/builder roles, RFC-like implementation-local specdocs, CWT/COSE-shaped secure tokens, encrypted payloads, map-permitted payload profiles, lifecycle/resource promises, and strict CID rendering. It is executable evidence, not a final secure-token, kernel, app, parser, encryption, or spec registry API. Source: `TODO-zugok`; `TODO-sosoj`; `TODO-milov`.
- `poc17-m4-lora-runtime/` — proof-of-concept 17: constrained-runtime behavior simulator for a Cortex-M4/LoRa-shaped agent. It keeps real pCID bytes, compact CBOR payloads, binary CIDs on wire, base32 printable CIDs, radio-visible peer storage promises, resource-limit reporting, and restart recovery pressure. It is executable behavior evidence, not final Feather/RFM95 firmware or radio-driver fidelity. Source: `TODO-komon`.
- `poc18-cas-git-replacement/` — proof-of-concept 18: CAS-backed VCS and Git/GitHub replacement pressure. It uses sparse per-agent CAS, Rabin chunking, POSIX node promises, reference sets, snapshots, Git bridge adapters, TCP-carried object retrieval, CAR payloads, signed CWT/COSE economics, continuous peer DAG sync, repo-local `.grid` state, and diagnostic CBOR rendering. It is executable evidence, not a final VCS, daemon, app install, or production storage API. Source: `TODO-nahop`.
- `poc19-production-shape/` — proof-of-concept 19 planning tree: design-doc-first successor to POC18. It targets one `grid` binary with daemon/client modes, VCS/CAS app installation through signed reference sets, WASI-first app execution, and equal TCP/WebSocket PromiseGrid messaging. It is a design artifact until implementation begins. Source: `DI-lumir`; `TODO-vumas`.
