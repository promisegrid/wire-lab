...
func run() error {
	...
	kernelBinary := flag.String("kernel-bin", "poc15-kernel", "POC15 kernel binary path")
	flag.Parse()
	if *containerName == "" {
...
func appBinaryForAgent(cfg config.Config, agentName string) (string, error) {
	...
	switch agent.Kind {
	case "":
		return "poc15-relationship-agent", nil
	case "fulfillment":
		return "poc15-fulfillment", nil
	case "postal_scale":
		return "poc15-postal-scale", nil
	case "ups_label_printer":
		return "poc15-ups-label-printer", nil
	case "printer_port":
		return "poc15-printer-port", nil
	case "accounting":
		return "poc15-accounting", nil
	case "wasm_agent":
		// Intent: POC15 adds heterogeneous runtime-adapter app roles as separate
		// supervised processes while preserving the same local-kernel routing
		// model used by POC12/POC13 agents. Source: DI-linof
		return "poc15-wasm-agent", nil
	case "stdio_agent":
		...
		// DI-linof
		return "poc15-stdio-adapter", nil
	default:
