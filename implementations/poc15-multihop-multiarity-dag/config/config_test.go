...
func TestComposeMountsRunVolumeOnlyForCollector(t *testing.T) {
	...
	if strings.Contains(composeText, "poc15-run") {
		t.Fatalf("compose should not retain the old shared poc15-run volume")
	}
	if count := strings.Count(composeText, "/run/poc15"); count != 1 {
		t.Fatalf("compose /run/poc15 mount count = %d, want exactly collector-only mount", count)
	}
}
