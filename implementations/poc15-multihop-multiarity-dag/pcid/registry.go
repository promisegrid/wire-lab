...
func NewRegistry() Registry {
	...
	for _, entry := range []struct {
		...
		{RelationshipV1, "poc15 relationship trust discovery observation protocol v1"},
		{PostalScaleV1, "poc15 postal scale package weighing protocol v1"},
		{UPSLabelV1, "poc15 ups label printing cost tracking protocol v1"},
		{AccountingV1, "poc15 accounting address lookup shipment update protocol v1"},
		{PrinterPortV1, "poc15 printer port capability token issue redemption protocol v1"},
		{KernelReceiveV1, "poc15 local app receive promise registration protocol v1"},
		{CASStorageV1, "poc15 decentralized cas storage promise protocol v1"},
...
func (registry Registry) MustCID(name string) protocol.ProtocolCID {
	...
	if !ok {
		panic(fmt.Sprintf("unknown POC15 pCID name %q", name))
	}
	return protocolCID
}
...
// Known reports whether a protocol name is in the POC15 registry.
