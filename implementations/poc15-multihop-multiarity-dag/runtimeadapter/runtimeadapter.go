...
// Intent: POC15 should now distinguish real runtime execution from the older
...
func RunWASMModule(ctx context.Context, moduleBytes []byte, inputValue uint64) (result WASMRunResult, err error) {
	...
	module, err := wasmRuntime.InstantiateModule(ctx, compiledModule, wazero.NewModuleConfig().WithName("poc15-peggy"))
	if err != nil {
