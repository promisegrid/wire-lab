...
// Intent: Keep POC10 aligned with grid([42(pCID), ...protocol-defined-slots])
// while testing LLM-directed behavior above the envelope layer. Source: DI-pijan
...
// envelope and exact-byte signature mechanics. Source: DI-pijan
...
func ParseEnvelope(envelopeBytes []byte) (Envelope, error) {
	...
	if arrayLength != 3 {
		return Envelope{}, fmt.Errorf("poc10 envelope must have three slots, got %d", arrayLength)
	}
...
func DeterministicPrivateKey(seedText string) ed25519.PrivateKey {
	seed := sha256.Sum256([]byte("poc10 protocol signer: " + seedText))
