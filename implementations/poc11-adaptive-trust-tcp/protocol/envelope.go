...
// Intent: Keep POC11 aligned with grid([42(pCID), ...protocol-defined-slots])
// while testing LLM-directed behavior above the envelope layer. Source: DI-hotos
...
// envelope and exact-byte signature mechanics. Source: DI-hotos
...
func ParseEnvelope(envelopeBytes []byte) (Envelope, error) {
	...
	if arrayLength != 3 {
		return Envelope{}, fmt.Errorf("poc11 envelope must have three slots, got %d", arrayLength)
	}
...
func DeterministicPrivateKey(seedText string) ed25519.PrivateKey {
	seed := sha256.Sum256([]byte("poc11 protocol signer: " + seedText))
