...
const (
	...
	// interpreted as 0x67726964 / decimal 1735551332. Intent: Make POC9's bytes
...
// Intent: Keep POC9 aligned with the current grid-envelope rule:
...
// view. Intent: Make POC9 sign exact CBOR bytes without making proof placement a
...
// Intent: Replace the first POC9 JSON-as-protocol sketch with a real
...
func ParseEnvelope(envelopeBytes []byte) (Envelope, error) {
	...
	if arrayLength != 3 {
		return Envelope{}, fmt.Errorf("poc9 envelope must have three slots, got %d", arrayLength)
	}
...
func DeterministicPrivateKey(seedText string) ed25519.PrivateKey {
	seed := sha256.Sum256([]byte("poc9 protocol signer: " + seedText))
