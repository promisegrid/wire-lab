...
func (issuer *Issuer) Issue(id string, originalPeer string, resourceKind string, resourceID string, transferRule string) (Token, error) {
	return issuer.IssueExpiring(id, originalPeer, resourceKind, resourceID, transferRule, 0)
}

// IssueExpiring signs a token whose own promise states a last-valid time.
// Intent: Expiry is part of Alice's signed promise, so redemption after that
// time is not treated as a broken promise by Alice. Source: DI-vujil
func (issuer *Issuer) IssueExpiring(id string, originalPeer string, resourceKind string, resourceID string, transferRule string, expiresAtUnix int64) (Token, error) {
...
// SetNowUnix gives the bounded POC a deterministic issuer clock for expiry.
// Intent: POC9 needs expiry evidence without using wall-clock time or test-race
// timing; production protocols would specify their own time promise semantics.
// Source: DI-vujil
func (issuer *Issuer) SetNowUnix(nowUnix int64) {
	issuer.nowUnix = nowUnix
}
...
func (issuer *Issuer) Redeem(holder string, presented Token) Event {
	...
	case stored.ExpiresAtUnix > 0 && issuer.nowUnix > stored.ExpiresAtUnix:
		event.Outcome = OutcomeExpired
		event.Detail = "token expired under issuer's signed expiry promise"
...
// Intent: Let Dave judge Mallory's expired-token circulation separately from
// Dave's judgment about Alice's issuer promise. Source: DI-pabot; DI-vujil
func (wallet *Wallet) ApplyPeerObservation(peer string, event Event) {
	switch event.Outcome {
	case OutcomeKept:
		wallet.trust[peer] += 1
	case OutcomeBroken, OutcomeRefused, OutcomeExpired:
		wallet.trust[peer] -= 1
	}
...
func CanonicalBytes(token Token) ([]byte, error) {
	return protocol.MarshalStringMap(map[string]string{
		...
		"expires_at_unix": strconv.FormatInt(token.ExpiresAtUnix, 10),
...
func Decode(tokenBytes []byte) (Token, error) {
	...
	expiresAtUnix, expiresErr := parseExpiresAtUnix(fields["expires_at_unix"])
	if expiresErr != nil {
		return Token{}, expiresErr
	}
	return Token{
		ID:            fields["id"],
...
func parseExpiresAtUnix(raw string) (int64, error) {
	if raw == "" {
		return 0, nil
	}
	expiresAtUnix, parseErr := strconv.ParseInt(raw, 10, 64)
	if parseErr != nil {
		return 0, fmt.Errorf("parse expires_at_unix: %w", parseErr)
	}
	return expiresAtUnix, nil
}

func deterministicPrivateKey(seedText string) ed25519.PrivateKey {
	seed := sha256.Sum256([]byte("poc9 token issuer: " + seedText))
