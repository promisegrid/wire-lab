...
// Intent: POC16 must not import or shell out to ga-runner; this client is
...
// Intent: Use provider schema enforcement without changing the POC16 protocol
...
func (client LiveClient) Evaluate(ctx context.Context, events []Event) (MonitorReport, error) {
	...
	userPrompt := "Evaluate this POC16 run as an observer only. Return exactly one JSON object matching " +
...
func monitorSystemPrompt() string {
	// Intent: POC16 deliberately includes malformed, adversarial, replay, and
	// unsupported-pCID probes. The observer-only monitor should score protocol
	// validity by whether those probes are contained correctly, not by treating
	// their mere presence as a protocol failure. Source: DI-daruf
	return "You are an observer-only PromiseGrid monitor. Evaluate logs for Promise Theory fit, autonomy, protocol validity, local trust correctness, and imposition avoidance. Score protocol validity by containment: deliberate malformed, adversarial, replay, unsupported-pCID, bad-proof, and wrong-result probes should not lower protocol_validity when agents or kernels reject them, record them as non_commitment or malformed, and keep trust local. Lower protocol_validity only when invalid probes are accepted as valid, valid pCID-shaped promises cannot be exchanged, or RPC/authority semantics leak into protocol behavior. You are not an authority and must not control agents. Return only JSON."
}
