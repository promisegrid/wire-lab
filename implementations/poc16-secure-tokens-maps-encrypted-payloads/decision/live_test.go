...
func TestAPIKeyReadsDirectEnvironment(t *testing.T) {
	t.Setenv("POC16_TEST_API_KEY", "direct-test-key")
	client := NewLiveClient("https://example.invalid/v1/responses", "POC16_TEST_API_KEY", "agent", "monitor", "medium", "flex", time.Second)
...
func TestAPIKeyReadsFileEnvironment(t *testing.T) {
	...
	t.Setenv("POC16_TEST_API_KEY_FILE", secretPath)
	client := NewLiveClient("https://example.invalid/v1/responses", "POC16_TEST_API_KEY", "agent", "monitor", "medium", "flex", time.Second)
	apiKey, err := client.apiKey()
	if err != nil {
		t.Fatalf("api key: %v", err)
...
func TestAPIKeyRejectsMissingSources(t *testing.T) {
	client := NewLiveClient("https://example.invalid/v1/responses", "POC16_TEST_MISSING_API_KEY", "agent", "monitor", "medium", "flex", time.Second)
...
func TestLiveClientRequestsStructuredDecisionOutput(t *testing.T) {
	t.Setenv("POC16_TEST_API_KEY", "test-key")
	...
	if formatObject["type"] != "json_schema" || formatObject["name"] != "poc16_promise_decision" {
...
func TestMonitorPromptScoresNegativeProbesByContainment(t *testing.T) {
	prompt := monitorSystemPrompt()
	for _, requiredText := range []string{
		"Score protocol validity by containment",
		"unsupported-pCID",
		"bad-proof",
		"should not lower protocol_validity when agents or kernels reject them",
		"Lower protocol_validity only when invalid probes are accepted as valid",
	} {
		if !strings.Contains(prompt, requiredText) {
			t.Fatalf("monitor prompt missing %q: %s", requiredText, prompt)
		}
	}
}
