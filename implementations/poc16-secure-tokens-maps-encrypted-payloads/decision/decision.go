...
type Observation struct {
	...
	ProtocolSpecs  []ProtocolSpecContext `json:"protocol_specs,omitempty"`
	RecentEvents   []Event               `json:"recent_events"`
	RequiredAct    string                `json:"required_act"`
}

// ProtocolSpecContext is the exact pCID spec context supplied to a live LLM
// agent for the pCIDs it locally promises to handle.
//
// Intent: POC16 should not ask LLM agents to infer payload semantics from pCID
// names alone; prompt context records the spec CID and excerpt while Go still
// owns CBOR parsing, signing, and transport mechanics. Source: DI-vulit
...
// DI-galin; DI-punib; DI-sihuz; DI-vipih; DI-gazin
func Prompt(observation Observation) (string, error) {
	...
	return "Return exactly one JSON object matching this shape: " +
		...
		"Use protocol=relationship_v1 for ordinary trust, discovery, observation, capacity, or pricing promises unless you can provide every required payload field for another pCID. Use protocol=production_shipping_v1 for package weighing, address lookup, label/cost/tracking, printer-port token, and shipment-update promises when the payload includes the concrete operation fields. Use protocol=cas_storage_v1 only for content-addressed storage payloads with concrete content/token fields, and protocol=cid_compute_v1 only for CID-named compute/cache/verifier payloads with concrete function/input/context/result fields. identity_key_v1 is reserved for scripted pCID-owned key-rotation array payloads, not live generic-map turns. secure_capability_v1, encrypted_payload_v1, parser_builder_role_v1, and map_payload_profile_v1 are broad protocol profiles; never use a pCID as a peer address or message type. " +
...
// Intent: DI-kirat makes event the active POC16 runtime/log term; live LLMs may
