...
func TestComposeMountsRunVolumeOnlyForCollector(t *testing.T) {
	...
	if strings.Contains(composeText, "poc16-run") {
		t.Fatalf("compose should not retain the old shared poc16-run volume")
	}
	if count := strings.Count(composeText, "/run/poc16"); count != 1 {
		t.Fatalf("compose /run/poc16 mount count = %d, want exactly collector-only mount", count)
	}
}
