...
// Config is the repo-local, non-secret runtime configuration for POC16.
...
// Load reads and validates a POC16 config file.
...
// Validate checks that the config is complete enough for a bounded POC16 run.
...
// lifecycle as stuck.
// Intent: POC16 clean runs should allow live agents and the collector-side
...
// POC16 runtime uses KernelAppPortForContainer and KernelPeerPortForContainer
// instead so every container has one kernel interface and local app processes
// register receive promises with that kernel.
// Intent: Keep old test helpers stable while moving production POC16 routing to
...
// Intent: POC16 tests kernel routing by slot-0 pCID to app receive promises;
