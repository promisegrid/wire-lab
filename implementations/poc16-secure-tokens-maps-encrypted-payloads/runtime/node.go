...
// Node runs one local POC16 app process. A container may run several app
...
// Intent: POC16 needs promise-state words that distinguish local failure,
...
// currently tracking.
// Intent: POC16 applies peer trust only after a local promise event is recorded
...
// muddying the next POC16 run. Source: DI-sunuf; DI-manul; DI-fagog
type runScopedState struct {
	Version int `json:"version"`
	// CASObjects is read-only legacy migration input from earlier POC16 state
...
// Intent: Earlier POC16 nodes wrote marker files and one node ran the monitor
...
func (node *Node) runStartupWorkflow(ctx context.Context) error {
	...
	// Intent: POC16 is now a superset of POC12, so the production shipping
	// workflow remains intact while Alice and Mallory add CAS/compute protocol
	// pressure above the same app/kernel interface, while POC16 adds WASM and
...
// Intent: POC16 is a POC13 superset, so this preserves inherited
...
func (node *Node) runCASComputeWorkflow() error {
	...
	if err := node.runPOC16ProtocolProfileWorkflow(contentCID); err != nil {
		return err
	}
	...
	missingObjectCID := production.ContentCID([]byte("poc16 missing sparse CAS object|" + node.Config.RunID))
...
// same app/kernel TCP path as other POC16 messages. Source: DI-sihuz
...
func (node *Node) runAdversaryWorkflow() error {
	...
	if _, err := node.sendIdentityKeyRotationPromise("grace", "mallory-next-key", "future-poc16-identity"); err != nil {
...
func (node *Node) newAckBytes(target, outcome, promiseText string, protocolCID protocol.ProtocolCID, parentCID string, extraFields map[string]string) ([]byte, error) {
	...
	if protocolCID.Equal(node.Protocols.MustCID(pcid.IdentityKeyV1)) && extraFields["promise_about"] == production.PromiseRotateSigningKey {
		// Intent: identity_key_v1 is the first POC16 protocol whose request and
...
func (node *Node) sendUnknownProtocolPromise(target string) error {
	...
	unknownCID := protocol.NewProtocolCID([]byte("poc16 unknown mallory probe protocol"))
...
// the first POC16 payload migrated under `DI-vipih`. Source: DI-vipih
...
// Intent: POC16 stdio workers must observe the peer's original ACK envelope
...
// Intent: POC16 needs a minimal bearer-token incentive path where the current
...
func (node *Node) observation(turnIndex int) decision.Observation {
	...
	return decision.Observation{
		...
		ProtocolSpecs:  node.protocolSpecContexts(),
		RecentEvents:   recentEvents,
		RequiredAct:    decision.ActPromise,
	}
}

func (node *Node) protocolSpecContexts() []decision.ProtocolSpecContext {
	// Intent: Live-agent prompt context should include the actual pCID spec prose
	// for locally supported protocols, but prompt construction must not make the
	// kernel route by prose or by pCID-as-address. Source: DI-vulit
	contexts, contextsErr := specdocs.ContextsFor(node.Agent.Protocols(), 1600)
	if contextsErr != nil {
		return nil
	}
	observationContexts := make([]decision.ProtocolSpecContext, 0, len(contexts))
	for _, context := range contexts {
		protocolCID, known := node.Protocols.CID(context.Name)
		if !known {
			continue
		}
...
func (node *Node) observeOutcome(peerName string, outcome relationship.Outcome) {
	...
	// Intent: POC16 keeps POC12 relationship-transition events and also emits
	// the inherited POC16 analyzer's `trust_updated` event name so future
...
// restraint event record for later turns in the same POC16 run.
...
// RPC-like; POC16 should instead record local non-commitment and wait for a new
...
// vocabulary used by existing POC16 logs and analyzer summaries.
...
// loadRunScopedState restores app-local state that belongs to this one POC16
...
// for any already-written POC16 run state that still has `cas_objects_b64`.
// Source: DI-fagog
...
func (node *Node) recordRunScopedRetentionAndGC() {
	...
	state := node.exportRunScopedState()
	node.record("retention_promise_recorded", "kept", "", "run-scoped CAS, compute, token, replay, and journal state is retained only for this POC16 run")
...
func (node *Node) recordReplayWindowPromise(protocolName string) {
	...
	if node.rememberCheckpoint(checkpointRecord{
		...
		Detail:       "exact-envelope replay window scoped to one POC16 run",
	}) {
		return
	}
...
// Intent: Keep relationship learning across POC16 runs without introducing a
...
// Intent: Let multi-run POC16 experiments test relationship decay and repair
