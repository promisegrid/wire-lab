...
// Intent: POC16 adds heterogeneous process-runtime adapter events without making
...
func (node *Node) runStdioComputeWorker(message parsedMessage) ([]byte, error) {
	command := exec.Command("poc16-stdio-worker")
	...
	node.record("stdio_compute_worker_started", "kept", target, "worker=poc16-stdio-worker pcid="+pcid.CIDComputeV1)
...
// Intent: POC16 tests subprocess adapters without giving stdio messages RPC
...
func (node *Node) runStdioAdapterWorkflow(ctx context.Context) error {
	...
	command := exec.CommandContext(ctx, "poc16-stdio-worker")
	command.Stderr = os.Stderr
	stdin, stdinErr := command.StdinPipe()
	if stdinErr != nil {
		return stdinErr
	...
	node.record("stdio_worker_started", "kept", target, "worker=poc16-stdio-worker")
...
// recordDecentralizedMonitoringEvents emits POC16 event records for analysis
...
// recordPermanentDistrustAndTransitExclusionEvents records two POC16 local
...
// Intent: POC16 should test crash/restart expectations as local promises by each
