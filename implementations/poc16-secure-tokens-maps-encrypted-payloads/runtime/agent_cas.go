...
// message review store so POC16 can model incomplete per-agent stores, local
...
// recordAgentCASAccessEvents records the baseline promises every POC16 app makes
...
// Intent: POC16 deliberately mixes .bin, typed .cbor, and local CBOR wrapper
...
func (node *Node) encryptLocalCASBytes(cleartextBytes []byte) ([]byte, string, error) {
	...
	keySeed := sha256.Sum256([]byte("poc16-agent-cas-key|" + node.Agent.Name))
	...
	nonceSeed := sha256.Sum256(append([]byte("poc16-agent-cas-nonce|"+node.Agent.Name+"|"), cleartextBytes...))
	// Intent: The nonce is deterministic only so clean POC16 runs are
