...
// cborWriter writes the deterministic CBOR subset POC16 needs.
...
func (writer *cborWriter) writeBool(value bool) error {
	// Intent: POC16's CWT-like token profile needs real CBOR boolean claims so
	// bearer/non-transferable token promises are not flattened into string-only
	// maps. Source: DI-vulit
	if value {
		return writer.buffer.WriteByte(0xf5)
	}
	return writer.buffer.WriteByte(0xf4)
}
...
// cborReader reads the deterministic CBOR subset POC16 writes.
...
func (reader *cborReader) readBool() (bool, error) {
	// Intent: Decode the same narrow boolean subset written by writeBool for
	// signed capability-token claims; other CBOR simple values remain unsupported
	// in token parsing. Source: DI-vulit
	...
	case 0xf5:
		return true, nil
	default:
		return false, fmt.Errorf("expected cbor bool, got 0x%x", value)
	}
}
