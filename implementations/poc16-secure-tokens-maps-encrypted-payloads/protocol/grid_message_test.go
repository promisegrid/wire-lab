...
func TestGridMessageParsesVariableArity(t *testing.T) {
	// Intent: POC16 must be able to inspect pCID-owned slot vectors without
	// assuming every grid message is the current payload/proof pair. Source:
	// DI-mosat
	protocolCID := NewProtocolCID([]byte("poc16 variable arity test protocol"))
...
func TestGridMessageAcceptsTransportOnlyShape(t *testing.T) {
	protocolCID := NewProtocolCID([]byte("poc16 transport only test protocol"))
...
func TestCOSESign1VerifiesAndRejectsTampering(t *testing.T) {
	payload := []byte("poc16 COSE payload")
