# message_shape_payload_parents_v1
...
message_shape_payload_parents_v1 specifies one concrete envelope arity specimen: `grid([42(pCID), payload, parents, proof])`. It exists so
...
```text
grid([42(pCID), payload, parents, proof])
```
...
```text
[payload parsed] --slot 2 parents decoded--> [DAG edge visible after payload parse]
