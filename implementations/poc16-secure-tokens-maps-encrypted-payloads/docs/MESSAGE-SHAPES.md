# POC16 Message Shapes, Parent Links, COSE, and Raw CAS DAG

POC16 tests that `pCID` really owns arity, slot meaning, signable view,
...
Every raw artifact observed during a POC16 run should be retained under the
...
POC16 uses mixed per-agent file profiles. A local CAS object may be a generic
...
POC16 should keep two DAG concepts separate:
...
recorded as secondary indexes, but parent links for POC16 should point at exact
envelope CIDs so replay and review are byte-faithful.

## Slot-Vector Specimens

POC16 now emits exact raw-message specimens for these pCID-owned slot vectors:
...
POC16 should test transport-auth-only messages, but it must not report them as
