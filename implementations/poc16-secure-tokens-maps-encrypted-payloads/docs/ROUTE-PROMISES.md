# POC16 Route Promises

POC16 routing should be modeled as promises between neighboring agents, not as a
...
The POC16 route setup should test a lightning-leader pattern without importing
...
Forwarding is not free. POC16 should test incentives as ordinary reciprocal
promises:
...
A route is durable only if each hop promises durability. POC16 should test:
...
POC16 should add a narrow route pCID because route setup and forwarding have real
...
events or payload meanings unless POC16 locks a specific pCID-owned payload.
