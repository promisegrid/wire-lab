...
// RunSummary is the deterministic JSON report emitted by poc16-analyze.
// Intent: Keep POC16 live runs comparable without committing provider logs or
...
// ScoreReport gives the operator a fast POC16 fitness view in addition to the
// raw event counters.
...
type ScoreReport struct {
	...
	ParserBuilder  int      `json:"parser_builder"`
	PayloadProfile int      `json:"payload_profile"`
	SpecContext    int      `json:"spec_context"`
	POC15Superset  int      `json:"poc15_superset"`
...
// AcceptanceCriteria describes the event-record gates for a clean POC16 regression
// run.
...
func main() {
	if len(os.Args) != 2 {
		fmt.Fprintf(os.Stderr, "usage: poc16-analyze RUN_DIR\n")
		os.Exit(2)
	}
	summary, err := analyzeRun(os.Args[1])
	if err != nil {
		fmt.Fprintf(os.Stderr, "poc16-analyze: %v\n", err)
		os.Exit(1)
	}
	if err := validateSummary(summary, cleanRegressionCriteria()); err != nil {
		fmt.Fprintf(os.Stderr, "poc16-analyze: acceptance criteria failed: %v\n", err)
		os.Exit(1)
	}
	encoder := json.NewEncoder(os.Stdout)
	encoder.SetIndent("", "  ")
	if err := encoder.Encode(summary); err != nil {
		fmt.Fprintf(os.Stderr, "poc16-analyze: encode summary: %v\n", err)
		os.Exit(1)
	}
}

// analyzeRun summarizes one POC16 run directory containing agent JSONL files and
...
// validateSummary checks the POC16 clean-run event-record gates without mutating
...
func validateSummary(summary RunSummary, criteria AcceptanceCriteria) error {
	...
	if criteria.RequireCASComputeSuperset {
		...
		// Intent: POC16 should prove that the vague generic-report pCID was
	...
	if criteria.RequirePOC15Superset {
		failures = append(failures, poc15SupersetFailures(summary)...)
	}
...
func summarizeMessageArtifacts(runDir string, summary *RunSummary) error {
	...
	defer func() {
		...
		if closeErr != nil {
			fmt.Fprintf(os.Stderr, "poc16-analyze: close %s: %v\n", indexPath, closeErr)
		}
	}()
...
// unique DAG nodes.
// Intent: POC16 should prove the retained CAS/index can be traversed as a DAG,
...
func persistentSessionFailures(summary RunSummary) []string {
	// Intent: POC16 should fail loudly if transport silently regresses to
...
func poc15SupersetFailures(summary RunSummary) []string {
	// Intent: POC16 must preserve the named-agent surface and POC15 functionality
	// lineage rather than silently shrinking the run while adding security/profile
	// pressure. Source: DI-vulit
	var failures []string
	for _, agentName := range []string{"alice", "bob", "carol", "dave", "ellen", "frank", "grace", "heidi", "ivan", "judy", "mallory", "oscar", "fulfillment", "postal_scale", "ups_label_printer", "printer_port", "accounting", "peggy", "victor"} {
		if summary.AgentCounts[agentName] == 0 {
			failures = append(failures, "agent missing from run events: "+agentName)
		}
	}
	if summary.EventCounts["poc15_superset_named_agent_preserved"] < 19 {
		failures = append(failures, fmt.Sprintf("poc15_superset_named_agent_preserved=%d want >=19", summary.EventCounts["poc15_superset_named_agent_preserved"]))
	}
	return failures
}

func poc16ProtocolProfileFailures(summary RunSummary) []string {
	// Intent: Analyzer acceptance should fail if POC16 only keeps inherited POC15
	// behavior and omits the new secure-token, encryption, map-profile, and
	// parser/builder role pressure. Source: DI-vulit
	var failures []string
	for _, protocolName := range []string{pcid.SecureCapabilityV1, pcid.EncryptedPayloadV1, pcid.ParserBuilderRoleV1, pcid.MapPayloadProfileV1} {
		if summary.ProtocolCounts[protocolName] == 0 {
			failures = append(failures, "POC16 protocol profile missing: "+protocolName)
		}
		if summary.MessageArtifactProtocolCounts[protocolName] == 0 {
			failures = append(failures, "POC16 raw message artifact missing: "+protocolName)
		}
	}
	if summary.ScoreReport.SpecContext < 5 {
		failures = append(failures, fmt.Sprintf("score_report.spec_context=%d want 5", summary.ScoreReport.SpecContext))
	}
	if summary.ScoreReport.TokenSecurity < 5 {
		failures = append(failures, fmt.Sprintf("score_report.token_security=%d want 5", summary.ScoreReport.TokenSecurity))
	}
	if summary.ScoreReport.Encryption < 5 {
		failures = append(failures, fmt.Sprintf("score_report.encryption=%d want 5", summary.ScoreReport.Encryption))
	}
	if summary.ScoreReport.ParserBuilder < 5 {
		failures = append(failures, fmt.Sprintf("score_report.parser_builder=%d want 5", summary.ScoreReport.ParserBuilder))
	}
	if summary.ScoreReport.PayloadProfile < 5 {
		failures = append(failures, fmt.Sprintf("score_report.payload_profile=%d want 5", summary.ScoreReport.PayloadProfile))
	}
	return failures
}
...
func forbiddenVocabularyTerms() []string {
	// Intent: Keep active POC16 run output from drifting back to retired
...
func summarizeLog(logPath string, summary *RunSummary) error {
	...
	for scanner.Scan() {
		...
		if isEncryptedPayloadEvent(event.Event) {
			summary.EncryptedPayloadCounts[event.Event]++
		}
		if isParserBuilderEvent(event.Event) {
			summary.ParserBuilderCounts[event.Event]++
		}
...
func finalizePersistentSessionLifecycle(summary *RunSummary) {
	...
	// so shutdown races and leaked streams are visible in POC16 reports. Source:
	// DI-mapop
...
func requiredRegressionEvents() []string {
	return []string{
		...
		// Intent: POC16 must keep DI-fijov recovery caution visible in analyzer
		...
		// Intent: POC16's regression gate now covers run-scoped stores,
		...
		// Intent: POC16 must remain a POC13 superset while adding heterogeneous
		...
		// Intent: POC16 monitoring experiments must be decentralized because
		...
		// Intent: POC16 must test pCID evolution and same-run restart recovery
		...
		// Intent: POC16 should exercise permanent local distrust and
		...
		// Intent: POC16 now has an executable route_v1 slice where Alice,
		...
		// Intent: DI-kohuj moves POC16 route pressure beyond one-shot route
		...
		// Intent: POC16 now requires agent-accessible sparse CAS, local message
		...
		"poc15_superset_named_agent_preserved",
		"poc16_protocol_spec_doc_recorded",
		"llm_spec_context_embedded",
		"llm_spec_context_cid_recorded",
		"poc16_map_payload_specimen_emitted",
		"poc16_map_payload_parsed",
		"poc16_secure_capability_specimen_emitted",
		"cwt_capability_token_issued",
		"cwt_capability_token_verified",
		"cwt_capability_token_wrong_audience_rejected",
		"cwt_capability_token_replay_rejected",
		"cwt_capability_token_expired_rejected",
		"cwt_capability_token_bearer_transfer_promised",
		"cwt_capability_token_holder_bound_checked",
		"cwt_capability_token_transfer_mismatch_rejected",
		"poc16_encrypted_payload_specimen_emitted",
		"poc16_key_discovery_profile_recorded",
		"poc16_key_rotation_context_recorded",
		"encrypted_payload_e2e_promised",
		"encrypted_payload_ciphertext_stored",
		"encrypted_payload_decrypted",
		"encrypted_payload_wrong_recipient_rejected",
		"encrypted_payload_tamper_rejected",
		"encrypted_payload_visible_parent_recorded",
		"encrypted_payload_hidden_parent_recorded",
		"encrypted_payload_relay_non_inspection_promised",
		"encrypted_payload_unsupported_pcid_not_promised",
		"pcid_slot0_selected_parser",
		"pcid_only_dispatch_recorded",
		"pcid_address_separation_recorded",
		"builder_role_payload_built",
...
func computeScores(summary RunSummary) ScoreReport {
	...
	// replay as first-class POC16 fitness dimensions instead of burying them under
	...
	// Intent: POC16 adds runtime-adapter and decentralized-monitoring dimensions
	...
	addScore(&scores.TokenSecurity, summary.EventCounts["capability_token_signed"] > 0 && summary.EventCounts["capability_token_signature_verified"] > 0 && summary.EventCounts["capability_token_expiry_checked"] > 0 && summary.EventCounts["normal_traffic_cose_token_issued"] > 0 && summary.EventCounts["normal_traffic_cose_token_received"] > 0 && summary.EventCounts["capability_token_replay_rejected"] > 0 && summary.EventCounts["cwt_capability_token_issued"] > 0 && summary.EventCounts["cwt_capability_token_verified"] > 0 && summary.EventCounts["cwt_capability_token_replay_rejected"] > 0 && summary.EventCounts["cwt_capability_token_expired_rejected"] > 0 && summary.EventCounts["cwt_capability_token_holder_bound_checked"] > 0 && summary.EventCounts["cwt_capability_token_transfer_mismatch_rejected"] > 0)
	// Intent: POC16-specific score dimensions make the new secure-token,
	// encrypted-payload, pCID parser/builder, map-profile, spec-context, and
	...
	scores.Overall = (scores.Transport + scores.Storage + scores.Compute + scores.Economics + scores.Trust + scores.Verification + scores.Replica + scores.Durability + scores.Retention + scores.Pressure + scores.Replay + scores.RuntimeAdapter + scores.Monitoring + scores.Route + scores.AgentCAS + scores.CASRetrieval + scores.TokenSecurity + scores.Encryption + scores.ParserBuilder + scores.PayloadProfile + scores.SpecContext + scores.POC15Superset + scores.Session + scores.Migration + scores.Restart) / 25
	if len(summary.MissingRequiredEventNames) > 0 {
...
func isEncryptedPayloadEvent(eventName string) bool {
	return strings.HasPrefix(eventName, "encrypted_payload_")
}

func isParserBuilderEvent(eventName string) bool {
	return strings.HasPrefix(eventName, "parser_role_") || strings.HasPrefix(eventName, "builder_role_") || strings.HasPrefix(eventName, "pcid_")
}
