...
// diagnosticValue is the small CBOR value tree used by the POC16 diagnostic
// tool.
...
// POC16, plus enough generic CBOR to make malformed or future artifacts clear.
...
func main() {
	if err := run(os.Args[1:], os.Stdout); err != nil {
		fmt.Fprintf(os.Stderr, "poc16-cbor-diag: %v\n", err)
		os.Exit(1)
	}
}
...
func findMessageDAGRecord(runRoot, hashValue string) (messageDAGRecord, error) {
	...
	defer func() {
		...
		if closeErr != nil {
			fmt.Fprintf(os.Stderr, "poc16-cbor-diag: close %s: %v\n", indexPath, closeErr)
		}
	}()
