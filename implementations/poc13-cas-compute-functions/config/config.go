...
// Config is the repo-local, non-secret runtime configuration for POC13.
...
// Load reads and validates a POC13 config file.
...
// Validate checks that the config is complete enough for a bounded POC13 run.
...
// POC13 runtime uses KernelAppPortForContainer and KernelPeerPortForContainer
// instead so every container has one kernel boundary and local app processes
// register receive promises with that kernel.
// Intent: Keep old test helpers stable while moving production POC13 routing to
...
// Intent: POC13 tests kernel routing by slot-0 pCID to app receive promises;
