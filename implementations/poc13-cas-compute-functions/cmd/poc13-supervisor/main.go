...
func run() error {
	...
	kernelBinary := flag.String("kernel-bin", "poc13-kernel", "POC13 kernel binary path")
	flag.Parse()
	if *containerName == "" {
...
func appBinaryForAgent(cfg config.Config, agentName string) (string, error) {
	...
	switch agent.Kind {
	case "":
		return "poc13-relationship-agent", nil
	case "fulfillment":
		return "poc13-fulfillment", nil
	case "postal_scale":
		return "poc13-postal-scale", nil
	case "ups_label_printer":
		return "poc13-ups-label-printer", nil
	case "printer_port":
		return "poc13-printer-port", nil
	case "accounting":
		return "poc13-accounting", nil
	default:
