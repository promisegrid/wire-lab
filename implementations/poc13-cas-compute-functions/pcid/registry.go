...
// Registry is the POC13 kernel's local pCID table. It is not a central service
...
func NewRegistry() Registry {
	...
	for _, entry := range []struct {
		...
		{RelationshipV1, "poc13 relationship trust discovery observation protocol v1"},
		{PostalScaleV1, "poc13 postal scale package weighing protocol v1"},
		{UPSLabelV1, "poc13 ups label printing cost tracking protocol v1"},
		{AccountingV1, "poc13 accounting address lookup shipment update protocol v1"},
		{PrinterPortV1, "poc13 printer port capability token issue redemption protocol v1"},
		{KernelReceiveV1, "poc13 local app receive promise registration protocol v1"},
...
func (registry Registry) MustCID(name string) protocol.ProtocolCID {
	...
	if !ok {
		panic(fmt.Sprintf("unknown POC13 pCID name %q", name))
	}
	return protocolCID
}
...
// Known reports whether a protocol name is in the POC13 registry.
