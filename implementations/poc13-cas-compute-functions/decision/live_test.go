...
func TestAPIKeyReadsDirectEnvironment(t *testing.T) {
	t.Setenv("POC13_TEST_API_KEY", "direct-test-key")
	client := NewLiveClient("https://example.invalid/v1/responses", "POC13_TEST_API_KEY", "agent", "monitor", "medium", "flex", time.Second)
...
func TestAPIKeyReadsFileEnvironment(t *testing.T) {
	...
	t.Setenv("POC13_TEST_API_KEY_FILE", secretPath)
	client := NewLiveClient("https://example.invalid/v1/responses", "POC13_TEST_API_KEY", "agent", "monitor", "medium", "flex", time.Second)
	apiKey, err := client.apiKey()
	if err != nil {
		t.Fatalf("api key: %v", err)
...
func TestAPIKeyRejectsMissingSources(t *testing.T) {
	client := NewLiveClient("https://example.invalid/v1/responses", "POC13_TEST_MISSING_API_KEY", "agent", "monitor", "medium", "flex", time.Second)
...
func TestLiveClientRequestsStructuredDecisionOutput(t *testing.T) {
	t.Setenv("POC13_TEST_API_KEY", "test-key")
	...
	if formatObject["type"] != "json_schema" || formatObject["name"] != "poc13_promise_decision" {
