...
// FrameConn sends exact envelope bytes over a length-framed stream.
//
// Intent: Keep transport boring so the POC tests promise-message semantics, not
// partial-read stream behavior. Source: DI-horak.
...
func (frameConn FrameConn) WriteFrame(frameBytes []byte) error {
	if len(frameBytes) > 16*1024*1024 {
