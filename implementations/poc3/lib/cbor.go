...
// cborWriter writes only the small deterministic CBOR subset needed by poc3.
// Intent: Keep the proof focused on PromiseGrid envelope behavior rather than a
// general CBOR dependency or a new wire shape. Source: DI-horak.
...
func (writer *cborWriter) writeTypeAndLength(major byte, length uint64) error {
	if major > 7 {
		return fmt.Errorf("invalid cbor major type %d", major)
	}
	prefix := major << 5
	switch {
	case length < 24:
...
// MarshalStringMap encodes a deterministic string map for POC protocol
// payloads. Keys are sorted so signed envelope bytes are stable.
...
// cborReader reads the matching deterministic CBOR subset used by poc3.
...
// UnmarshalStringMap decodes the deterministic string-map payload subset.
