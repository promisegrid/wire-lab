...
const (
	...
	ActionQuote     = "quote_exchange_rate"
)

// ActionContext is the private information one POC8 agent uses when judging an
...
// AgentPolicy is a deterministic local utility model for one POC8 agent.
// Intent: Make the demo reproducible while still requiring every accept,
// counteroffer, redemption, transfer, and quote to clear a local threshold.
// Source: DI-sirus
...
// score applies a deliberately small local formula. Intent: POC8 is testing
// promise-economy shape and incentives, not claiming this is the final
// PromiseGrid economics model. Source: DI-sirus
func (agentPolicy AgentPolicy) score(context ActionContext) int {
	...
	case ActionQuote:
		return context.PeerTrust + context.IssuerTrust - context.Scarcity
	default:
		return -100
	}
}
