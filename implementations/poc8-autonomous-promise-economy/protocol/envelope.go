...
const (
	...
	// interpreted as 0x67726964 / decimal 1735551332. Intent: Make POC8's bytes
...
// Intent: Keep POC8 aligned with the current grid-envelope rule:
...
// view. Intent: Make POC8 sign exact CBOR bytes without making proof placement a
...
// Intent: Replace the first POC8 JSON-as-protocol sketch with a real
...
func ParseEnvelope(envelopeBytes []byte) (Envelope, error) {
	...
	if arrayLength != 3 {
		return Envelope{}, fmt.Errorf("poc8 envelope must have three slots, got %d", arrayLength)
	}
...
func DeterministicPrivateKey(seedText string) ed25519.PrivateKey {
	seed := sha256.Sum256([]byte("poc8 protocol signer: " + seedText))
