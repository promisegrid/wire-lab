...
- [ ] 011.1b Steve answers DF-22.2 (tooling language for freeze and audit). Recommended: 2.b (Go with `github.com/ipfs/go-cid`). Reason: matches Steve's stated language preference (chat 2026-04-29), aligns with the wider PromiseGrid Go ecosystem (`grid-poc`), and uses the canonical CIDv1 reference implementation. The bot's sandbox installs Go on demand via `sudo apt-get install -y golang` (verified 2026-04-29: produces canonical CID for the test vector matching `py-multiformats-cid`).
...
- [ ] 011.4 Steve answers DF-22.5 (freezing trigger). Recommended: 5.d (manual ritual + scheduled CI audit). Under the recommended DF-22.2 (Go), the freeze ritual is `go run ./tools/freeze-spec <slug>` and the audit is `go run ./tools/check-spec-format`.
...
- [ ] 011.8 Implement `tools/freeze-spec/main.go` and `tools/check-spec-format/main.go` per the locked DF answers. Pin the Go module versions in `go.sum`. The freeze ritual's bootstrap step is idempotent: it checks `go version`, runs `sudo apt-get install -y golang` if Go is missing, then proceeds. This matters for ephemeral bot sandboxes that may not have Go preinstalled.
