...
- **[TE-fikoj: Universal `42(pCID)` envelope shape](docs/thought-experiments/TE-fikoj-universal-42-pcid-envelope-shape.md)** — Tests the stronger premise that the outer PromiseGrid envelope may standardize slot 0 itself as `42(pCID)`, then compares fixed three-slot `grid([42(pCID), payload, sig])` versus variable-arity `grid([42(pCID), payload, ...])`. The current locked direction is the variable-arity branch: slot 0 belongs to a tagged CBOR protocol-selector family, tag `42` is the current standard instance, CBOR array length carries arity, and the protocol named by `pCID` defines later outer-slot roles. Decision status is `decided by DI-sisak`. Source: `DI-kafat`; `DI-sisak`.
...
- **[TE-pudiv: App/kernel grid message boundary](docs/thought-experiments/TE-pudiv-app-kernel-grid-message-boundary.md)** — Tests whether the local app/kernel boundary should use the same `grid([42(pCID), payload, ...])` message format as the wire boundary, with local APIs treated as adapters over pCID-selected messages and evidence records. Decision status is `needs DF`. Source: `DI-gumum`.
