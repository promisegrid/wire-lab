...
The full pressure-test of every currently-locked artifact in the wire-lab against these six constraints lives in [TE-dajot: The 100-year goal as a load-bearing design constraint](docs/thought-experiments/TE-dajot-100-year-goal-as-design-constraint.md). Five open questions surfaced from that pressure-test (OQ-100.1 through OQ-100.5) are recorded in §12 below.
...
`protocols/wire-lab.d/archive/migrations/SIM-piloh-turns-149-208-recovery/legacy-shell/supersession-map.md`
and the content-named simulation roots under `simulations/`. Source: `DI-sujan`;
example-location source: `DI-fakin`.
...
- **[TE-famar: Promise-stack ordering](docs/thought-experiments/TE-famar-promise-stack-ordering.md)** — Run two variants — signature outermost vs.
- **[TE-jovoj: Trust-ledger merge after partition](docs/thought-experiments/TE-jovoj-trust-ledger-merge-after-partition.md)** — Two clusters trade independently for simulated days, then reconnect.
- **[TE-pubum: Currency exchange-rate equilibration](docs/thought-experiments/TE-pubum-currency-exchange-rate-equilibration.md)** — Three agents issue tokens.
- **[TE-himug: Sybil under double auction](docs/thought-experiments/TE-himug-sybil-under-double-auction.md)** — A single human spawns 50 identities to manipulate prices.
- **[TE-jikaf: Kernel-as-handler vs. classical kernel](docs/thought-experiments/TE-jikaf-kernel-as-handler-vs-classical-kernel.md)** — Run scenario S-mixed (multiple resources on one node) under K1 and K3.
- **[TE-fijub: Capability-token revocation propagation](docs/thought-experiments/TE-fijub-capability-token-revocation-propagation.md)** — Alice issues tokens that B, C, D delegate among themselves through three more hops.
- **[TE-kuhog: Human-novice onboarding under K4](docs/thought-experiments/TE-kuhog-human-novice-onboarding-under-k4.md)** — Drop a `user-novice` stochastic agent into a K4 (no-kernel) community.
- **[TE-sigan: Generational handoff](docs/thought-experiments/TE-sigan-generational-handoff.md)** — A `community-elder` retires after 6 simulated months.
- **[TE-morid: Two communities, two pCIDs, same intent](docs/thought-experiments/TE-morid-two-communities-two-pcids-same-intent.md)** — Community A's spec for compute-call hashes to pCID `0xAAAA…`.
- **[TE-mokut: Slow-mover survival](docs/thought-experiments/TE-mokut-slow-mover-survival.md)** — Ninety percent of the network has upgraded to `protocol-v2`; ten percent are stuck on `protocol-v1` (microcontrollers, sealed appliances, sentimentally maintained legacy).
- **[TE-rotim: Ostrom's principles audit](docs/thought-experiments/TE-rotim-ostroms-principles-audit.md)** — Take a winning C-scenario (cooperators dominate at month 12) and a losing one.
- **[TE-muvuv: Promise-stack as zero-knowledge envelope](docs/thought-experiments/TE-muvuv-promise-stack-as-zero-knowledge-envelope.md)** — Can a promise frame contain a ZK proof that "the inner payload satisfies predicate P" without revealing the payload?
- **[TE-robub: Time-traveling break-witness](docs/thought-experiments/TE-robub-time-traveling-break-witness.md)** — Charlie discovers in year 5 that an event in year 2 was actually fraudulent.
- **[TE-botom: A harness-spec change walks through the unified flow](docs/thought-experiments/TE-botom-harness-spec-change-walks-through-unified-flow.md)** — Stress-test of the single-flow design when the proposal targets the harness spec itself. Walks through end-to-end with a stochastic elder, two LLM analysts, an adversary, and Steve as deciding signer. Findings: the design holds; surfaces wobbles around proposal-collector matching, pointer storage, and settlement-window conventions.
- **[TE-dodaf: Should this design become `promisegrid/promisegrid/README.md`?](docs/thought-experiments/TE-dodaf-should-this-design-become-promisegrid-readme.md)** — Compares the existing org-level README against the Wire Lab design, identifies stale and missing material, and recommends a four-phase migration: separate Wire Lab repo now, minor org-README touchup later, substantive rewrite once findings exist, full graduation eventually.
- **[TE-hogus: Group-transport envelope: `grid <pcid>` carrier, canonical bytes, and explicit promise body](docs/thought-experiments/TE-hogus-group-transport-envelope.md)** — For the wire-lab's first transport-protocol (the group-transport-protocol class: small-finite-closed-group, N≥2, all-see-all, multi-writer DAG of messages), compares `grid <pcid>`, ordinary protocol headers, filename/path-selected protocols, and a pure structured object. Current finding: use `grid <pcid>` so the repo can test pCID-selected dispatch, message-CID-linked references, and explicit promise bodies without prematurely freezing the canonical PromiseGrid wire format. The substantive v0 contract — `Parents:` header for DAG links, body-level acknowledgement for receipts, no `Kind:` header, flat subdirectory layout — now lives as a simulation-local specimen at `simulations/SIM-rakot-group-session/protocols/group-session.d/specs/group-session-draft.md`. Source: `DI-009-20260430-204108`; Mupoz relocation source: `DI-fakin`; open graduation question: `DR-009-20260430-204108`. Numbering note: this TE was originally drafted as TE-nibar on `stevegt/channels-grid-pcid` and used "channel" vocabulary; per TE-titur (numbering reconciliation, drafting-time invariant) it is TE-hogus on `ppx/main`, and per TE-junil + TODO-motof it has been renamed and rewritten in place under transport vocabulary.
- **[TE-titur: TE-nibar numbering collision and harness-spec path](docs/thought-experiments/TE-titur-te-numbering-collision-and-harness-spec-path.md)** — Reconciles a numbering collision when two branches independently drafted a "TE-nibar." Locks the drafting-time invariant: integers anchor on the first-drafted timestamp; later-drafted TEs renumber when branches collide. Same rule applies to TODO numbers. Also reaffirms `specs/harness-spec-draft.md` as the canonical path for the harness spec (post-TE-rujak freeze convention).
- **[TE-zalut: Transport-protocol types, pCID-keyed transport paths, and DAG message graphs](docs/thought-experiments/TE-zalut-channel-transport-types-and-threaded-replies.md)** — Surveys nine transport patterns (1:1, broadcast, subgroup multicast, pub/sub, anycast, request-reply, broadcast-with-receipts, single-writer log, general DAG) and concludes that transport type does not drive on-disk path layout. Locks four principles for the outer wire-lab transport-spec: a message does not declare its transport (no `Transport:` header); transport directories are keyed `transports/<pcid>--<slug>/` where the pCID is canonical protocol identity; each transport-protocol-pCID names a spec defining its directory's interior; the code that reads the directory structure is the handler for that pCID. The extracted feed-outer specimen now lives in `simulations/SIM-labit-feed-outer/protocols/feed-outer.d/specs/feed-outer-draft.md`, and TE-hogus's v0 contract remains in the separate group-session specimen (`simulations/SIM-rakot-group-session/protocols/group-session.d/specs/group-session-draft.md`). The conceptual shift toward DAG-shaped message graphs (zero-or-more parents per message) is surfaced; concrete header shapes are delegated to per-transport-protocol specs. (Originally drafted using "channel"; rewritten in place per TE-junil. Filename retains "channel" from original drafting.)
- **[TE-junil: Transports rename and axes of transport-protocol differentiation](docs/thought-experiments/TE-junil-transports-rename-and-axes-of-differentiation.md)** — Renames `channels/` to `transports/` throughout the repo (the directory is the wire lab's simulation surface for the network being studied; "channel" suggested an above-transport logical-addressing concept that wire-lab does not need yet). Establishes per-axis meta-rule for distinguishing transport-protocols: visibility (B) and routing topology (C) warrant distinct pCIDs; cardinality (A) is a parameter except at extremes (large-N, unbounded); membership (D), persistence (E), message-graph shape (F), direction (G), and receipts (H) are parameters within a single transport-protocol's spec. Locks the carve-out plan: one starter spec, now a simulation-local specimen at `simulations/SIM-rakot-group-session/protocols/group-session.d/specs/group-session-draft.md`, covering small-finite-closed-group with N≥2; the codex-perplexity case is the N=2 instance, not a separate spec. Anticipates future transport-protocol specs (ring, star, cluster-of-clusters, gossip) and a future TE on transport-protocol migration semantics.
- **[TE-dajot: The 100-year goal as a load-bearing design constraint](docs/thought-experiments/TE-dajot-100-year-goal-as-design-constraint.md)** — Names the **100-year goal** that has been operating implicitly throughout the wire-lab's design and articulates its six load-bearing constraints (C-1 no central registry; C-2 multi-generational durability; C-3 adversarial-by-default; C-4 protocol forking is normal; C-5 trust accrues per-burden; C-6 signing key is the only structural lock). Pressure-tests every currently-locked artifact (harness-spec, transport-spec, group-transport-spec, TE-hogus/25/26/27, branching policy, TE/TODO numbering) against each constraint. Finds that no current lock fails outright, but several locks are silent on C-4 (protocol forking) and on cryptographic-signing migration discipline, surfacing five open questions OQ-100.1 through OQ-100.5 recorded in §12. Recommends the constraint codes C-1 through C-6 be used as shared shorthand in subsequent TEs so each TE does not re-derive the constraint set.

- **[TE-vipir: Protocols as simulated repos, and the L4-binding layer](docs/thought-experiments/TE-vipir-protocols-as-simulated-repos-and-binding-layer.md)** — Locks the directory shape under which each protocol becomes its own simulated repo (`protocols/<slug>.d/` for live, `protocols/<slug>-<pcid>.{md,d/}` sibling pair for frozen), absorbs the former top-level `DR/`/`TODO/`/`DI/` directories as inline sections inside the relevant protocol's spec, and decomposes `transports/` into a five-level path `transports/<real-world-transport>/<L4-binding-pCID>/<session-pCID>/<message-pCID>/<message-id>.txt` aligned with the existing-Internet reality that PromiseGrid rides L4+ transports rather than re-specifying them. Defines the **L4-binding layer**: small specs (e.g. UDP-binding v0) that translate between opaque PromiseGrid session messages and the framing/sizing/addressing/connection conventions of one specific real-world transport. Renames `group-transport` to `group-session` (DFs T1-T6 unchanged; session-layer semantics, not L4 framing). Treats `proposals/` content as instances of a `ppx-dr` message protocol that should live under `transports/...`. TE-mupoz later preserves the repo-like insight but moves non-wire-lab candidate protocol trees and legacy proposal records into content-named simulation and rooted archive artifacts. Companion artifact: UDP-binding v0 now lives at `simulations/SIM-ludaf-udp-feed/protocols/udp-feed.d/specs/udp-binding-draft.md`. Source: `DI-fakin`.

- **[TE-sihih: Substrate-agnostic layered model (L5/L6/L7) and L6 CAS subtree](docs/thought-experiments/TE-sihih-substrate-agnostic-layered-model.md)** — Refines TE-vipir without superseding it: path-level-2 vocabulary is now **feed** rather than binding, path-level-3 vocabulary is now **group** rather than forum, and every future message file in `transports/` is a CBOR pointer into an L6 CAS subtree `cas/<cas-protocol-pCID>/<chunk-cid>` with no small-message inline exception. Locks L5 feeds, L6 CAS, and L7 groups as the substrate-agnostic model that answers the 100-year goal; today's inline `wire-lab-devs-draft` files are now simulation-local pre-CAS draft state at `simulations/SIM-ludut-wire-lab-devs/world/transports/wire-lab-devs-draft/` until the first CAS spec and migration land. Source: `DI-fakin`.

- **[TE-nijab: Transport layering and freeze boundaries](docs/thought-experiments/TE-nijab-transport-layering-and-freeze-boundaries.md)** — Tests the conflict between older `transports/<pcid>--<slug>/` language and the later TE-vipir / TE-sihih layered model. The central question is whether `transports/` is a namespace of frozen higher-layer protocol instances, or lower-layer network/feed simulation data that can carry many higher-layer protocols like TCP carrying HTTP, SSH, and SMTP. Decides that transport data is lower-layer network/feed simulation data; TE-mupoz later moves `wire-lab-devs-draft` into simulation-local world transport evidence (current location: `simulations/SIM-ludut-wire-lab-devs/world/transports/wire-lab-devs-draft/`) with CID-preserving evidence. Source: `DI-fakin`.

- **[TE-domat: `transports/` / `groups/` reconciliation after TE-vipir, TE-sihih, and turn 176](docs/thought-experiments/TE-domat-transports-groups-reconciliation.md)** — Tests the missing recovery question behind DR-nugog: whether Steve's turn-176 `groups` choice should land as a literal `transports/` -> `groups/` rename, remain vocabulary-only, or split responsibilities so `groups/` becomes the L7 group registry/view while `transports/` remains lower-layer feed/transport specimen data per TE-nijab. TE-mupoz later chooses simulation-local migration for the current specimen instead of preserving root `transports/` as active layout; future group/feed/CAS/root layout still graduates through DR/DI/spec work.

- **[TE-pahah: Wire-lab simulation-first structure for design decisions](docs/thought-experiments/TE-pahah-wire-lab-simulation-first-structure.md)** — Reframes the `transports/` / `groups/` debate under Steve's 2026-05-09 premise that wire-lab is still brainstorming and not compatibility-bound. Tests which structure best serves simulation and decision-making, and recommends a top-level `simulations/` tree with one scoped world per experiment (`world/sites`, `world/groups`, `world/cas`, `world/feeds`, `world/wires`) rather than treating root-level `transports/` or `groups/` as the primary experiment home; decision status is `needs DF`.

- **[TE-gurov: Promise-shaped coordination artifacts](docs/thought-experiments/TE-gurov-promise-shaped-artifacts.md)** — Tests whether durable artifacts such as docs, TEs, DRs, DIs, TODOs, specs, reviews, and guide material should be expressed as PT-style promises with explicit author/promiser/LLM provenance and an `I promise` body opening. Rejects strict whole-body promise form for analysis, question, and work-queue artifacts; keeps promise wrappers, explicit promise blocks, and specs/adoption-only scope as surviving alternatives; decision status is `needs DF`.

- **[TE-vilot: Promise-shaped artifacts inside simulation-first wire-lab](docs/thought-experiments/TE-vilot-promise-shaped-simulation-artifacts.md)** — Reconsiders TE-pahah and TE-gurov together. Finds that `simulations/` gives the missing boundary for testing promise-shaped artifact protocols: apparatus documents such as TEs, DRs, TODOs, and guide-resource notes should stay role-specific with explicit promise blocks only when genuine promises appear, while named simulations can test stronger `I promise` wrappers and identity/provenance fields as specimens before any repo-wide template is locked; decision status is `needs DF`.

- **[TE-hirap: Artifacts as PromiseGrid messages](docs/thought-experiments/TE-hirap-artifacts-as-promisegrid-messages.md)** — Extends TE-pahah and TE-vilot from promise prose to full PromiseGrid-message shape. Compares global CBOR artifacts, global plain-text `grid <pcid>` artifacts, dual canonical representations, simulation-scoped artifact-message protocols, and commitment/specimen-only adoption. Recommends testing message-shaped artifacts inside named simulations, using plain text first for human/LLM-readable specimens, CBOR as a structured production-wire candidate, and one canonical representation per protocol to avoid CID split; decision status is `needs DF`.

- **[TE-nizor: Can implementing Pahah satisfy the recovered concerns?](docs/thought-experiments/TE-nizor-pahah-implementation-sufficiency.md)** — Reconsiders TE-pahah, TE-vilot, TE-hirap, and turns 149-208 as one critical decision. Finds that Pahah can satisfy the recovered concerns only if implemented as `simulations/` plus a minimal simulation contract (`QUESTION`, concerns matrix, protocol set, scoped world, events, observations, results, decision handoff), not as a bare directory or a root-level `groups/`/`transports` migration. Recommends Pahah-with-contract as the immediate apparatus umbrella while leaving envelope, CBOR/text, CAS/chunking, promise economy, and artifact-shape decisions to simulations and later DR/DI/spec work; decision status is `needs DF`.

- **[TE-mupoz: Root and protocol migration scope under simulations](docs/thought-experiments/TE-mupoz-root-protocol-migration-scope-under-simulations.md)** — Tests how much existing root-level and `protocols/` content should move if TE-nizor's Pahah-with-contract recommendation is implemented. Locks `DF-mupoz.3` via `DI-pakid` and the remaining DFs plus implementation path via `DI-fakin`: root `protocols/` contains only `wire-lab.d`; candidate protocol trees initially moved under `SIM-piloh` and were later split into content-named lineages (`SIM-rakot-group-session`, `SIM-ludaf-udp-feed`, retired `ppx-dr` archive); concrete `wire-lab-devs-draft` evidence moved into simulation world transport evidence; legacy proposal records moved into archive/provenance; simulation results graduate only through DR/DI/spec/dev-guide handoff. Decision status is `decided`.

- **[TE-david: PromiseGrid dev-guide resources from wire-lab evidence](docs/thought-experiments/TE-david-promisegrid-dev-guide-resources.md)** — Tests what humans and LLMs need from wire-lab to write the PromiseGrid Development Guide without mistaking wire-lab for the guide or for the final PromiseGrid source of truth. Locks top-level `DEV-GUIDE-RESOURCES.md` as a PromiseGrid-facing source map plus writer notes, with stable guide prose later superseding exploratory wire-lab notes except where the guide cites frozen protocol specs by pCID. Source: `DI-nunut`.

- **[TE-dojab: Simulation run model and scenario/result matrix](docs/thought-experiments/TE-dojab-simulation-run-model-and-scenario-result-matrix.md)** — Locks root `scenarios/`, root `results/<sim-id>/<scenario-id>/<model-id>/<YYYYMMDD-HHMMSS>.md`, Codex-only first runs, one result file per model/timestamp run, and evidence-only result authority as the first simulation/scenario comparison apparatus. Root scenarios and root results are harness comparison apparatus; results graduate through DR/DI/spec/guide handoff rather than direct design authority. Result paths use specific provider/model/reasoning slugs and UTC timestamps per `DI-miror`; the full DF-dojab closure is locked by `DI-faros`. Decision status is `decided, refined`.

- **[TE-magup: TODO numbering and per-protocol TODO shape](docs/thought-experiments/TE-magup-todo-numbering-and-per-protocol-shape.md)** — Closes out the TODO-numbering question that TE-vipir had named but not resolved. Locks Option D from the conversation of 2026-05-01: per-protocol `TODO/` subtrees inside each `protocols/<slug>.d/`, **no top-level `TODO/` directory**, timestamp-named TODO files matching the TE convention `TODO-YYYYMMDD-HHMMSS-<slug>.md`. The harness is itself a protocol (`protocols/wire-lab.d/`); harness TODOs live at `wire-lab.d/TODO/` like every other protocol's TODOs live in their own. Each protocol's local `TODO.md` lists only that protocol's TODOs; the master cross-listed `TODO.md` at `wire-lab.d/TODO/TODO.md` lists everything across the wire-lab. Boundary rule: a TODO is per-protocol if and only if its work touches files only inside one `protocols/<slug>.d/` tree; anything else is harness-level. **Renumbers all 19 existing TODOs**: 16 are harness-level and migrate to `wire-lab.d/TODO/`; 3 are per-protocol (012 -> group-session, 016 -> ppx-dr, 018 -> udp-binding). RETIRED (015) and FOLDED (017) status records migrate as ordinary timestamped files; no integer survivors. Canonical first-drafted timestamps are recovered from git commit history, never set to migration-time. Migration is delegated to TODO-vuhuj as new subtasks 10 and 11 so it happens atomically with the rest of the protocols-migration. Surfaces five sub-OQs OQ-30.1 through OQ-30.5 covering boundary changes, sync discipline, DR/DI parallelism, harness-shape sub-categories, and `proposals/` cleanup. Partially closes OQ-100.4 (TODO half resolved by per-protocol timestamping; TE half still uses integer aliases per TE-titur).

- **[TE-zukug: Spec-doc as upstream, simrepo as implementation: inverting the conformance reference](docs/thought-experiments/TE-zukug-spec-doc-inversion-and-conformance-changelog.md)** — Closes OQ-29.1 (freeze ceremony) by inverting the doc/repo reference direction. Old mental model: spec doc embeds a tree-hash that names the simrepo. New mental model: each `protocols/<slug>.d/` simrepo carries a `CHANGELOG.md` whose entries each name a spec-doc CID and a conformance verb (`implements`, `partially-implements`, `extends`, `deprecates`); the spec doc itself stays upstream and oblivious, RFC-shaped. Locks **Alt-G**: the spec is frozen when its doc-CID is published and at least one simrepo CHANGELOG entry references it; no tree hash, no bundle. Supersedes Alt-A through Alt-F enumerated in TE-vipir. Justified by five arguments: (1) matches RFC reference direction; (2) one doc-CID supports many implementations without republishing; (3) implementations evolve their conformance claim cheaply; (4) companion files travel with the doc automatically because the doc cites them by pCID (per the user's 2.C answer that machine-checkable bits are hybrid inline + companion-pCID); (5) keeps the spec doc as a universally-quantified promise per TE-nibar. Sketches the CHANGELOG header-block format, harness conformance-check loop, breaking-change migration shape, and self-consistency with PromiseGrid's promise/acceptance primitive (the doc-CID is a promise; the CHANGELOG entry is an acceptance). Surfaces five follow-on questions OQ-31.1 through OQ-31.5 covering harness self-application, CHANGELOG location/format, breaking-change flagging, CHANGELOG-entry pCIDs, and an optional reverse-index in the spec-doc store. Implementation impact deferred to a forthcoming TODO-dinub (CHANGELOG.md format and harness parser); TODO-vuhuj unaffected.

- **[TE-liviv: Spec-side vs implementation-side split, and the `implementations/` top-level](docs/thought-experiments/TE-liviv-spec-vs-implementation-split.md)** — Amends TE-zukug (does not retract). TE-zukug conflated two distinct categories of file: **A-side** (spec/design WIP — TEs, draft specs, design TODOs, manifests; what `protocols/<slug>.d/` was always for per TE-vipir) and **B-side** (conformance/implementation artifacts — code, test vectors, harness fixtures). The CHANGELOG language in TE-zukug was always about the B-side. Locks the split: A-side stays under `protocols/<slug>.d/`; B-side moves to a new top-level `implementations/<impl-name>/` that does not exist yet but is created by TODO-vuhuj. Each side has its own `CHANGELOG.md` with different semantics: A-side records **freeze events** (`event: freeze` with a doc-CID) authored by spec maintainers; B-side records **conformance claims** (`claim: implements/partially-implements/extends/deprecates` with an upstream doc-CID) authored by implementers. Neither names the other directly; discovery is a query over known CHANGELOG sets in both directions. Reasons for top-level `implementations/` (not a subtree of `protocols/<slug>.d/`): (1) one impl may implement many protocols (a Go reference shipping UDP-binding+group-session+ppx-dr is one tree); (2) external implementations live in their own external repos with the same shape, location-independent; (3) spec and impl fork independently per C-4. Refines Alt-G slightly: a spec is frozen when its A-side CHANGELOG records a `freeze` event with a doc-CID (one-sided act, controlled by spec maintainers; does not require an implementation to exist, matching how RFC 768 was frozen on publication not on first BSD release). Resolves TE-zukug OQ-31.3 (yes, explicit `breaking-change` boolean on B-side entries). Refines TE-zukug OQ-31.2 (per-side CHANGELOG location specified; YAML-vs-fenced-vs-HTML-comment for the header block remains open). Refines TE-zukug OQ-31.5 (bidirectional reverse indices; neither embedded in docs). Surfaces five sub-OQs OQ-32.1 through OQ-32.5 (external-implementation discovery; impl-of-multiple-protocols mapping; test-vector A-vs-B classification; harness-as-protocol vs harness-as-implementation per TE-magup; frozen-sibling tree size). Adds steps 12-15 to TODO-vuhuj to keep `protocols/<slug>.d/` design-only, create `implementations/` top-level with stub README, retarget TODO-jodon and TODO-bihon to live under `implementations/`, and seed empty CHANGELOG.md stubs in each migrated `protocols/<slug>.d/`.

- **[TE-vudaf: Tabletop simulation of the TE editing policy](docs/thought-experiments/TE-vudaf-editing-policy-tabletop.md)** — Stress-tests the policy locked in TE-dabol / DI-020-20260502-213103/-213104/-213105 under six concrete scenarios with named actors (Alice, Bob, Carol, Dave, Ellen, Frank, Mallory). Filed because TE-dabol's own scenario analysis was one-paragraph judgments rather than tabletop play; Steve flagged this on 2026-05-02. Locked policy substantially survives: categorical scheme (Cat-1 / Cat-2 / Cat-3 / Cat-5) holds in all six scenarios. **Findings**: (1) Cat-1 must split into Cat-1a (current-pointer paths, swept) and Cat-1b (historical-quotation paths, left); a naive regex sweep would distort historical claims in TE-magup / TE-liviv / TE-potar; this rises to a superseding DI candidate (DF-35.1). (2) Cat-2 vocabulary updates need a strengthened top-of-file note that enumerates by ID the DIs the rewrite asserts unchanged; defeats Mallory's bad-faith semantic-shift attack (S5); Cat-3 Refinement on TE-dabol (DF-35.2). (3) Cat-2 sweeps must include a cross-TE quoting check; cheap with `grep`; Cat-3 Refinement on TE-dabol (DF-35.3). (4) `Decision status` line should relocate from bottom-of-file to a top-of-file header field; a reader who acts on a superseded TE's body may never see the supersedence marker; Cat-1a sweep, files new TODO subtask 020.10 (DF-35.4). DIs from TE-dabol remain in force as written; DF-35.1 may produce a superseding DI for one of them after Steve's choice.

- **[TE-dabol: TE editing policy and the TE corpus as one document with facets](docs/thought-experiments/TE-dabol-te-editing-policy-and-holistic-corpus.md)** — Retires the (overgeneralized) bot rule "do not back-edit TEs" and replaces it with a per-category policy. **Three locks** (pending DF). (1) TE filenames remain immutable per TE-titur; TE contents are edited under three regimes by category — *mechanical* (path renames, vocabulary updates) edit in place with a top-of-file vocabulary note for vocabulary edits per the TE-hogus / TE-zalut precedent; *navigational* (forward-pointers to later TEs / DRs / DIs that resolved a future-work bullet, or back-pointers added after the fact) go in an append-only `## Refinements` section at the bottom of the TE; *substantive* (recasting alternative analysis, reversing a locked decision, invalidating an assumption) are not edits at all — write a new TE that supersedes the old one and update only the old TE's `Decision status` line. (2) Per-protocol simrepos under `protocols/<slug>.d/docs/thought-experiments/` set their own editing policy in a protocol-specific equivalent of this TE, matching the standing "let each protocol name its own internals" rule (TE-lozip DI-23.2 Alt-2.B). (3) The bot reads the corpus *holistically by default* — when any TE is in scope, the first move is to scan TE titles + `Decision under test` sections across the corpus to find facets that share assumptions, vocabulary, or decisions. Single-TE reading is reserved for obviously mechanical questions. The previous bot claim that TE files were immutable historical record was an overgeneralization from the contest-artifact rule (DI-003), which applies to adversarial review records; TEs are the author's own thinking and the rationale does not transfer. The just-completed TODO-vuhuj migration intentionally did not update paths inside TE files because of the now-retired rule; a follow-on sweep is filed alongside this TE.

- **[TE-potar: Spec-doc Informative References to its workshop, RFC-shaped](docs/thought-experiments/TE-potar-spec-doc-informative-references.md)** — Refines TE-zukug and TE-liviv with the spec-side half of the bidirectional spec<->workshop trail. The bot's first answer in conversation ("spec docs MUST NOT reference workshops") was wrong; Steve pushed back and the IETF precedent was investigated. RFCs in fact have a well-developed practice for spec-to-workshop references: split bibliography into Normative and Informative subsections (RFC 7322); standards-track RFCs MAY NOT have normative references to non-standards-track or lower-maturity documents but MAY have informative references to them (RFC 3967); RFCs MAY cite Internet-Drafts informatively with version pinned and marked "Work in Progress" (draft-carpenter-rfc-citation-recs). Adopts the same convention adapted to PromiseGrid's content-addressed world: PromiseGrid is in fact stricter than the IETF because Internet-Drafts are version-flaky while a tree-hash or pCID is forever-stable. **Three locks**: (1) Two-section bibliography RFC-shaped — if a spec doc has a References section it MUST split into Normative and Informative subsections in that order; (2) Normative references are tightly constrained — must be a spec doc-CID, a registry-backed external standard (RFC/ISO/IEEE), or a frozen `protocols/<slug>-<pcid>.md` doc-CID; never a workshop tree-hash, branch ref, commit ID, draft path, or moveable URL; (3) Informative references must be content-addressed — may include git tree-hashes (git tree-hashes ARE content addresses), TE pCIDs, frozen workshop tree pCIDs, or long-lived URLs on stable domains (`rfc-editor.org`, `ietf.org`, `w3.org`, `iana.org`); never branch refs, bare commit IDs, ephemeral hosts, or non-content-addressed mailing-list URLs (PromiseGrid's C-2 multi-generational durability constraint precludes the latter even though the IETF tolerates them). Establishes a clean bidirectional trail between spec and workshop: workshop->spec via A-side CHANGELOG `freeze` entries (machine-checkable, authoritative); spec->workshop via Informative References (human-readable, advisory). Neither direction is load-bearing for B-side conformance, which still references upstream spec doc-CIDs only per TE-zukug. Includes a worked example of a future UDP-binding v1 spec showing both reference types. Surfaces five sub-OQs OQ-33.1 through OQ-33.5 (Acknowledgments-section convention; SHA-1-vs-SHA-256 for tree-hashes during transition; mailing-list-equivalent for PromiseGrid via ppx-dr; cross-repo informative references; frozen Informative References when the world prunes referenced content).

- **[TE-puzud: Codex local author identity](docs/thought-experiments/TE-puzud-codex-local-author-identity.md)** — Decides that local Codex work in a human-operated clone should use the actual configured local git author identity rather than impersonating Steve or inventing a separate Codex-only principal. Keeps the existing `ppx` bot identity rules unchanged.
...
- **[TE-titaj: pCID slot-0 bootstrap across decades](docs/thought-experiments/TE-titaj-pcid-slot0-bootstrap-across-decades.md)** — Historical predecessor to `TE-tilir`. It framed the receiver-bootstrap problem correctly, kept `pCID` as the semantic role, and rejected protocol-spec self-bootstrap as circular, but it is now superseded for current slot-0 weighting and recommendation. Decision status is `superseded by TE-tilir / DI-lasah`. Source: `DI-pikos`; `DI-lasah`.
- **[TE-tilir: Slot-0 IPLD interop and bootstrap supersedence](docs/thought-experiments/TE-tilir-slot0-ipld-interop-and-bootstrap-supersedence.md)** — Historical predecessor to `TE-fikoj`. It added ATProto / Bluesky ecosystem-interop pressure and the small-byte-delta packet, then recommended semantic `pCID` plus strongly preferred `42(pCID)` in DAG-CBOR / IPLD profiles. It is now superseded for current slot-0 framing by the stronger universal-`42(pCID)` premise. Decision status is `superseded by TE-fikoj / DI-kafat`. Source: `DI-lasah`; `DI-kafat`.
- **[TE-fikoj: Universal `42(pCID)` envelope shape](docs/thought-experiments/TE-fikoj-universal-42-pcid-envelope-shape.md)** — Tests the stronger premise that the outer PromiseGrid envelope may standardize slot 0 itself as `42(pCID)`, then compares fixed three-slot `grid([42(pCID), payload, sig])` versus variable-arity `grid([42(pCID), payload, ...])`. Historical lock for universal slot 0 as `42(pCID)`, tag `42` as the current standard instance, and CBOR array length as the arity mechanism. Superseded by `TE-lamun` / `DI-rojij` for the hard slot-1 payload rule. Source: `DI-kafat`; `DI-sisak`; `DI-rojij`.
- **[TE-lamun: pCID-defined slot vector](docs/thought-experiments/TE-lamun-pcid-defined-slot-vector.md)** — Refines `TE-fikoj` by changing the formal envelope rule from hard `grid([42(pCID), payload, ...])` to `grid([42(pCID), ...protocol-defined-slots])`, while keeping `grid([42(pCID), payload, ...])` as the recommended example profile after `DI-punam` narrowed the earlier "default" wording. Slot 0 remains `42(pCID)`; the protocol named by `pCID` defines slots `1..N`. Decision status is `decided by DI-rojij`, refined by `DI-punam`. Source: `DI-rojij`; `DI-punam`.
- **[TE-mazop: Kernel promise boundary and minimum port contract](docs/thought-experiments/TE-mazop-kernel-promise-boundary-and-minimum-port-contract.md)** — Tests the app/kernel/host promise surface and minimum credible first-port contract that `TE-jimar` left unresolved. It keeps `DR-davod` open and routes the next evidence path through `SIM-fovip`, focused on app-facing promises, host assumptions, unsupported features, pCID coverage, evidence records, and broken-promise handling. Decision status is `needs DF`. Source: `DI-funaf`.
- **[TE-pudiv: App/kernel grid message boundary](docs/thought-experiments/TE-pudiv-app-kernel-grid-message-boundary.md)** — Tests whether the local app/kernel boundary should use the same pCID-selected grid message format as the wire boundary, with local APIs treated as adapters over pCID-selected messages and evidence records. Under `DI-rojij`, that formal message family is now `grid([42(pCID), ...protocol-defined-slots])`, with `grid([42(pCID), payload, ...])` as the recommended example profile after `DI-punam`. Decision status is `needs DF`. Source: `DI-gumum`; `DI-rojij`; `DI-punam`.
- **[TE-dunas: Prior-art influence on the PromiseGrid kernel](docs/thought-experiments/TE-dunas-prior-art-influence-on-promisegrid-kernel.md)** — Tests how much lower-patent-risk distributed-OS prior art should influence PromiseGrid kernel work. It keeps V as same-message simulation pressure, Amoeba as service/capability-token pressure, Plan 9 / 9P as small-protocol and local-view pressure, and GNU Hurd translators as replaceable/deferred-service pressure, while excluding Spring and modern multikernel details from current influence. Decision status is `needs DF`. Source: `DI-somok`.
- **[TE-gakoh: Local views over a promise/event hypergraph](docs/thought-experiments/TE-gakoh-local-views-over-promise-event-hypergraph.md)** — Tests the post-TE-dunas synthesis that PromiseGrid can be explained as a decentralized mainframe: each agent gets a local, trust-filtered, file-like view over a shared promise/event hypergraph; trusted groups may voluntarily maintain shared namespaces through reciprocal promises; portable references are CID-rooted and promise-bound; and file-like resources can be checkpoints or projections over promise/event logs. Decision status is `needs DF`. Source: `DI-kuvum`.
- **[TE-ritig: pCID cardinality and parser/builder kernel roles](docs/thought-experiments/TE-ritig-pcid-cardinality-parser-builder-kernel-roles.md)** — Tests whether production-shaped PromiseGrid should use many pCIDs, moderate protocol-family pCIDs, very few broad pCIDs, or layered pCIDs, and whether slot 0 should select pCID-specific parser/builder kernel roles rather than a monolithic kernel parser or universal routing record. Decision status is `needs DF`. Source: `DI-nogij`.
- **[TE-vahoj: POC18 superset architecture](docs/thought-experiments/TE-vahoj-poc18-superset-architecture.md)** — Extends the POC18 Git/native architecture work with the full session-log requirement set: sparse shared CAS, active background chunk pulling, promise-shaped retrieval and capability-token economics, UI/backend library boundaries, in-band collaboration surfaces, DevOps ordered replay from `implementations/poc18-cas-git-replacement/docs/turing-equiv.html`, and first-slice sequencing. Decision status is `needs DF`. Source: `DI-fusir`.

- **[TE-hikar: POC18 grid CLI command model](docs/thought-experiments/TE-hikar-poc18-grid-cli-command-model.md)** — Completes the POC18 command-language DF left open by TE-vahoj. Chooses a balanced `grid` CLI that avoids Git's staging and native push/pull assumptions while keeping familiar workflow words where they match PromiseGrid objects: `init`, `ingest`, `snapshot`, `checkout`, `refs`, and `diag` first, with future `sync`, `change`, `review`, `merge`, and explicit `grid git ...` bridge groups after the underlying objects exist. Locks `CLI/core` terminology and keeps `poc-*` commands as non-production deterministic fixtures.
- **[DN-lujad: PromiseGrid kernel role/profile design](docs/research/DN-lujad-promisegrid-kernel-role-profile.md)** — Plain-English synthesis for the open `DR-davod` packet. It does not freeze a kernel API; it routes guide writers and focused `SIM-fovip` evidence toward role/profile promises, pCID-selected app/kernel messages where supported, local trust judgments, voluntary namespaces, CID-rooted promise-bound references, and checkpointed resource views. Source: `DI-fidot`.
...
Proposals — whether targeting in-sim pCIDs or the harness spec itself — originally landed in a single review queue. That legacy root `proposals/` queue has now been archived under `protocols/wire-lab.d/archive/migrations/SIM-piloh-turns-149-208-recovery/archive/proposals/`, and current guide-writer coordination runs through `DEV-GUIDE-RESOURCES.md` plus the PromiseGrid dev-guide feedback process. The shape that matters is still the workflow, not the filesystem. Source: `DI-fakin`.
...
17. **When does Phase 2 of the README migration trigger (from TE-dodaf)?** Phase 2 is the additive "two paragraphs pointing at the Wire Lab" update to the canonical PromiseGrid README. What's the right trigger? My instinct: the first thought experiment whose findings produce a concrete recommendation worth quoting publicly — not earlier (premature) and not after several findings (the README falls behind). Probably TE-jovoj or TE-pubum will be the trigger.
...
25. **OQ-29.1 — Freeze ceremony (from TE-vipir). CLOSED by TE-zukug with Alt-G; refined by TE-liviv.** The spec is frozen when its A-side `protocols/<slug>.d/CHANGELOG.md` records a `freeze` event with a doc-CID. No tree hash, no bundle. Supersedes Alt-A through Alt-F. The freeze is a one-sided act controlled by spec maintainers (does not require any implementation to exist). B-side `implementations/<impl-name>/CHANGELOG.md` entries reference the doc-CID upstream when claiming conformance (RFC-shape inversion: implementation-to-spec, never spec-to-implementation). See TE-zukug for the inversion thesis and TE-liviv for the A/B split that disambiguates which CHANGELOG carries which semantics.
26. **OQ-29.2 — Leaf filename in `transports/...` (from TE-vipir).** Should `<message-id>.txt` use a content hash (free cross-transport deduplication, hash-as-identity), a ULID (trivially sortable, ordering preserved by filename), or both (`<ulid>-<hash>.txt`)? Lean: hash; ordering recovered from session-layer Parents header.
27. **OQ-29.3 — Empty-leaf rule (from TE-vipir).** Is `transports/<wire>/<binding-pCID>/` allowed to exist with no session-protocol subdirectory beneath it (meaning the transport+binding is provisioned but no session has spoken yet)? Lean: yes — honest about provisioning state.
28. **OQ-29.4 — Framing intermediate layer (from TE-vipir).** Do we need a separate framing layer between the L4-binding and the session protocol, or does each binding own its framing? Lean: each binding owns its framing (no reuse to be had across UDP datagrams vs. TCP byte-stream vs. SMTP body vs. MQTT publishes).
29. **OQ-29.5 — Binding-layer signing (from TE-vipir / C-6).** May a binding sign per-frame for spam resistance independently of session and message-layer signing? Lean: yes-permitted but out of scope for v0 specs.
30. **OQ-29.6 — Negotiation/handshake protocol (from TE-vipir / C-1).** Where does "agree on the (binding, session, message) tuple" live? Probably its own protocol on a bootstrap transport. Future work; cannot be a singleton per C-1.
31. **OQ-29.7 — Real-world transport slug list (from TE-vipir / C-1).** Who decides which slugs appear at level 1 of `transports/<slug>/`? Lean: any author may introduce a new directory; collisions resolve by C-1 (no central registry); practical norm is to use IANA names where they exist.
32. **OQ-29.8 — Adversarial test-harness placement (from TE-vipir / C-3).** Does the wire-lab simulator emulate loss, reordering, latency, partition? If yes, where does the emulation live in the directory shape? Lean: adversarial harness lives in `tools/` and produces test vectors that violate binding promises in controlled ways.
33. **OQ-29.9 — External network-simulator integration, ns-3 or similar (from TE-vipir; revised 2026-05-01 with empirical sandbox data).** Once at least two binding specs and one session protocol have v0 implementations in `tools/`, can the wire-lab harness drive a packet-level simulator to produce realistic loss/latency adversarial scenarios? Empirical results from the Perplexity Computer sandbox (Debian trixie, kernel 6.1.158): ns-3 3.43 works (Debian package, CMake+Ninja+pkg-config, UDP-echo+PCAP smoke-tested); Mininet 2.3.0 works with LinuxBridge backend (requires `PYTHONPATH=/usr/lib/python3/dist-packages`); raw netns+veth+bridge works under sudo; **`tc netem` is unavailable** (kernel lacks `sch_netem`, `tbf`, `fq_codel`); OMNeT++ and Shadow are not packaged. Revised lean: **adopt ns-3 as the primary network-realism fixture** (it is in fact lower-friction in the sandbox than `tc netem`, which the original lean had recommended first). Mininet remains useful for topology-only experiments. Steve's local dev box has `tc netem` and may use it; the sandbox does not. Integration shape unchanged: ns-3 emulates the wire, Go reference implementations run as applications above sockets via tap-bridge or DCE, PCAP output post-processed into `transports/<wire>/<binding-pCID>/.../<msg-id>.txt` artifacts. Future home: `tools/ns3-harness/`. Warrants a dedicated TE once the first Go binding+session implementations exist.
