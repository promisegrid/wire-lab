...
- **[TE-mupoz: Root and protocol migration scope under simulations](docs/thought-experiments/TE-mupoz-root-protocol-migration-scope-under-simulations.md)** — Tests how much existing root-level and `protocols/` content should move if TE-nizor's Pahah-with-contract recommendation is implemented. Locks DF-mupoz.3 via `DI-pakid`: root `protocols/` contains only `wire-lab.d`, and candidate protocol trees such as `group-session`, `udp-binding`, and `ppx-dr` move into simulations as specimens. Other DFs still recommend moving concrete `transports/wire-lab-devs-draft/` evidence into the first simulation with a source-path/source-commit/CID migration manifest and writing new concrete world state under `simulations/<sim>/world/`; decision status is `needs DF`.
