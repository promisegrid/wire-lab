...
26. **OQ-29.2 — Leaf filename in `transports/...` (from TE-29).** Should `<message-id>.txt` use a content hash (free cross-transport deduplication, hash-as-identity), a ULID (trivially sortable, ordering preserved by filename), or both (`<ulid>-<hash>.txt`)? Lean: hash; ordering recovered from session-layer Parents header.
