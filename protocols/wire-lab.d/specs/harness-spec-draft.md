...
- **[TE-dojab: Simulation run model and scenario/result matrix](docs/thought-experiments/TE-dojab-simulation-run-model-and-scenario-result-matrix.md)** — Tests Steve's lean toward root `scenarios/`, root `results/<sim-id>/<scenario-id>/<model-id>/<YYYYMMDD-HHMMSS>.md`, Codex-only first runs, and a 2D scenario-by-simulation matrix. Recommends that root scenarios and root results be treated as harness comparison apparatus, while results remain evidence that graduates through DR/DI/spec/guide handoff rather than direct design authority. Result paths use specific provider/model/reasoning slugs and UTC timestamps per `DI-miror`; the broader TE decision status is `needs DF`.
