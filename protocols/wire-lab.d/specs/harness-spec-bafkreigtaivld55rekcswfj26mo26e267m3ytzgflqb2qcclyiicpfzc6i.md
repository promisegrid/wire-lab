...
Each TE is a falsifiable experiment whose outcome teaches us something about the right design. There are currently fifteen, listed in chronological order of first drafting. The TE numbers are stable identifiers used throughout the spec; on disk each TE lives in a timestamped, content-addressable file under [`docs/thought-experiments/`](docs/thought-experiments/). Summaries below; click through for the full design and any findings.
